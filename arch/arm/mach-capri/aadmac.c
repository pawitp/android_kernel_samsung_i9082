...
#define AADMA_48KHZ_MONO_IGR_FIFO_SIZE    32
#define AADMA_48KHZ_MONO_EGR_FIFO_SIZE    512
#define AADMA_48KHZ_STEREO_IGR_FIFO_SIZE  32
#define AADMA_48KHZ_STEREO_EGR_FIFO_SIZE  1024

#define AADMA_8KHZ_MONO_IGR_FIFO_SIZE     32
#define AADMA_8KHZ_MONO_EGR_FIFO_SIZE     32
#define AADMA_8KHZ_STEREO_IGR_FIFO_SIZE   32
#define AADMA_8KHZ_STEREO_EGR_FIFO_SIZE   32
...
#define CFIFO_NUM_SEG_PER_SLOT      \
	(CFIFO_SLOT_SIZE_BYTES / CFIFO_SEG_SIZE_BYTES)
#define CFIFO_NUM_SLOT_IN_BUF       \
	(CFIFO_TOTAL_BUF_SIZE / CFIFO_SLOT_SIZE_BYTES)
...
static spinlock_t gAADmaDevLock;	/* acquired when setting device
					 * handler
					 */
...
{ \
	...
	.cfifo_size = AADMA_48KHZ_MONO_IGR_FIFO_SIZE, \
...
{ \
	...
	.cfifo_size = AADMA_48KHZ_MONO_EGR_FIFO_SIZE, \
...
{ \
	...
	.aadma_trans_size = AADMA_DEF_BURST_SIZE,     \
	.cfifo_dir = CAPH_CFIFO_IN, \
	.cfifo_size = AADMA_48KHZ_STEREO_EGR_FIFO_SIZE, \
	.cfifo_thres1 = AADMA_DEF_EGR_THRES1, \
	.cfifo_thres2 = AADMA_DEF_THRES2, \
...
#define DEVICE_AUDIOH_MEM_TO_CFIFO_TO_HANDSFREE_STEREO \
{ \
	.flags = AADMA_DEVICE_FLAG_SECURE, \
	.name = "audioh mem to handsfree", \
	.aadma_trans_size = AADMA_DEF_BURST_SIZE,     \
...
#define DEVICE_PCM_SSP6_TO_CFIFO_TO_MEM \
{ \
	.flags = AADMA_DEVICE_FLAG_SECURE,           \
	.name = "pcm ssp6 to mem",                   \
	.aadma_trans_size = AADMA_DEF_BURST_SIZE,    \
	.cfifo_dir = CAPH_CFIFO_OUT,                 \
	.cfifo_size = AADMA_8KHZ_MONO_IGR_FIFO_SIZE, \
	.cfifo_thres1 = AADMA_DEF_IGR_THRES1,        \
	.cfifo_thres2 = AADMA_DEF_THRES2,            \
	.dev_fifo_addr = SSP6_BASE_ADDR + SSPIL_FIFO_ENTRY0RX_OFFSET, \
}

#define DEVICE_PCM_MEM_TO_CFIFO_TO_SSP6 \
{ \
	.flags = AADMA_DEVICE_FLAG_SECURE,           \
	.name = "pcm mem to ssp6",                   \
	.aadma_trans_size = AADMA_DEF_BURST_SIZE,    \
	.cfifo_dir = CAPH_CFIFO_IN,                  \
	.cfifo_size = AADMA_8KHZ_MONO_EGR_FIFO_SIZE, \
	.cfifo_thres1 = AADMA_DEF_EGR_THRES1,        \
	.cfifo_thres2 = AADMA_DEF_THRES2,            \
	.dev_fifo_addr = SSP6_BASE_ADDR + SSPIL_FIFO_ENTRY0TX_OFFSET, \
}

AADMA_DeviceAttribute_t AADMA_gDeviceAttribute[AADMA_NUM_DEVICE_ENTRIES] = {
	[AADMA_DEVICE_AUDIOH_VINR_CFIFO_TO_MEM] =
	    DEVICE_AUDIOH_VINR_CFIFO_TO_MEM,
	[AADMA_DEVICE_AUDIOH_VINL_CFIFO_TO_MEM] =
	    DEVICE_AUDIOH_VINL_CFIFO_TO_MEM,
	[AADMA_DEVICE_AUDIOH_NVINR_CFIFO_TO_MEM] =
	    DEVICE_AUDIOH_NVINR_CFIFO_TO_MEM,
	[AADMA_DEVICE_AUDIOH_NVINL_CFIFO_TO_MEM] =
	    DEVICE_AUDIOH_NVINL_CFIFO_TO_MEM,
	[AADMA_DEVICE_AUDIOH_MEM_TO_CFIFO_TO_EARPIECE] =
	    DEVICE_AUDIOH_MEM_TO_CFIFO_TO_EARPIECE,
	[AADMA_DEVICE_AUDIOH_MEM_TO_CFIFO_TO_HEADSET] =
	    DEVICE_AUDIOH_MEM_TO_CFIFO_TO_HEADSET,
	[AADMA_DEVICE_AUDIOH_MEM_TO_CFIFO_TO_HANDSFREE] =
	    DEVICE_AUDIOH_MEM_TO_CFIFO_TO_HANDSFREE,
	[AADMA_DEVICE_AUDIOH_MEM_TO_CFIFO_TO_HANDSFREE_STEREO] =
	    DEVICE_AUDIOH_MEM_TO_CFIFO_TO_HANDSFREE_STEREO,
	[AADMA_DEVICE_AUDIOH_MEM_TO_CFIFO_TO_VIBRA] =
	    DEVICE_AUDIOH_MEM_TO_CFIFO_TO_VIBRA,
	[AADMA_DEVICE_PCM_SSP6_TO_CFIFO_TO_MEM] =
	    DEVICE_PCM_SSP6_TO_CFIFO_TO_MEM,
	[AADMA_DEVICE_PCM_MEM_TO_CFIFO_TO_SSP6] =
	    DEVICE_PCM_MEM_TO_CFIFO_TO_SSP6,
};
...
static int aadma_proc_read_channels(char *buf, char **start, off_t offset,
				    int count, int *eof, void *data)
{
	...
	for (channelIdx = 0; channelIdx < AADMA_NUM_CHANNELS; channelIdx++) {
		...
		len +=
		    sprintf(buf + len, "Fifo: %d  ",
			    AADMA_gDeviceAttribute[channel->
						   devType].cfifo_size);
...
static int aadma_proc_read_devices(char *buf, char **start, off_t offset,
				   int count, int *eof, void *data)
{
	int limit = count - 200;
	int len = 0;
	...
	for (devIdx = 0; devIdx < AADMA_NUM_DEVICE_ENTRIES; devIdx++) {
		...
		len += sprintf(buf + len,
			       ...
			       devAttr->transferTicks, devAttr->transferBytes);
	}
...
{
	return (device >= 0)
	    && (device < (AADMA_Device_t) AADMA_NUM_DEVICE_ENTRIES);
}
...
static irqreturn_t aadma_interrupt_handler(int irq, void *dev_id)
{
	...
	status =
	    chal_caph_intc_read_dma_intr(chal_hdl->chalAhintcHandle, CAPH_ARM);
	...
	for (i = 0; i < AADMA_NUM_CHANNELS; i++) {
		...
		if (status & dma_ch) {
			...
			/* Obtain DMA status */
			dma_status =
			    chal_caph_dma_read_ddrfifo_status
			    (chal_hdl->chalAadmacHandle, dma_ch);

			/* Always clear status */
			chal_caph_dma_clr_ddrfifo_status
			    (chal_hdl->chalAadmacHandle, dma_ch, dma_status);
			...
			if (found) {
				...
				devAttr =
				    &AADMA_gDeviceAttribute[channel->devType];
	...
	chal_caph_intc_clr_dma_intr(chal_hdl->chalAhintcHandle, status,
				    CAPH_ARM);

	return IRQ_HANDLED;
}

static int aadma_request_cfifo_mem_alloc(uint32_t *start_addr,
					 uint16_t *req_bytes, int fifo_ch)
{
	int i, j;
	...
	/* Special case where sizes less than 2 slots will default to a single
	 * segment
	 */
	...
	for (i = 0; i < CFIFO_NUM_SLOT_IN_BUF; i++, cfifo_buf_end_p++) {
		...
		if (num_slots > 0) {
			if (cfifo_buf_end_p->num_seg_allocated == 0) {
				...
				/* Track the amount of contiguous slots until
				 * we reach the desired goal
				 */
				num_contiguous_slots++;
			} else {
				/* Found a slot with allocated segments so
				 * reset contiguous tracker
			...
			if (num_contiguous_slots >= num_slots) {
				...
				while (cfifo_buf_start_p <= cfifo_buf_end_p) {
					...
					for (j = 0; j < CFIFO_NUM_SEG_PER_SLOT;
					     j++) {
						cfifo_buf_start_p->
						    seg[j].seg_allocated = 1;
						cfifo_buf_start_p->
						    seg[j].seg_alloc_ch_id =
						    fifo_ch;
				...
				*start_addr =
				    ((i + 1) -
				     num_slots) * CFIFO_SLOT_SIZE_BYTES;
				*req_bytes = bytes;
				done = 1;
				break;
			}
			...
			if (cfifo_buf_end_p->num_seg_allocated <
			    CFIFO_NUM_SEG_PER_SLOT) {
				/* Still available segments to be allocated */
				for (j = 0; j < CFIFO_NUM_SEG_PER_SLOT; j++) {
					if (cfifo_buf_start_p->
					    seg[j].seg_allocated == 0) {
						cfifo_buf_start_p->
						    seg[j].seg_allocated = 1;
						cfifo_buf_start_p->
						    seg[j].seg_alloc_ch_id =
				...
				*start_addr =
				    (i * CFIFO_SLOT_SIZE_BYTES) +
				    (j * CFIFO_SEG_SIZE_BYTES);
				*req_bytes = bytes;
				done = 1;
				break;
			}
	...
	for (i = 0; i < CFIFO_NUM_SLOT_IN_BUF; i++, cfifo_buf_p++) {
		for (j = 0; j < CFIFO_NUM_SEG_PER_SLOT; j++) {
			if (cfifo_buf_p->num_seg_allocated > 0) {
				if (cfifo_buf_p->seg[j].seg_allocated
				    && cfifo_buf_p->seg[j].seg_alloc_ch_id ==
				    fifo_ch) {
...
static int aadma_reset_intc(AADMA_ChalHandle_t * chal_handle)
{
	...
	chal_caph_intc_set_cfifo_int_detect_level(chal_handle->chalAhintcHandle,
						  0xFFFF, 0x0000);
	return 0;
}
...
static int aadma_ioremap_init(AADMA_ChalHandle_t * chal_handle)
{
	...
	cfifo_ioarea =
	    request_mem_region(CFIFO_PHYS_BASE_ADDR_START,
			       CFIFO_REGISTER_LENGTH,
			       "CFIFO Component Mem Region");
	aadmac_ioarea =
	    request_mem_region(AADMAC_PHYS_BASE_ADDR_START,
			       AADMAC_REGISTER_LENGTH,
			       "AADMAC Component Mem Region");
	ahintc_ioarea =
	    request_mem_region(AHINTC_PHYS_BASE_ADDR_START,
			       AHINTC_REGISTER_LENGTH,
			       "AHINTC Component Mem Region");

	if (!cfifo_ioarea || !aadmac_ioarea || !ahintc_ioarea)
		return -EBUSY;

	cfifo_virt_addr =
	    ioremap(CFIFO_PHYS_BASE_ADDR_START, CFIFO_REGISTER_LENGTH);
	aadmac_virt_addr =
	    ioremap(AADMAC_PHYS_BASE_ADDR_START, AADMAC_REGISTER_LENGTH);
	ahintc_virt_addr =
	    ioremap(AHINTC_PHYS_BASE_ADDR_START, AHINTC_REGISTER_LENGTH);

	if (!cfifo_virt_addr || !aadmac_virt_addr || !ahintc_virt_addr)
		return -ENOMEM;

	chal_handle->chalAadmacHandle =
	    chal_caph_dma_init((uint32_t)aadmac_virt_addr);
	chal_handle->chalCfifoHandle =
	    chal_caph_cfifo_init((uint32_t)cfifo_virt_addr);
	chal_handle->chalAhintcHandle =
	    chal_caph_intc_init((uint32_t)ahintc_virt_addr);
...
*/
/****************************************************************************/
uint32_t aadma_get_cfifo_address(AADMA_Handle_t handle	/* DMA handle. */
    )
{
	...
	cfifo_addr = (uint16_t)(CFIFO_PHYS_BASE_ADDR_START +
				chal_caph_cfifo_get_fifo_addr_offset
				(chal_handle->chalCfifoHandle,
				 channel->caph_cfifo_ch));
...
int aadma_init(void)
{
	...
	printk(KERN_INFO "%s: initializing AADMA\n", __func__);

	rc = aadma_ioremap_init(&gChalHandle);
	if (rc != 0) {
...
int aadma_setup(void)
{
	...
	printk(KERN_INFO "%s: Setting up AADMA\n", __func__);
	...
	rc = request_irq(BCM_INT_ID_CAPH, aadma_interrupt_handler,
			 IRQF_DISABLED, "AADMA Handler", &gChalHandle);
	if (rc != 0) {
	...
	if (gDmaDir == NULL) {
		...
		create_proc_read_entry("channels", 0, gDmaDir,
				       aadma_proc_read_channels, NULL);
		create_proc_read_entry("devices", 0, gDmaDir,
				       aadma_proc_read_devices, NULL);
	}
...
#if (AADMA_DEBUG_TRACK_RESERVATION)
AADMA_Handle_t aadma_request_channel_dbg(AADMA_Device_t dev,
					 const char *fileName, int lineNum)
#else
AADMA_Handle_t aadma_request_channel(AADMA_Device_t dev)
#endif
{
	...
	while (handle == AADMA_INVALID_HANDLE) {
		channelIdx = aadma_find_channel(gAADMA.channel);
		if (channelIdx >= 0) {
			channel = &gAADMA.channel[channelIdx];

			channel->caph_dma_ch =
			    chal_caph_dma_alloc_given_channel
			    (chal_handle->chalAadmacHandle, CAPH_DMA_CH_VOID);
			if ((CAPH_DMA_CHANNEL_e) channel->caph_dma_ch ==
			    CAPH_DMA_CH_VOID) {
				printk(KERN_ERR
				       "Failed to request CAPH AADMA channel\n");
				goto error;
			}

			channel->caph_cfifo_ch =
			    chal_caph_cfifo_alloc_given_channel
			    (chal_handle->chalCfifoHandle, CAPH_CFIFO_VOID);
			if ((CAPH_CFIFO_e) channel->caph_dma_ch ==
			    CAPH_CFIFO_VOID) {
				printk(KERN_ERR
				       "Failed to request CAPH CFIFO channel\n");
				goto error;
			}
			...
			if (aadma_request_cfifo_mem_alloc
			    (&startAddr, &devAttr->cfifo_size,
			     channel->caph_cfifo_ch)) {
				/* Failed to reserve enough data in CFIFO
				 * buffers
				 */
				printk(KERN_ERR
				       "%s Failed to reserve enough data in CFIFO buffers\n",
				       __func__);
				goto error;
			}

			/* Configure DMA burst size */
			chal_caph_dma_set_tsize(chal_handle->chalAadmacHandle,
						channel->caph_dma_ch,
						devAttr->aadma_trans_size);

			/* Link CFIFO with DMA channel */
			chal_caph_dma_set_cfifo(chal_handle->chalAadmacHandle,
						channel->caph_dma_ch,
						channel->caph_cfifo_ch);

			/* Set CFIFO address */
			chal_caph_cfifo_set_address
			    (chal_handle->chalCfifoHandle,
			     channel->caph_cfifo_ch, startAddr);

			/* Configure DMA direction */
			chal_caph_dma_set_direction
			    (chal_handle->chalAadmacHandle,
			     channel->caph_dma_ch, devAttr->cfifo_dir);

			/* Configure CFIFO direction */
			chal_caph_cfifo_set_direction
			    (chal_handle->chalCfifoHandle,
			     channel->caph_cfifo_ch, devAttr->cfifo_dir);

			/* Set CFIFO size */
			chal_caph_cfifo_set_size(chal_handle->chalCfifoHandle,
						 channel->caph_cfifo_ch,
						 devAttr->cfifo_size);

			/* Set CFIFO threshold */
			chal_caph_cfifo_set_fifo_thres
			    (chal_handle->chalCfifoHandle,
			     channel->caph_cfifo_ch, devAttr->cfifo_thres1,
			     devAttr->cfifo_thres2);

			/* Clear DMA channel FIFO */
			chal_caph_dma_clr_channel_fifo
			    (chal_handle->chalAadmacHandle,
			     channel->caph_dma_ch);

			/* Enable DMA interrupt for ARM */
			chal_caph_intc_enable_dma_intr
			    (chal_handle->chalAhintcHandle,
			     channel->caph_dma_ch, CAPH_ARM);
			chal_caph_intc_enable_dma_int_err
			    (chal_handle->chalAhintcHandle,
			     channel->caph_dma_ch, CAPH_ARM);

			/* Clear CFIFO */
			chal_caph_cfifo_clr_fifo(chal_handle->chalCfifoHandle,
						 channel->caph_cfifo_ch);

			/* Enable CFIFOF error interrupt */
			chal_caph_intc_enable_cfifo_int_err
			    (chal_handle->chalAhintcHandle,
			     (uint16_t)channel->caph_cfifo_ch, CAPH_ARM);
...
error:
	if (handle != AADMA_INVALID_HANDLE) {
		handle = AADMA_INVALID_HANDLE;
		chal_caph_dma_free_channel(chal_handle->chalAadmacHandle,
					   channel->caph_dma_ch);
		chal_caph_cfifo_free_channel(chal_handle->chalCfifoHandle,
					     channel->caph_cfifo_ch);
		channel->flags &= ~AADMA_CHANNEL_FLAG_IN_USE;
		devAttr->flags &= ~AADMA_DEVICE_FLAG_IN_USE;
	}
...
int aadma_free_channel(AADMA_Handle_t handle	/* DMA handle. */
    )
{
	int rc = 0;
	AADMA_Channel_t *channel;
...
*/
/****************************************************************************/
int aadma_enable(AADMA_Handle_t handle	/* AADMA handle. */
    )
{
	AADMA_Channel_t *channel;
	AADMA_ChalHandle_t *chal_handle;
	...
	/*Enable CFIFO */
	chal_caph_cfifo_enable(chal_handle->chalCfifoHandle,
			       channel->caph_cfifo_ch);

	/* Enable DMA */
	chal_caph_dma_enable(chal_handle->chalAadmacHandle,
			     channel->caph_dma_ch);
...
*/
/****************************************************************************/
int aadma_set_device_fifo_address(AADMA_Device_t dev, uint32_t dev_fifo_addr)
{
	AADMA_DeviceAttribute_t *devAttr;
	...
	if ((devAttr->flags & AADMA_DEVICE_FLAG_IN_USE)) {
		up(&gAADMA.lock);
		printk(KERN_ERR
		       "%s Unable to set FIFO address as device currently in use\n",
		       __func__);
		return -EBUSY;
	}
...
*/
/****************************************************************************/
int aadma_set_device_fifo_size(AADMA_Device_t dev, uint16_t fifo_size)
{
	AADMA_DeviceAttribute_t *devAttr;
	...
	if ((devAttr->flags & AADMA_DEVICE_FLAG_IN_USE)) {
		up(&gAADMA.lock);
		printk(KERN_ERR
		       "%s Unable to set FIFO size as device currently in use\n",
		       __func__);
		return -EBUSY;
	}
...
*/
/****************************************************************************/
int aadma_set_device_thres(AADMA_Device_t dev, uint16_t thres1, uint16_t thres2)
{
	AADMA_DeviceAttribute_t *devAttr;
	...
	if ((devAttr->flags & AADMA_DEVICE_FLAG_IN_USE)) {
		up(&gAADMA.lock);
		printk(KERN_ERR
		       "%s Unable to set FIFO threshold as device currently in use\n",
		       __func__);
		return -EBUSY;
	}
...
int aadma_alloc_descriptors(AADMA_Handle_t handle,	/* DMA Handle */
			    dma_addr_t data_addr,
			    /* Place to read/write data to/from device */
			    size_t numBytes
    ...
    )
{
	...
	/* Set DDR address of buffer.  Assumes contiguous lo/hi buffer so must
	 * double the number of bytes
	 */
	chal_caph_dma_set_buffer(chal_handle->chalAadmacHandle,
				 channel->caph_dma_ch, data_addr,
				 (numBytes * 2));
...
int aadma_alloc_ring_descriptors(AADMA_Handle_t handle,	/* DMA Handle */
				 dma_addr_t data_addr_lo,
				 /* Place to read/write low buffer data to/from device */
				 dma_addr_t data_addr_hi,
				 /* Place to read/write high buffer data to/from device */
				 size_t numBytes
    ...
    )
{
	...
	/* Set low buffer DDR address */
	chal_caph_dma_set_buffer(chal_handle->chalAadmacHandle,
				 channel->caph_dma_ch, data_addr_lo,
				 (numBytes * 2));

	/* Set high buffer DDR address */
	chal_caph_dma_set_hibuffer(chal_handle->chalAadmacHandle,
				   channel->caph_dma_ch, data_addr_hi);
...
int aadma_start_transfer(AADMA_Handle_t handle	/* DMA Handle */
    )
{
	AADMA_Channel_t *channel;
	AADMA_DeviceAttribute_t *devAttr;
	...
	if (gKnllogIrqSchedEnable & KNLLOG_DMA) {
		KNLLOG("tstart [%s hdl=%u devType=%u bytes=%u]\n",
		       devAttr->name,
		       handle, channel->devType, devAttr->numBytes);
	}
#endif
	...
	if (channel->ring_buffer) {
		...
		chal_caph_dma_set_ddrfifo_status(chal_handle->chalAadmacHandle,
						 channel->caph_dma_ch,
						 CAPH_READY_HIGHLOW);
	} else {
		/* Enable Low buffer */
...
int aadma_continue_transfer(AADMA_Handle_t handle,	/* DMA Handle */
...
int aadma_stop_transfer(AADMA_Handle_t handle)
{
	...
	chal_caph_dma_set_ddrfifo_status(chal_handle->chalAadmacHandle,
					 channel->caph_dma_ch, CAPH_READY_NONE);
	chal_caph_dma_clr_channel_fifo(chal_handle->chalAadmacHandle,
				       channel->caph_cfifo_ch);
	chal_caph_dma_disable(chal_handle->chalAadmacHandle,
			      channel->caph_cfifo_ch);

	chal_caph_intc_clr_dma_intr(chal_handle->chalAhintcHandle,
				    (uint16_t)channel->caph_dma_ch, CAPH_ARM);
	chal_caph_intc_clr_dma_int_err(chal_handle->chalAhintcHandle,
				       (uint16_t)channel->caph_dma_ch,
				       CAPH_ARM);

	chal_caph_intc_disable_dma_intr(chal_handle->chalAhintcHandle,
					(uint16_t)channel->caph_dma_ch,
					CAPH_ARM);
	chal_caph_intc_disable_dma_int_err(chal_handle->chalAhintcHandle,
					   (uint16_t)channel->caph_dma_ch,
					   CAPH_ARM);

	chal_caph_cfifo_int_disable(chal_handle->chalCfifoHandle,
				    channel->caph_cfifo_ch);
	chal_caph_cfifo_int_err_disable(chal_handle->chalCfifoHandle,
					channel->caph_cfifo_ch);

	chal_caph_cfifo_disable(chal_handle->chalCfifoHandle,
				channel->caph_cfifo_ch);
	chal_caph_cfifo_clr_fifo(chal_handle->chalCfifoHandle,
				 channel->caph_cfifo_ch);

	chal_caph_intc_disable_cfifo_intr(chal_handle->chalAhintcHandle,
					  channel->caph_cfifo_ch, CAPH_ARM);
	chal_caph_intc_disable_cfifo_int_err(chal_handle->chalAhintcHandle,
					     channel->caph_cfifo_ch, CAPH_ARM);
...
int aadma_transfer(AADMA_Handle_t handle,	/* DMA Handle */
		   dma_addr_t data_addr,
		   /* Place to read/write data to/from device */
		   size_t numBytes
		   /* Number of bytes to transfer to the device */
    )
{
	...
	rc = aadma_alloc_descriptors(handle, data_addr, numBytes);
	if (rc != 0)
		return rc;
...
int aadma_set_device_handler(AADMA_Device_t dev,
			     /* Device to set the callback for. */
			     AADMA_DeviceHandler_t devHandler,
