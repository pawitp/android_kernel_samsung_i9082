...
CHAL_DMUX_STATUS_t chal_dmux_alloc_channel(CHAL_HANDLE handle, uint32_t *chan)
{
	...
	channel =
	    CHAL_REG_READ32(pDmuxDev->baseAddr +
			    DMUX_CHAN_ALLOC_DEALLOC_OFFSET) &
	    DMUX_CHAN_ALLOC_DEALLOC_CHANNEL_AD_MASK;
 ...
 * ****************************************************************************
 */
CHAL_DMUX_STATUS_t chal_dmux_dealloc_channel(CHAL_HANDLE handle,
					     uint32_t channel)
{
	chal_dmux_t *pDmuxDev = (chal_dmux_t *) handle;

	CHAL_REG_WRITE32(pDmuxDev->baseAddr + DMUX_CHAN_ALLOC_DEALLOC_OFFSET,
			 channel);
 ...
 * ****************************************************************************
 */
CHAL_DMUX_STATUS_t chal_dmux_alloc_peripheral(CHAL_HANDLE handle,
					      uint32_t channel,
					      ...
					      uint8_t *src_id, uint8_t *dst_id)
{
	...
	if ((peri_a != CHAL_DMUX_EPT_INVALID)
	    && (peri_b != CHAL_DMUX_EPT_INVALID)) {
		val1 = peri_a | (peri_b << DMUX_PER_0_PER_B_0_SHIFT);
	} else if (peri_a != CHAL_DMUX_EPT_INVALID) {
		val1 =
		    peri_a | (CHAL_DMUX_PERI_INVALID <<
			      DMUX_PER_0_PER_B_0_SHIFT);
	} else {
		val1 =
		    peri_b | (CHAL_DMUX_PERI_INVALID <<
			      DMUX_PER_0_PER_B_0_SHIFT);
	}

	CHAL_REG_WRITE32(pDmuxDev->baseAddr + DMUX_PER_0_OFFSET + channel * 4,
			 val1);

	do {
		val2 =
		    CHAL_REG_READ32(pDmuxDev->baseAddr + DMUX_PER_0_OFFSET +
				    channel * 4);
 ...
 * ****************************************************************************
 */
CHAL_DMUX_STATUS_t chal_dmux_dealloc_peripheral(CHAL_HANDLE handle,
						uint32_t channel)
{
	chal_dmux_t *pDmuxDev = (chal_dmux_t *) handle;
