...
typedef struct CHAL_IPC_DEV_T {
	void __iomem *ipc_open_reg_base;
	void __iomem *ipc_secure_reg_base;
 ...
 * ******************************************************************************
 */
CHAL_IPC_HANDLE chal_ipc_config(CHAL_IPC_CONFIG_T * pConfig)
{
	CHAL_UNUSED(pConfig);
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_write_mailbox(CHAL_IPC_HANDLE handle,
				    IPC_MAILBOX_ID mailboxId, uint32_t value)
{
	...
	CHAL_REG_WRITE32(device->ipc_secure_reg_base + IPCSEC_IPCMAIL0_OFFSET +
			 mailboxId * sizeof(uint32_t), value);
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_read_mailbox(CHAL_IPC_HANDLE handle,
				   IPC_MAILBOX_ID mailboxId, uint32_t *value)
{
	CHAL_IPC_DEV_T *device;
	...
	*value =
	    CHAL_REG_READ32(device->ipc_secure_reg_base +
			    IPCSEC_IPCMAIL0_OFFSET +
			    mailboxId * sizeof(uint32_t));
...
BCM_ERR_CODE chal_ipc_query_wakeup_vc(CHAL_IPC_HANDLE handle, uint32_t *result)
{
	CHAL_IPC_DEV_T *device;
...
#ifdef KONA_SECURE_MONITOR_CALL
	*result = wakeup_addr | wakeup_en;
#else
	*result =
	    CHAL_REG_READ32(device->ipc_secure_reg_base +
			    IPCSEC_IPCAWAKE_OFFSET);
#endif
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_wakeup_vc(CHAL_IPC_HANDLE handle, uint32_t address)
{
	CHAL_IPC_DEV_T *device;
	...
	device = (CHAL_IPC_DEV_T *) handle;
	printk(KERN_INFO "Wakeup VideoCore\n");

#ifdef CONFIG_KONA_SECURE_MONITOR_CALL
	wakeup_addr = address;
	wakeup_en = IPCSEC_IPCAWAKE_WAKEUP_MASK;
#ifdef CONFIG_MOBICORE_DRIVER
	secure_api_call(SMC_BCM_VC_CORE_START,
#else
	secure_api_call(SSAPI_BRCM_START_VC_CORE,
#endif
			wakeup_addr | IPCSEC_IPCAWAKE_WAKEUP_MASK, 0, 0, 0);
#else
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_sleep_vc(CHAL_IPC_HANDLE handle)
{
	CHAL_IPC_DEV_T *device;
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_int_vcset(CHAL_IPC_HANDLE handle,
				IPC_INTERRUPT_SOURCE irqNum)
{
	...
	CHAL_REG_WRITE32(device->ipc_open_reg_base + IPCOPEN_IPCASET_OFFSET,
			 1 << irqNum);
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_int_clr(CHAL_IPC_HANDLE handle,
			      IPC_INTERRUPT_SOURCE irqNum)
{
	CHAL_IPC_DEV_T *device;
	...
	CHAL_REG_WRITE32(device->ipc_open_reg_base + IPCOPEN_IPCACLR_OFFSET,
			 1 << irqNum);
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_int_secmode(CHAL_IPC_HANDLE handle,
				  IPC_INTERRUPT_SOURCE irqNum,
				  IPC_INTERRUPT_MODE intMode)
{
	...
	if (intMode == IPC_INTERRUPT_MODE_OPEN) {
		CHAL_REG_CLRBIT32(device->ipc_secure_reg_base +
				  IPCSEC_IPCASECURE_OFFSET, 1 << irqNum);
	} else if (intMode == IPC_INTERRUPT_MODE_SECURE) {
		CHAL_REG_SETBIT32(device->ipc_secure_reg_base +
				  IPCSEC_IPCASECURE_OFFSET, 1 << irqNum);
	} else {
		BCM_DBG_EXIT();
		return (BCM_ERROR);
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_get_int_status(CHAL_IPC_HANDLE handle, uint32_t *status)
{
	CHAL_IPC_DEV_T *device;
	...
	*status =
	    CHAL_REG_READ32(device->ipc_open_reg_base +
			    IPCOPEN_IPCASTATUS_OFFSET);
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_get_int_source(CHAL_IPC_HANDLE handle,
 ...
 * ******************************************************************************
 */
BCM_ERR_CODE chal_ipc_get_error_status(CHAL_IPC_HANDLE handle, uint32_t *status)
{
	CHAL_IPC_DEV_T *device;
