...
typedef struct tag_chal_nand_ops_t {
	...
	uint32_t (*block_erase) (chal_nand_info_t * ni, uint8_t bank,
				 uint32_t block);
	uint32_t (*page_write) (chal_nand_info_t * ni, uint8_t bank,
				uint32_t page, const uint8_t *buf);
	uint32_t (*oob_write) (chal_nand_info_t * ni, uint8_t bank,
			       uint32_t page, const uint8_t *buf);
	uint32_t (*page_oob_write) (chal_nand_info_t * ni, uint8_t bank,
				    uint32_t page, const uint8_t *buf);
#endif
	uint32_t (*id_get) (chal_nand_info_t * ni, uint8_t bank, uint8_t addr,
			    uint8_t *buf);
	uint32_t (*page_read) (chal_nand_info_t * ni, uint8_t bank,
			       uint32_t page, uint8_t *buf);
	uint32_t (*oob_read) (chal_nand_info_t * ni, uint8_t bank,
			      uint32_t page, uint8_t *buf);
	uint32_t (*param_read) (chal_nand_info_t * ni, uint8_t bank,
				uint8_t *buf);
} chal_nand_ops_t;
...
static uint32_t uc_load_function(chal_nand_info_t * ni, chal_nand_uc_cmd_t cmd,
				 const uint32_t *ucode, uint16_t nb_instr);
static uint32_t nand_do_cmd(chal_nand_info_t * ni, chal_nand_uc_cmd_t cmd,
			    uint8_t bank, uint32_t page, void *buf);
...
/*============================================================================*/
/* pio/test methods */
static uint32_t page_write_pio(chal_nand_info_t * ni, uint8_t bank,
			       uint32_t page, const uint8_t *buf);
static uint32_t page_oob_write_pio(chal_nand_info_t * ni, uint8_t bank,
				   uint32_t page, const uint8_t *buf);
static uint32_t page_read_pio(chal_nand_info_t * ni, uint8_t bank,
			      uint32_t page, uint8_t *buf);
static uint8_t status_get_pio(chal_nand_info_t * ni, uint8_t bank);
static uint32_t id_get_pio(chal_nand_info_t * ni, uint8_t bank, uint8_t addr,
			   uint8_t *buf);
static uint32_t block_erase_pio(chal_nand_info_t * ni, uint8_t bank,
				uint32_t block);
static uint32_t oob_read_pio(chal_nand_info_t * ni, uint8_t bank, uint32_t page,
			     uint8_t *buf);
static uint32_t oob_write_pio(chal_nand_info_t * ni, uint8_t bank,
			      uint32_t page, const uint8_t *buf);
static uint32_t param_read_pio(chal_nand_info_t * ni, uint8_t bank,
			       uint8_t *buf);
#endif

/* "normal" methods */
#ifndef __BROM_CODE__
static uint32_t page_write(chal_nand_info_t * ni, uint8_t bank, uint32_t page,
			   const uint8_t *buf);
static uint32_t page_oob_write(chal_nand_info_t * ni, uint8_t bank,
			       uint32_t page, const uint8_t *buf);
static uint32_t oob_write(chal_nand_info_t * ni, uint8_t bank, uint32_t page,
			  const uint8_t *buf);
static uint32_t block_erase(chal_nand_info_t * ni, uint8_t bank,
			    uint32_t block);
static uint8_t status_get(chal_nand_info_t * ni, uint8_t bank);
#endif
static uint32_t page_read(chal_nand_info_t * ni, uint8_t bank, uint32_t page,
			  uint8_t *buf);
static uint32_t id_get(chal_nand_info_t * ni, uint8_t bank, uint8_t addr,
		       uint8_t *buf);
static uint32_t oob_read(chal_nand_info_t * ni, uint8_t bank, uint32_t page,
			 uint8_t *buf);
...
{
	CHAL_REG_WRITE32(NAND_CTRL_CTRL,
			 CHAL_REG_READ32(NAND_CTRL_CTRL) | NAND_CTRL_DMA_START);
}

static inline void dma_stop(void)
{
	CHAL_REG_WRITE32(NAND_CTRL_CTRL,
			 CHAL_REG_READ32(NAND_CTRL_CTRL) &
			 ~NAND_CTRL_DMA_START);
}
...
static inline int rb_wait(int bank, int usec)
{
	...
	while (1) {
		if (CHAL_REG_READ32(NAND_CTRL_IRQ_STATUS) &
		    (((0x1 << bank) << NAND_IRQ_RB_CHANGE_SHIFT) &
		     NAND_IRQ_RB_CHANGE_MASK)) {
			return 0;	/* ready */
		}
		if (usec <= 0) {
			return 1;	/* timeout */
...
{
	CHAL_REG_WRITE32(NAND_CTRL_IRQ_STATUS,
			 (CHAL_REG_READ32(NAND_CTRL_IRQ_STATUS) &
			  NAND_IRQ_RB_CHANGE_MASK));
}
...
* @tRHZ:             [in] tRHZ   (ns)
*******************************************************************************/
void chal_nand_timing(chal_nand_info_t * ni,
		      uint8_t tRP,
		      uint8_t tREH,
		      uint8_t tWP,
		      uint8_t tWH,
		      uint8_t tS,
		      uint8_t tH,
		      uint8_t tOE, uint8_t diff_tCEA_tREA, uint8_t tRHZ)
{
	CHAL_NAND_TIMING_CONF1(ni) =
	    (((tRP /
	       CHAL_NAND_TIMING_STEP) << NAND_CTRL_CONF1_TRP_SHIFT) &
	     NAND_CTRL_CONF1_TRP_MASK) | (((tREH / CHAL_NAND_TIMING_STEP) <<
					   NAND_CTRL_CONF1_TREH_SHIFT) &
					  NAND_CTRL_CONF1_TREH_MASK) |
	    (((tWP /
	       CHAL_NAND_TIMING_STEP) << NAND_CTRL_CONF1_TWP_SHIFT) &
	     NAND_CTRL_CONF1_TWP_MASK) | (((tWH / CHAL_NAND_TIMING_STEP) <<
					   NAND_CTRL_CONF1_TWH_SHIFT) &
					  NAND_CTRL_CONF1_TWH_MASK) |
	    (((tS /
	       CHAL_NAND_TIMING_STEP) << NAND_CTRL_CONF1_TS_SHIFT) &
	     NAND_CTRL_CONF1_TS_MASK) | (((tH / CHAL_NAND_TIMING_STEP) <<
					  NAND_CTRL_CONF1_TH_SHIFT) &
					 NAND_CTRL_CONF1_TH_MASK);

	CHAL_NAND_TIMING_CONF2(ni) =
	    (((tOE /
	       CHAL_NAND_TIMING_STEP) << NAND_CTRL_CONF2_TOE_SHIFT) &
	     NAND_CTRL_CONF2_TOE_MASK) |
	    (((diff_tCEA_tREA /
	       CHAL_NAND_TIMING_STEP) << NAND_CTRL_CONF2_TCEA_TREA_SHIFT) &
	     NAND_CTRL_CONF2_TCEA_TREA_MASK) | (((tRHZ / CHAL_NAND_TIMING_STEP)
						 << NAND_CTRL_CONF2_TRHZ_SHIFT)
						& NAND_CTRL_CONF2_TRHZ_MASK);
}
...
uint32_t chal_nand_init(chal_nand_info_t * ni, uint8_t flags)
{
	...
	if ((flags & CHAL_NAND_FLAG_DMA)
	    && (CHAL_NAND_DMA_SETUP_CB(ni) == NULL)) {
		return rc;
	}
 ...
 */
		...
		if ((CHAL_NAND_FLAGS(ni) & CHAL_NAND_FLAG_ECC)
			...
			lrc |=
			    uc_load_function(ni, CHAL_NAND_UC_RESET, uc_RESET,
					     N_ELEMENTS(uc_RESET));
			lrc |=
			    uc_load_function(ni, CHAL_NAND_UC_ID_GET, uc_ID_GET,
					     N_ELEMENTS(uc_ID_GET));
			lrc |=
			    uc_load_function(ni, CHAL_NAND_UC_PARAM_READ_PRE,
					     uc_PARAM_READ_PRE,
					     N_ELEMENTS(uc_PARAM_READ_PRE));
			lrc |=
			    uc_load_function(ni, CHAL_NAND_UC_PARAM_READ,
					     uc_PARAM_READ,
					     N_ELEMENTS(uc_PARAM_READ));

			if (CHAL_NAND_FLAGS(ni) & CHAL_NAND_FLAG_ECC)
#if ( CHAL_NAND_ECC_HM == 1 )
				CHAL_REG_WRITE32(NAND_CTRL_CTRL,
						 NAND_CTRL_NORMAL_INIT);
#else
				CHAL_REG_WRITE32(NAND_CTRL_CTRL,
						 (NAND_CTRL_NORMAL_INIT |
    ...
    if ((rc = chal_nand_id_get (ni, 0, 0x20, id)) == nand_rc(CHAL_NAND_RC_SUCCESS)) {
       if ((id[0]=='O')&&(id[1]=='N')&&(id[2]=='F')&&(id[3]=='I'))
       {
		...
		if (CHAL_NAND_BUS_WIDTH(ni) == 16)
			CHAL_REG_WRITE32(NAND_CTRL_CONF0,
					 NAND_CTRL_CONF0_GOLD |
					 NAND_CTRL_CONF0_BWIDTH_16);
...
*/
		...
		if (CHAL_NAND_FLAGS(ni) & CHAL_NAND_FLAG_DMA) {
			...
			lrc |=
			    uc_load_function(ni, CHAL_NAND_UC_PAGE_READ_PRE,
					     uc_PAGE_READ_PRE,
					     N_ELEMENTS(uc_PAGE_READ_PRE));
#ifndef __BROM_CODE__
			lrc |=
			    uc_load_function(ni, CHAL_NAND_UC_STATUS_GET,
					     uc_STATUS_GET,
					     N_ELEMENTS(uc_STATUS_GET));
			lrc |=
			    uc_load_function(ni, CHAL_NAND_UC_BLOCK_ERASE,
					     uc_BLOCK_ERASE,
					     N_ELEMENTS(uc_BLOCK_ERASE));
#endif
			...
			case 0x800:
				if (CHAL_NAND_FLAGS(ni) & CHAL_NAND_FLAG_ECC) {
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_READ_ECC,
							     uc_PAGE_READ_ECC,
							     N_ELEMENTS
							     (uc_PAGE_READ_ECC));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_READ_ECC,
							     uc_OOB_READ_ECC,
							     N_ELEMENTS
							     (uc_OOB_READ_ECC));
#ifndef __BROM_CODE__
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_WRITE_ECC,
							     uc_PAGE_WRITE_ECC,
							     N_ELEMENTS
							     (uc_PAGE_WRITE_ECC));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_WRITE_ECC,
							     uc_OOB_WRITE_ECC,
							     N_ELEMENTS
							     (uc_OOB_WRITE_ECC));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_OOB_WRITE_ECC,
							     uc_PAGE_OOB_WRITE_ECC,
							     N_ELEMENTS
							     (uc_PAGE_OOB_WRITE_ECC));
#endif
				} else {
			...
			case 0x1000:
				if (CHAL_NAND_FLAGS(ni) & CHAL_NAND_FLAG_ECC) {
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_READ_ECC_4K,
							     uc_PAGE_READ_ECC_4K,
							     N_ELEMENTS
							     (uc_PAGE_READ_ECC_4K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_READ_ECC_4K,
							     uc_OOB_READ_ECC_4K,
							     N_ELEMENTS
							     (uc_OOB_READ_ECC_4K));
#ifndef __BROM_CODE__
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_WRITE_ECC_4K,
							     uc_PAGE_WRITE_ECC_4K,
							     N_ELEMENTS
							     (uc_PAGE_WRITE_ECC_4K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_WRITE_ECC_4K,
							     uc_OOB_WRITE_ECC_4K,
							     N_ELEMENTS
							     (uc_OOB_WRITE_ECC_4K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_OOB_WRITE_ECC_4K,
							     uc_PAGE_OOB_WRITE_ECC_4K,
							     N_ELEMENTS
							     (uc_PAGE_OOB_WRITE_ECC_4K));
#endif
				} else {
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_READ_4K,
							     uc_PAGE_READ_4K,
							     N_ELEMENTS
							     (uc_PAGE_READ_4K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_READ_4K,
							     uc_OOB_READ_4K,
							     N_ELEMENTS
							     (uc_OOB_READ_4K));
#ifndef __BROM_CODE__
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_WRITE_4K,
							     uc_PAGE_WRITE_4K,
							     N_ELEMENTS
							     (uc_PAGE_WRITE_4K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_WRITE_4K,
							     uc_OOB_WRITE_4K,
							     N_ELEMENTS
							     (uc_OOB_WRITE_4K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_OOB_WRITE_4K,
							     uc_PAGE_OOB_WRITE_4K,
							     N_ELEMENTS
							     (uc_PAGE_OOB_WRITE_4K));
#endif
				}
			...
			case 0x2000:
				if (CHAL_NAND_FLAGS(ni) & CHAL_NAND_FLAG_ECC) {
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_READ_ECC_8K,
							     uc_PAGE_READ_ECC_8K,
							     N_ELEMENTS
							     (uc_PAGE_READ_ECC_8K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_READ_ECC_8K,
							     uc_OOB_READ_ECC_8K,
							     N_ELEMENTS
							     (uc_OOB_READ_ECC_8K));
#ifndef __BROM_CODE__
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_WRITE_ECC_8K,
							     uc_PAGE_WRITE_ECC_8K,
							     N_ELEMENTS
							     (uc_PAGE_WRITE_ECC_8K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_WRITE_ECC_8K,
							     uc_OOB_WRITE_ECC_8K,
							     N_ELEMENTS
							     (uc_OOB_WRITE_ECC_8K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_OOB_WRITE_ECC_8K,
							     uc_PAGE_OOB_WRITE_ECC_8K,
							     N_ELEMENTS
							     (uc_PAGE_OOB_WRITE_ECC_8K));
#endif
				} else {
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_READ_8K,
							     uc_PAGE_READ_8K,
							     N_ELEMENTS
							     (uc_PAGE_READ_8K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_READ_8K,
							     uc_OOB_READ_8K,
							     N_ELEMENTS
							     (uc_OOB_READ_8K));
#ifndef __BROM_CODE__
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_WRITE_8K,
							     uc_PAGE_WRITE_8K,
							     N_ELEMENTS
							     (uc_PAGE_WRITE_8K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_OOB_WRITE_8K,
							     uc_OOB_WRITE_8K,
							     N_ELEMENTS
							     (uc_OOB_WRITE_8K));
					lrc |=
					    uc_load_function(ni,
							     CHAL_NAND_UC_PAGE_OOB_WRITE_8K,
							     uc_PAGE_OOB_WRITE_8K,
							     N_ELEMENTS
							     (uc_PAGE_OOB_WRITE_8K));
#endif
				}
...
uint32_t chal_nand_id_get(chal_nand_info_t * ni,
			  uint8_t bank, uint8_t addr, uint8_t *buf)
{
...
uint32_t chal_nand_page_read(chal_nand_info_t * ni,
			     uint8_t bank, uint32_t page, uint8_t *buf)
{
...
uint32_t chal_nand_block_isbad(chal_nand_info_t * ni,
			       uint8_t bank, uint32_t block, uint8_t *is_bad)
{
	...
	off[1] = 1;
	off[2] =
	    (1 << (CHAL_NAND_BLOCK_SHIFT(ni) - CHAL_NAND_PAGE_SHIFT(ni))) - 1;
...
* @buf:     [out] target buffer
*******************************************************************************/
uint32_t chal_nand_param_read(chal_nand_info_t * ni, uint8_t bank, uint8_t *buf)
{
...
uint32_t id_get_pio(chal_nand_info_t * ni, uint8_t bank, uint8_t addr,
		    uint8_t *buf)
{
	uint16_t *id;
			      ...
			      uint32_t page, uint8_t *buf)
{
	...
	return (!len ? nand_rc(CHAL_NAND_RC_SUCCESS) :
		nand_rc(CHAL_NAND_RC_FAILURE));
}
...
uint32_t block_erase_pio(chal_nand_info_t * ni, uint8_t bank, uint32_t block)
{
	...
	CHAL_REG_WRITE32(NAND_CTRL_MADDR0,
			 block << (CHAL_NAND_BLOCK_SHIFT(ni) -
				   CHAL_NAND_PAGE_SHIFT(ni)));
	...
	return (status & NAND_STATUS_FAIL ? nand_rc(CHAL_NAND_RC_FAILURE) :
		nand_rc(CHAL_NAND_RC_SUCCESS));
}
...
uint32_t id_get(chal_nand_info_t * ni, uint8_t bank, uint8_t addr, uint8_t *buf)
{
	...
	if (rb_wait(bank, CHAL_NAND_TIMEOUT_RESET)) {
		return ((rc !=
			 nand_rc(CHAL_NAND_RC_SUCCESS)) ? rc :
			nand_rc(CHAL_NAND_RC_RB_TOUT));
	}
			  ...
			  uint8_t *buf)
{
	...
	rc = nand_do_cmd(ni, CHAL_NAND_UC_PAGE_READ, bank, page, (void *)buf);

	(CHAL_NAND_ECC_STATS(ni))[bank] =
	    CHAL_REG_READ32(NAND_CTRL_ECC_STATUS + (bank << 2));
			 ...
			 uint8_t *buf)
{
	...
	return nand_do_cmd(ni, CHAL_NAND_UC_OOB_READ, bank, page, (void *)buf);
}
...
static uint32_t param_read(chal_nand_info_t * ni, uint8_t bank, uint8_t *buf)
{
	...
	rc = nand_do_cmd(ni, CHAL_NAND_UC_PARAM_READ, bank, 0, buf);
...
uint32_t block_erase(chal_nand_info_t * ni, uint8_t bank, uint32_t block)
{
	...
	rc = nand_do_cmd(ni, CHAL_NAND_UC_BLOCK_ERASE, bank,
			 (block <<
			  (CHAL_NAND_BLOCK_SHIFT(ni) -
			   CHAL_NAND_PAGE_SHIFT(ni))), NULL);
			  ...
			  const uint8_t *buf)
{
	...
	rc = nand_do_cmd(ni, CHAL_NAND_UC_OOB_WRITE, bank, page, (void *)buf);
			   ...
			   const uint8_t *buf)
{
	...
	rb_clear();
	rc = nand_do_cmd(ni, CHAL_NAND_UC_PAGE_WRITE, bank, page, (void *)buf);
			       ...
			       uint32_t page, const uint8_t *buf)
{
	...
	rb_clear();
	rc = nand_do_cmd(ni,
			 CHAL_NAND_UC_PAGE_OOB_WRITE, bank, page, (void *)buf);
			    ...
			    uint8_t bank, uint32_t page, void *buf)
{
	...
	case CHAL_NAND_UC_ID_GET:
		...
		size = 8;
		dma_desc =
		    (NAND_CTRL_DMAIL_ENABLE | (bank << 10) | (size >> 3));
	...
	case CHAL_NAND_UC_PARAM_READ:
		...
		/* =>>  -9 +5 = -4                                                          */
		dma_desc =
		    ((size >> 4) | NAND_CTRL_DMAIL_ENABLE | (bank << 10));
		ctrl &= ~NAND_CTRL_2NAND;
	...
	case CHAL_NAND_UC_PAGE_OOB_WRITE:
		...
		dma_desc =
		    ((CHAL_NAND_PAGE_SIZE(ni) >> 4) | NAND_CTRL_DMAIL_ENABLE |
		     (bank << 10) | (CHAL_NAND_AUX_DATA_SIZE(ni) >> 3));
		/* set direction to TO device */
		ctrl |= NAND_CTRL_2NAND;
		break;
#endif
  ...
  if ( dma_desc ) {
	...
	/* jump to ucode */
	CHAL_REG_WRITE32(NAND_CTRL_CMD,
			 (CHAL_NAND_UC_LKUP(ni))[cmd] | NAND_CTRL_CMD_VALID);
	...
	if (dma_desc) {
		...
		if ((cmd != CHAL_NAND_UC_PAGE_READ)
		    && (cmd != CHAL_NAND_UC_PARAM_READ)) {
			tmp =
			    (((0x1 << bank) << NAND_IRQ_BANK_COMPLETE_SHIFT) |
			     NAND_IRQ_DMA_COMPLETE);
		} else {
			tmp = NAND_IRQ_DMA_COMPLETE;
		}

		timer = CHAL_NAND_TIMEOUT_DMA;
		while (CHAL_REG_READ32(NAND_CTRL_STATUS) &
		       NAND_CTRL_STATUS_DMA_ACTIVE) {
			if (timer <= 0) {
				break;
			}
			timer -= d;
...
static void nand_cfg_geometry(chal_nand_info_t * ni)
{
	...
	CHAL_NAND_BANK_SHIFT(ni) =
	    23 +
	    ((sizes & CHAL_NAND_ID5_PLANE_SIZE) >>
	     CHAL_NAND_ID5_PLANE_SIZE_SHIFT) +
	    ((sizes & CHAL_NAND_ID5_NUM_PLANE) >>
	     CHAL_NAND_ID5_NUM_PLANE_SHIFT);
	...
	CHAL_NAND_PAGE_SHIFT(ni) =
	    10 +
	    ((sizes & CHAL_NAND_ID4_PAGE_SIZE) >>
	     CHAL_NAND_ID4_PAGE_SIZE_SHIFT);

	/* bl_size = 16 + (sizes >> 4) & 0x3 [min 64K] */
	CHAL_NAND_BLOCK_SHIFT(ni) =
	    16 +
	    ((sizes & CHAL_NAND_ID4_BLOCK_SIZE) >>
	     CHAL_NAND_ID4_BLOCK_SIZE_SHIFT);

	/*                      sectors in a page(512 bytes)     spare size 8 or 16 bytes per sector */
	CHAL_NAND_OOB_SIZE(ni) =
	    (CHAL_NAND_PAGE_SIZE(ni) >> 9) *
	    (8 <<
	     ((sizes & CHAL_NAND_ID4_SPARE_SIZE) >>
	      CHAL_NAND_ID4_SPARE_SIZE_SHIFT));
	...
	CHAL_NAND_BUS_WIDTH(ni) =
	    (8 <<
	     ((sizes & CHAL_NAND_ID4_BUS_WIDTH) >>
	      CHAL_NAND_ID4_BUS_WIDTH_SHIFT));
}
...
static void nand_cfg_onfi(chal_nand_info_t * ni, uint8_t *param_page)
{
	...
	page_size =
	    param_page[80] + (param_page[81] << 8) + (param_page[82] << 16) +
	    (param_page[83] << 24);
	spare_size = param_page[84] + (param_page[85] << 8);
	pages_per_block =
	    param_page[92] + (param_page[93] << 8) + (param_page[94] << 16) +
	    (param_page[95] << 24);
	blocks_per_lun =
	    param_page[96] + (param_page[97] << 8) + (param_page[98] << 16) +
	    (param_page[99] << 24);
	...
	CHAL_NAND_BLOCK_SHIFT(ni) =
	    shift_of(pages_per_block) + CHAL_NAND_PAGE_SHIFT(ni);

	CHAL_NAND_BANK_SHIFT(ni) =
	    shift_of(lun_per_chip_enable) + shift_of(blocks_per_lun) +
	    CHAL_NAND_BLOCK_SHIFT(ni);
