		 ...
		 unsigned long count, void *data)
{
	...
	if (start) {
		if (cfg->in_use) {
			...
			if (!cfg->setup_is_done) {
				printk(KERN_ERR
				       "Need to set up the memory before starting the test thread\n");
			} else {
				cfg->thread =
				    kthread_run((void *)test_thread, NULL,
						MODULE_NAME);
		 ...
		 unsigned long count, void *data)
{
	...
	if (cfg->in_use) {
		printk(KERN_ERR
		       "Test thread is runnig. Please stop it first before setup\n");
		return count;
	}
	...
	if (sscanf(kbuf, "%d %d %d %u", &dma_type, &mem_type, &dir, &bytes) !=
	    4) {
		printk(KERN_ERR
		       "echo <dma_type> <mem_type> <dir> <bytes> > /proc/%s/%s\n",
...
static int proc_init(void)
{
	...
	proc_start =
	    create_proc_entry(PROC_ENTRY_START, 0644, gProc.parent_dir);
	if (proc_start == NULL) {
		rc = -ENOMEM;
		goto err_exit;
	}
	...
	proc_setup =
	    create_proc_entry(PROC_ENTRY_SETUP, 0644, gProc.parent_dir);
	if (proc_setup == NULL) {
		rc = -ENOMEM;
		goto err_del_start;
	}
