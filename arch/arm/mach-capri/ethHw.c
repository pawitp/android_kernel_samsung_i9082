...
int ethHw_Init(void)
{
	...
	ETHHW_REG_MASK_SET(ethHw_regCtrlSwitchMode,
			   ETHHW_REG_CTRL_SW_FWDG_EN |
			   ETHHW_REG_CTRL_SW_FWDG_MODE);
...
int ethHw_miiSet(int port, ETHHW_MII_FLAGS flags, uint32_t addr, uint32_t data)
{
	...
	cmd =
	    ETHHW_REG_MDIO_DONE_MASK | ETHHW_REG_MDIO_EXTRA_MASK |
	    ETHHW_REG_MDIO_WRITE_MASK;
...
int ethHw_miiGet(int port, ETHHW_MII_FLAGS flags, uint32_t addr,
		 uint32_t *datap)
{
	uint32_t mdioAddr;
	uint32_t cmd;
...
int ethHw_impSpeedSet(int mbpsRx, int mbpsTx)
{
	...
	if (mbpsRx == 1000) {
		reg |=
		    ETHHW_REG_IMP_SPEED_RX(ETHHW_REG_IMP_SPEED_MASK_1000MBPS);
	} else if (mbpsRx == 100) {
	...
	if (mbpsTx == 1000) {
		reg |=
		    ETHHW_REG_IMP_SPEED_TX(ETHHW_REG_IMP_SPEED_MASK_1000MBPS);
	} else if (mbpsTx == 100) {
...
int ethHw_impSpeedGet(int *mbpsRxp, int *mbpsTxp)
{
	...
	if (mask == ETHHW_REG_IMP_SPEED_RX(ETHHW_REG_IMP_SPEED_MASK_1000MBPS)) {
		*mbpsRxp = 1000;
		   ...
		   ETHHW_REG_IMP_SPEED_RX(ETHHW_REG_IMP_SPEED_MASK_100MBPS)) {
		*mbpsRxp = 100;
	} else if (mask ==
		   ETHHW_REG_IMP_SPEED_RX(ETHHW_REG_IMP_SPEED_MASK_10MBPS)) {
		*mbpsRxp = 10;
	} else {
		*mbpsRxp = -1;
	}
	...
	if (mask == ETHHW_REG_IMP_SPEED_TX(ETHHW_REG_IMP_SPEED_MASK_1000MBPS)) {
		*mbpsTxp = 1000;
		   ...
		   ETHHW_REG_IMP_SPEED_TX(ETHHW_REG_IMP_SPEED_MASK_100MBPS)) {
		*mbpsTxp = 100;
	} else if (mask ==
		   ETHHW_REG_IMP_SPEED_TX(ETHHW_REG_IMP_SPEED_MASK_10MBPS)) {
		*mbpsTxp = 10;
	} else {
		*mbpsTxp = -1;
	}
		      ...
		      int entryValid, int entryStatic)
{
	...
	ETHHW_REG_SET(ethHw_regArlRwCtrl,
		      (ETHHW_REG_ARL_RWCTRL_START_DONE |
		       ETHHW_REG_ARL_RWCTRL_READ));
	if (!arlAccessDone())
		return ETHHW_RC_TIMEOUT;
	...
	for (i = ETHHW_REG_ARL_ENTRY_MIN; i <= ETHHW_REG_ARL_ENTRY_MAX; i++) {
		...
		if ((ETHHW_REG_GET_VAL(ethHw_regArlFwdEntry(i)) &
		     ETHHW_REG_ARL_FWDENTRY_VALID)) {
	...
	/* Validate entry */
	reg =
	    ETHHW_REG_ARL_FWDENTRY_AGE | (cos <<
					  ETHHW_REG_ARL_FWDENTRY_TC_SHIFT) |
	    port;
...
int ethHw_portLinkStatus(int port)
{
	...
	up = ((ETHHW_REG_GET_VAL(ethHw_regStatusLinkStatus) &
	       ETHHW_PORT_MASK(port)) ? 1 : 0);
...
int ethHw_portSpeed(int port)
{
	...
	speed_bmp =
	    (ETHHW_REG_GET_VAL(ethHw_regStatusPortSpeed) >>
	     ETHHW_REG_STATUS_PORT_SPEED_SHIFT(port)) &
...
static int miiAccessDone(void)
{
	...
	timeout = 0;
	while (ETHHW_REG_GET_VAL(ethHw_regMdioStatus) &
	       ETHHW_REG_MDIO_DONE_MASK)
...
static int arlAccessDone(void)
{
	...
	timeout = 0;
	while (ETHHW_REG_GET_VAL(ethHw_regArlRwCtrl) &
	       ETHHW_REG_ARL_RWCTRL_START_DONE)
