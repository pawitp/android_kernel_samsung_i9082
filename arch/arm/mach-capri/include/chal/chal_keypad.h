...
/* ---- Public Function Prototypes --------------------------------------- */
	...
	static inline void
	    chal_keypad_interrupt_disable_all(CHAL_KEYPAD_HANDLE_t * handle);
	static inline void chal_keypad_interrupt_clear_all(CHAL_KEYPAD_HANDLE_t
							   * handle);
	static inline uint32_t
	    chal_keypad_scan_mode_is_active_low(CHAL_KEYPAD_HANDLE_t * handle);
...
*  @return
****************************************************************************/
	static inline void chal_keypad_interrupt_clear(CHAL_KEYPAD_HANDLE_t *
						       handle,
						       CHAL_KEYPAD_MATRIX_t *
						       status) {
    /** @todo Handle any endianess issues. This is OK right now because everything is little-endian */

		CHAL_REG_WRITE32(handle->regBaseAddr + KEYPAD_KPICR0_OFFSET,
				 status->scan.reg[0]);
		CHAL_REG_WRITE32(handle->regBaseAddr + KEYPAD_KPICR1_OFFSET,
				 status->scan.reg[1]);
	}
...
*
****************************************************************************/
	static inline void chal_keypad_interrupt_enable(CHAL_KEYPAD_HANDLE_t *
							handle,
							CHAL_KEYPAD_MATRIX_t *
							mask) {
   /** @todo Handle any endianess issues. This is OK right now because everything is little-endian */

		CHAL_REG_WRITE32(handle->regBaseAddr + KEYPAD_KPIMR0_OFFSET,
				 mask->scan.reg[0]);
		CHAL_REG_WRITE32(handle->regBaseAddr + KEYPAD_KPIMR1_OFFSET,
				 mask->scan.reg[1]);
	}
...
****************************************************************************/
	static inline uint32_t
	    chal_keypad_interrupt_get_status(CHAL_KEYPAD_HANDLE_t * handle,
					     CHAL_KEYPAD_MATRIX_t * status) {
   /** @todo Handle any endianess issues. This is OK right now because everything is little-endian */

		status->scan.reg[0] =
		    CHAL_REG_READ32(handle->regBaseAddr + KEYPAD_KPISR0_OFFSET);
		status->scan.reg[1] =
		    CHAL_REG_READ32(handle->regBaseAddr + KEYPAD_KPISR1_OFFSET);
...
*
****************************************************************************/
	static inline uint32_t chal_keypad_scan_get_status(CHAL_KEYPAD_HANDLE_t
							   * handle,
							   CHAL_KEYPAD_MATRIX_t
							   * status) {
   /** @todo Handle any endianess issues. This is OK right now because everything is little-endian */
		...
		status->scan.reg[0] =
		    CHAL_REG_READ32(handle->regBaseAddr + KEYPAD_KPSSR0_OFFSET);
		status->scan.reg[1] =
		    CHAL_REG_READ32(handle->regBaseAddr + KEYPAD_KPSSR1_OFFSET);
...
****************************************************************************/
	static inline uint32_t chal_keypad_scan_get_cols(CHAL_KEYPAD_HANDLE_t *
							 handle) {
		return (1 +
			((CHAL_REG_READ32
			  (handle->regBaseAddr +
			   KEYPAD_KPCR_OFFSET) & KEYPAD_KPCR_COLUMNWIDTH_MASK)
			 >> KEYPAD_KPCR_COLUMNWIDTH_SHIFT));
	}
	static inline uint32_t chal_keypad_scan_get_rows(CHAL_KEYPAD_HANDLE_t *
							 handle) {
		return (1 +
			((CHAL_REG_READ32
			  (handle->regBaseAddr +
			   KEYPAD_KPCR_OFFSET) & KEYPAD_KPCR_ROWWIDTH_MASK) >>
			 KEYPAD_KPCR_ROWWIDTH_SHIFT));
	}
...
*           and vice-versa.
****************************************************************************/
	static inline void chal_keypad_scan_swap_row_column(CHAL_KEYPAD_HANDLE_t
							    * handle,
							    int swap) {
		if (swap) {
			CHAL_REG_SETBIT32(handle->regBaseAddr +
					  KEYPAD_KPCR_OFFSET,
					  KEYPAD_KPCR_SWAPROWCOLUMN_MASK);
		} else {
			CHAL_REG_CLRBIT32(handle->regBaseAddr +
					  KEYPAD_KPCR_OFFSET,
					  KEYPAD_KPCR_SWAPROWCOLUMN_MASK);
		}
	}
...
*  @return  none
****************************************************************************/
	static inline void chal_keypad_matrix_clear(CHAL_KEYPAD_MATRIX_t *
						    matrix, unsigned row,
						    unsigned col) {
...
*  @return  none
****************************************************************************/
	static inline void chal_keypad_matrix_clear_all(CHAL_KEYPAD_MATRIX_t *
							matrix) {
		matrix->scan.reg[0] = 0;
		matrix->scan.reg[1] = 0;
	}
...
*  @return  none
****************************************************************************/
	static inline unsigned chal_keypad_matrix_is_set(CHAL_KEYPAD_MATRIX_t *
							 matrix, unsigned row,
							 unsigned col) {
...
*  @return  none
****************************************************************************/
	static inline void chal_keypad_matrix_set(CHAL_KEYPAD_MATRIX_t * matrix,
...
*  @return  none
****************************************************************************/
	static inline void chal_keypad_matrix_set_all(CHAL_KEYPAD_MATRIX_t *
						      matrix, unsigned rowCnt,
						      unsigned colCnt) {
		uint32_t colMask;
		unsigned rowIdx;
