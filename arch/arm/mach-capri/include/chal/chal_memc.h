...
/* function declarations */
	...
	MEMC_EXPORT BCM_ERR_CODE chal_memc_init(CHAL_MEMC_HANDLE handle);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_soft_reset(CHAL_MEMC_HANDLE handle);

	MEMC_EXPORT BCM_ERR_CODE chal_lpddr2_get_dev_info(CHAL_MEMC_HANDLE
							  handle,
							  MEMC_CS_CONNECTION cs,
							  CHAL_LPDDR2_DEV_INFO_T
							  * pDevInfo);

	MEMC_EXPORT BCM_ERR_CODE chal_lpddr2_get_mem_size(CHAL_MEMC_HANDLE
							  handle,
							  MEMC_CS_CONNECTION cs,
							  uint32_t *memSize);

	MEMC_EXPORT BCM_ERR_CODE chal_lpddr2_get_base_addr(CHAL_MEMC_HANDLE
							   handle,
							   MEMC_REG_BLOCK base);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_set_clock(CHAL_MEMC_HANDLE handle,
						     MEMC_CLOCK_T clock);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_get_clock(CHAL_MEMC_HANDLE handle,
						     MEMC_CLOCK_T * clock);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_get_clock_speed(CHAL_MEMC_HANDLE
							   handle,
							   uint32_t
							   *clock_speed_hz);

	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_set_auto_power_down_cycles(CHAL_MEMC_HANDLE handle,
						 uint32_t cycles);

	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_get_auto_power_down_cycles(CHAL_MEMC_HANDLE handle,
						 uint32_t *cycles);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_enable_auto_pwr(CHAL_MEMC_HANDLE
							   handle,
							   _Bool enable);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_enter_sw_power_down(CHAL_MEMC_HANDLE
							       handle,
							       CHAL_MEMC_POWER_DOWN_MODE
							       mode);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_exit_selfrefresh(CHAL_MEMC_HANDLE
							    handle);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_sw_power_fully_on(CHAL_MEMC_HANDLE
							     handle);

	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_get_sw_power_down_status(CHAL_MEMC_HANDLE handle,
					       uint32_t *status);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_config_burst_length(CHAL_MEMC_HANDLE
							       handle,
							       uint8_t blen);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_config_sequence(CHAL_MEMC_HANDLE
							   handle, uint8_t seq);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_config_wrap_nowrap(CHAL_MEMC_HANDLE
							      handle,
							      uint8_t wrap);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_config_stats(CHAL_MEMC_HANDLE handle,
							uint32_t cycles);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_get_stats(CHAL_MEMC_HANDLE handle,
						     CHAL_MEMC_STATS_T *
						     pStats);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_meu_config_region(CHAL_MEMC_HANDLE
							     handle,
							     uint8_t rgn_num,
							     uint32_t rgn_type,
							     uint32_t
	...
	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_meu_enable_disable_region(CHAL_MEMC_HANDLE handle,
						uint8_t rgn_num, _Bool enable);

	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_meu_enable_region_data_scramble(CHAL_MEMC_HANDLE handle,
						      uint8_t rgn_num);

	MEMC_EXPORT void
	    chal_memc_meu_enable_global_data_scrambling(CHAL_MEMC_HANDLE
							handle);

	MEMC_EXPORT void
	    chal_memc_meu_enable_global_addr_scrambling(CHAL_MEMC_HANDLE
							handle);

	MEMC_EXPORT void chal_memc_meu_lock_global_gid_check(CHAL_MEMC_HANDLE
							     handle);

	MEMC_EXPORT void chal_memc_meu_enable_global_gid_check(CHAL_MEMC_HANDLE
							       handle);

	MEMC_EXPORT void chal_memc_meu_disable_global_gid_check(CHAL_MEMC_HANDLE
								handle);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_meu_config_keys(CHAL_MEMC_HANDLE
							   handle,
							   CHAL_MEU_KEY_T key,
							   uint32_t key_value);

	MEMC_EXPORT void
	    chal_memc_meu_program_scrambling_scheme(CHAL_MEMC_HANDLE handle,
						    uint32_t scheme_value);

	MEMC_EXPORT void chal_memc_meu_program_scrambling_mask(CHAL_MEMC_HANDLE
							       handle,
							       uint32_t
							       mask_value);

	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_meu_get_access_vio_info(CHAL_MEMC_HANDLE handle,
					      CHAL_MEU_ACCESS_VIO_INFO_T *
					      info);

	MEMC_EXPORT _Bool
	    chal_memc_meu_is_gid_check_bit_enabled(CHAL_MEMC_HANDLE handle);

	MEMC_EXPORT _Bool
	    chal_memc_meu_is_gid_check_lock_bit_enabled(CHAL_MEMC_HANDLE
							handle);

	MEMC_EXPORT uint32_t chal_memc_read_register(CHAL_MEMC_HANDLE handle,
						     MEMC_REG_BLOCK reg_base,
						     uint32_t reg_addr);

	MEMC_EXPORT _Bool chal_memc_write_register(CHAL_MEMC_HANDLE handle,
						   MEMC_REG_BLOCK reg_base,
						   uint32_t reg_offset,
						   uint32_t reg_value);

	MEMC_EXPORT _Bool chal_memc_write_register_2(CHAL_MEMC_HANDLE handle,
						     uint32_t reg_addr,
						     uint32_t reg_value);

	MEMC_EXPORT uint32_t chal_memc_read_mode_register(CHAL_MEMC_HANDLE
							  handle,
							  MEMC_CS_CONNECTION cs,
							  uint32_t reg_addr);

	MEMC_EXPORT _Bool chal_memc_write_mode_register(CHAL_MEMC_HANDLE handle,
							MEMC_CS_CONNECTION cs,
							uint32_t reg_addr,
							uint32_t value);

	MEMC_EXPORT uint32_t chal_memc_get_ddr_pll_status(CHAL_MEMC_HANDLE
							  handle);

	MEMC_EXPORT _Bool chal_memc_get_master_dll_output(CHAL_MEMC_HANDLE
							  handle,
							  CHAL_MEMC_MASTER_DLL_STATUS_T
							  * status);

	MEMC_EXPORT _Bool chal_memc_enable_slave_dll_lock(CHAL_MEMC_HANDLE
							  handle,
							  uint32_t lock);

	MEMC_EXPORT _Bool chal_memc_set_zq_calibration(CHAL_MEMC_HANDLE handle,
						       uint32_t enable);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_reset_ddr2(CHAL_MEMC_HANDLE handle);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_pvt_calibration(CHAL_MEMC_HANDLE
							   handle);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_temperature_polling(CHAL_MEMC_HANDLE
							       handle,
							       _Bool enable);
 ...
 ***************************************************************/
	...
	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_nvm_disable_overlay_window(CHAL_MEMC_HANDLE handle);

	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_nvm_enable_overlay_window(CHAL_MEMC_HANDLE handle,
						uint32_t base_addr);

	MEMC_EXPORT _Bool
	    chal_memc_nvm_is_overlay_window_enable(CHAL_MEMC_HANDLE handle,
						   _Bool read_from_device);

	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_nvm_read_overlay_window_status(CHAL_MEMC_HANDLE handle,
						     uint32_t *status);

	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_nvm_clear_overlay_window_status(CHAL_MEMC_HANDLE handle,
						      uint32_t status_bit);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_is_busy(CHAL_MEMC_HANDLE handle,
						       _Bool * busy);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_block_unlock_all(CHAL_MEMC_HANDLE
								handle);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_block_lock(CHAL_MEMC_HANDLE
							  handle,
							  uint32_t start_addr,
							  uint32_t end_addr,
							  NVM_BLOCK_LOCK_MODE
	...
	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_block_lock_down(CHAL_MEMC_HANDLE
							       handle,
							       uint32_t
							       start_addr,
							       uint32_t
	...
	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_block_erase(CHAL_MEMC_HANDLE
							   handle,
							   uint32_t block_addr,
							   uint32_t
							   waitComplete);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_write_word(CHAL_MEMC_HANDLE
							  handle,
							  uint32_t word_addr,
							  uint16_t word_value,
							  uint32_t overwrite,
	...
	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_write_buffer(CHAL_MEMC_HANDLE
							    handle,
							    uint32_t address,
							    uint8_t *data,
							    uint32_t size,
	...
	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_suspend(CHAL_MEMC_HANDLE handle);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_resume(CHAL_MEMC_HANDLE handle);

	MEMC_EXPORT BCM_ERR_CODE
	    chal_memc_nvm_read_block_lock_status(CHAL_MEMC_HANDLE handle,
						 uint32_t address,
						 NVM_BLOCK_LOCK_MODE * lock);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_nvm_abort(CHAL_MEMC_HANDLE handle);
