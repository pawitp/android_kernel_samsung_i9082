...
#define CHAL_MEMC_DDR3_JEDEC_TYPE_IS_DDR3(t)    ((t) >= CHAL_MEMC_DDR3_JEDEC_TYPE_DDR3_MIN)
	...
	typedef enum {
		...
		CHAL_MEMC_DDR3_POWER_DOWN_MODE_PRECHARGE =
...
/* function declarations */
	...
	MEMC_EXPORT BCM_ERR_CODE chal_memc_ddr3_init(void *generic_hdl);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_ddr3_get_clock_speed(void
								*generic_hdl,
								uint32_t
								*clock_speed_hz);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_ddr3_get_dev_info(void *generic_hdl);

	MEMC_EXPORT uint32_t chal_memc_ddr3_read_mode_register(void
							       *generic_hdl,
							       uint32_t cs,
							       uint32_t
							       reg_addr);

	MEMC_EXPORT BCM_ERR_CODE ddr3_auto_power_down_disable(void
							      *generic_hdl);

	MEMC_EXPORT BCM_ERR_CODE ddr3_auto_power_down_enable(void *generic_hdl,
							     CHAL_MEMC_DDR3_POWER_DOWN_MODE
							     mode);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_ddr3_self_refresh_enter(void
								   *generic_hdl);

	MEMC_EXPORT BCM_ERR_CODE chal_memc_ddr3_self_refresh_exit(void
								  *generic_hdl);

	MEMC_EXPORT _Bool chal_memc_ddr3_write_mode_register(void *generic_hdl,
							     uint32_t cs,
							     uint32_t reg_addr,
							     uint32_t value);
