...
static inline void chal_nand_prd_addr_set(chal_nand_prd_entry_t * entry,
					  void *phys)
{
...
static inline void chal_nand_prd_desc_set(chal_nand_prd_entry_t * entry,
					  uint8_t bank,
					  uint32_t size, uint8_t eot)
{
	entry->desc = (eot << 31) |
	    ((bank & 0x7) << 28) | (size & ((0x1 << 22) - 1));
}
...
void *chal_nand_dma_setup_callback(uint8_t direction,
				   uint8_t bank, void *virt, uint32_t size);
