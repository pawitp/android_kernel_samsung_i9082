...
#include <mach/rdb/brcm_rdb_rtc.h>
#include <mach/rdb/brcm_rdb_rtc_apb.h>
#include <mach/rdb/brcm_rdb_sysmap.h>
#include <mach/rdb-fixups.h>
#define RTC_PERIODIC_TIMER_ADDR     (rtc->handle->rtcBaseAddr+RTC_PER_OFFSET)
#define RTC_MATCH_REGISTER_ADDR     (rtc->handle->rtcBaseAddr+RTC_MATCH_OFFSET)
#define RTC_CLEAR_INTR_ADDR         (rtc->handle->rtcBaseAddr+RTC_CLR_INT_OFFSET)
#define RTC_INTERRUPT_STATUS_ADDR   (rtc->handle->rtcBaseAddr+RTC_INT_STS_OFFSET)
#define RTC_CONTROL_ADDR            (rtc->handle->rtcBaseAddr+RTC_CTRL_OFFSET)
/* --- END:  CHAL compatability shim --- *//* ---- Public Constants and Types --------------------------------------- *//*
...
*
****************************************************************************/
	int chal_rtc_init(CHAL_RTC_HANDLE_t * handle, uint32_t bblBar,
			  uint32_t rtcBar);

	static inline void chal_rtc_divSet(CHAL_RTC_HANDLE_t * handle,
					   uint32_t val);
	static inline uint32_t chal_rtc_divGet(CHAL_RTC_HANDLE_t * handle);

	static inline void chal_rtc_secSet(CHAL_RTC_HANDLE_t * handle,
					   uint32_t val);
	static inline uint32_t chal_rtc_secGet(CHAL_RTC_HANDLE_t * handle);

	static inline void chal_rtc_ctrlSet(CHAL_RTC_HANDLE_t * handle,
					    chal_RTC_CTRL_e cmd);

	static inline void chal_rtc_periodInterruptValSet(CHAL_RTC_HANDLE_t *
							  handle,
							  chal_RTC_PER_INTERVAL_e
							  interval);
	static inline uint32_t chal_rtc_periodInterruptValGet(CHAL_RTC_HANDLE_t
							      * handle);

	static inline void chal_rtc_matchInterruptValSet(CHAL_RTC_HANDLE_t *
							 handle,
							 chal_rtc_TIME_t sec);
	static inline uint32_t chal_rtc_matchInterruptValGet(CHAL_RTC_HANDLE_t *
							     handle);
	...
	static inline void chal_rtc_intEnable(CHAL_RTC_HANDLE_t * handle,
					      chal_RTC_INT_e interrupt);
	static inline void chal_rtc_intEnableVal(CHAL_RTC_HANDLE_t * handle,
						 uint32_t bitsOn);
	static inline void chal_rtc_intDisable(CHAL_RTC_HANDLE_t * handle,
					       chal_RTC_INT_e interrupt);
	static inline void chal_rtc_intDisableVal(CHAL_RTC_HANDLE_t * handle,
						  uint32_t bitsOff);

	static inline int chal_rtc_intIsEnabled(CHAL_RTC_HANDLE_t * handle,
						chal_RTC_INT_e interrupt);
	static inline chal_RTC_INT_e
	    chal_rtc_interruptStatusGet(CHAL_RTC_HANDLE_t * handle);
	...
	static inline void chal_rtc_mtcIncrSet(CHAL_RTC_HANDLE_t * handle,
					       uint16_t amount);

	static inline void chal_rtc_mtcTimeSet(CHAL_RTC_HANDLE_t * handle,
					       chal_rtc_TIME_t time);
	static inline chal_rtc_TIME_t chal_rtc_mtcTimeGet(CHAL_RTC_HANDLE_t *
							  handle);

/* ---- Private Constants and Types ---------------------------------------- */
/* ---- Public Variable Externs -------------------------------------------- */
/* ---- Private Function Prototypes ----------------------------------------- */
	static inline uint32_t chal_rtc_readReg(CHAL_RTC_HANDLE_t * handle,
						uint32_t regAddr);
	static inline void chal_rtc_readRegIE(CHAL_RTC_HANDLE_t * handle,
					      uint32_t regAddr);
...
#define  chal_rtc_REG_COMMAND_WRITE(handle)       (CHAL_REG_SETBIT32(handle->ctrl_sts, RTC_APB_CTRL_STS_RTC_CMD_START_MASK | RTC_APB_CTRL_STS_RTC_APB_CMD_MASK))
#define  chal_rtc_REG_COMMAND_READ(handle)        (CHAL_REG_SETBIT32(handle->ctrl_sts, RTC_APB_CTRL_STS_RTC_CMD_START_MASK & ~RTC_APB_CTRL_STS_RTC_APB_CMD_MASK))
#define  chal_rtc_REG_IS_COMMAND_DONE(handle)     (CHAL_REG_READ32(handle->ctrl_sts) & RTC_APB_CTRL_STS_RTC_CMD_DONE_MASK)
#define  chal_rtc_REG_COMMAND_DONE_CLEAR(handle)  (CHAL_REG_SETBIT32(handle->ctrl_sts, RTC_APB_CTRL_STS_RTC_CMD_DONE_MASK))
...
/****************************************************************************/
	static inline void chal_rtc_divSet(CHAL_RTC_HANDLE_t * handle,
		...
		chal_rtc_writeReg(handle,
				  handle->rtcBaseAddr + RTC_SET_DIV_OFFSET,
				  val);
	}
...
/****************************************************************************/
	static inline uint32_t chal_rtc_divGet(CHAL_RTC_HANDLE_t * handle) {
		return (chal_rtc_readReg
			(handle, handle->rtcBaseAddr + RTC_SET_DIV_OFFSET));
	}
...
/****************************************************************************/
	static inline void chal_rtc_secSet(CHAL_RTC_HANDLE_t * handle,
		...
		chal_rtc_writeReg(handle,
				  handle->rtcBaseAddr + RTC_SEC_0_OFFSET, val);
	}
...
/****************************************************************************/
	static inline uint32_t chal_rtc_secGet(CHAL_RTC_HANDLE_t * handle) {
		uint32_t val =
		    chal_rtc_readReg(handle,
				     handle->rtcBaseAddr + RTC_SEC_0_OFFSET);
		return val;
	}
...
/****************************************************************************/
	static inline void chal_rtc_periodInterruptValSet(CHAL_RTC_HANDLE_t *
		...
		chal_dbg_printf(CHAL_DBG_DEBUG, "Write 0x%x to PER\n",
				interval);
		chal_rtc_writeReg(handle, handle->rtcBaseAddr + RTC_PER_OFFSET,
				  interval);
	}
...
/****************************************************************************/
	static inline uint32_t chal_rtc_periodInterruptValGet(CHAL_RTC_HANDLE_t
							      * handle) {
		return (chal_rtc_readReg
			(handle, handle->rtcBaseAddr + RTC_PER_OFFSET));
	}
...
/****************************************************************************/
	static inline void chal_rtc_matchInterruptValSet(CHAL_RTC_HANDLE_t *
							 ...
							 chal_rtc_TIME_t sec) {
		chal_rtc_writeReg(handle,
				  handle->rtcBaseAddr + RTC_MATCH_OFFSET,
...
/****************************************************************************/
	static inline uint32_t chal_rtc_matchInterruptValGet(CHAL_RTC_HANDLE_t *
							     handle) {
		return (chal_rtc_readReg
			(handle, handle->rtcBaseAddr + RTC_MATCH_OFFSET));
	}
...
/****************************************************************************/
	static inline void chal_rtc_intStatusClr(CHAL_RTC_HANDLE_t * handle,
						 chal_RTC_INT_e interrupt) {
		chal_rtc_writeReg(handle,
				  handle->rtcBaseAddr + RTC_CLR_INT_OFFSET,
				  1 << interrupt);
	}
...
/****************************************************************************/
	static inline int chal_rtc_intStatusGet(CHAL_RTC_HANDLE_t * handle,
		...
		return ((chal_rtc_readReg
			 (handle,
			  handle->rtcBaseAddr +
			  RTC_INT_STS_OFFSET) & (1 << interrupt)) ? 1 : 0);
	}
	...
	static inline uint32_t chal_rtc_intStatusGetVal(CHAL_RTC_HANDLE_t *
		...
		uint32_t val =
		    chal_rtc_readReg(handle,
				     handle->rtcBaseAddr + RTC_INT_STS_OFFSET);
		return val;
	}
...
//   val = chal_rtc_readReg(handle, addr);
	...
	static inline void chal_rtc_intDisable(CHAL_RTC_HANDLE_t * handle,
		...
		val = chal_rtc_readReg(handle, addr);
		val &= ~(1 << interrupt);
		chal_dbg_printf(CHAL_DBG_DEBUG,
				"Write %d to INT_ENABLE (disabling)\n", val);
		chal_rtc_writeReg(handle, addr, val);
	}
...
/****************************************************************************/
	static inline int chal_rtc_intIsEnabled(CHAL_RTC_HANDLE_t * handle,
						chal_RTC_INT_e interrupt) {
		return ((chal_rtc_readReg
			 (handle, handle->rtcBaseAddr + RTC_INT_ENABLE_OFFSET)
			 & (1 << interrupt)) ? 1 : 0);
	}
...
/****************************************************************************/
	static inline int chal_rtc_rstIsAsserted(CHAL_RTC_HANDLE_t * handle) {
		return ((chal_rtc_readReg
			 (handle,
			  handle->rtcBaseAddr + RTC_RESET_ACCESS_OFFSET) ==
...
/****************************************************************************/
	static inline void chal_rtc_mtcIncrSet(CHAL_RTC_HANDLE_t * handle,
		...
		val |=
		    (uint32_t)amount << RTC_MTC_CTRL_RTC_MT_CTR_INCR_AMT_SHIFT;
...
/****************************************************************************/
	static inline void chal_rtc_mtcTimeSet(CHAL_RTC_HANDLE_t * handle,
					       chal_rtc_TIME_t time) {
		chal_rtc_writeReg(handle,
				  handle->rtcBaseAddr + RTC_MTC_MSB_OFFSET,
				  time >> 32);
		chal_rtc_writeReg(handle,
				  handle->rtcBaseAddr + RTC_MTC_LSB_OFFSET,
				  time & 0xffffffff);
	}
...
/****************************************************************************/
	static inline chal_rtc_TIME_t chal_rtc_mtcTimeGet(CHAL_RTC_HANDLE_t *
		...
		time =
		    ((chal_rtc_TIME_t) mtcMsb) << 32 | (chal_rtc_TIME_t) mtcLsb;
...
/****************************************************************************/
	static inline uint32_t chal_rtc_readReg(CHAL_RTC_HANDLE_t * handle,
		...
		ctrlStsVal &= ~RTC_APB_CTRL_STS_RTC_APB_CMD_MASK;
		ctrlStsVal |= RTC_APB_CTRL_STS_RTC_CMD_START_MASK;
 ...
 */
	static inline void chal_rtc_readRegIE(CHAL_RTC_HANDLE_t * handle,
		...
		ctrlStsVal |= RTC_APB_CTRL_STS_APB_CMD_DONE_INT_EN_MASK;
		chal_dbg_printf(CHAL_DBG_DEBUG, "Write %d to ctrl_sts\n",
				ctrlStsVal);
		CHAL_REG_WRITE32(handle->ctrl_sts, ctrlStsVal);
	}
