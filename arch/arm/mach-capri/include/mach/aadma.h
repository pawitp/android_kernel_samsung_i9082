...
#define AADMA_DEVICE_FLAG_IN_USE       0x00000800	/* If set, device is
							 * in use on a channel
							 */
...
typedef enum {
	...
	AADMA_DEVICE_NONE = 0xff,	/* Special value to indicate that no
...
typedef void (*AADMA_DeviceHandler_t) (AADMA_Device_t dev,
				       AADMA_Status_t * dma_status,
				       void *userData);

/* ---- Constants and Types ---------------------------------------------- */

/*
 * If AADMA_DEBUG_TRACK_RESERVATION is set to a non-zero value, then the
 * filename and line number of the reservation request will be recorded in
 *the channel table
 */
 ...
 *       bitmask is used to determine which DMA controllers a given device
 *       can be used from, and the interface array determeines the actual
 *       interface number to use for a given controller.
 */
...
typedef struct {
	...
	AADMA_DeviceHandler_t devHandler;	/* Called when DMA operations
						 * finish.
						 */
	timer_tick_count_t transferStartTime;	/* Time the current transfer
						 * was started
						 */
	...
	/*
	 * The following statistical information will be collected and
	 * pesented in a proc entry.
	 * Note: With a contiuous bandwidth of 1 Gb/sec, it would take 584
...
typedef struct {
	...
	AADMA_Device_t devType;	/* Device this channel is currently reserved
				 * for
				 */
...
#define aadma_request_channel(dev)  \
	aadma_request_channel_dbg(dev, __FILE__, __LINE__)
#else
...
int aadma_free_channel(AADMA_Handle_t handle	/* AADMA handle. */
    );
...
int aadma_set_device_thres
    (AADMA_Device_t dev, uint16_t thres1, uint16_t thres2);
...
int aadma_alloc_descriptors(AADMA_Handle_t handle,	/* AADMA Handle */
			    dma_addr_t data_addr,	/* Place to read/write
							 * data to/from device
							 */
...
int aadma_alloc_ring_descriptors(AADMA_Handle_t handle,	/* AADMA Handle */
				 dma_addr_t data_addr_lo,
				 /* Place to read/write low buffer
				  * data to/from device
				  */
				 dma_addr_t data_addr_hi,
				 /* Place to read/write high buffer
				  * data to/from device
				  */
				 size_t numBytes
...
int aadma_transfer(AADMA_Handle_t handle,	/* AADMA Handle */
		   dma_addr_t data_addr,	/* Place to read/write data
						 * to/from device
						 */
		   size_t numBytes	/* Number of bytes to transfer to
					 * the device
					 */
    );
