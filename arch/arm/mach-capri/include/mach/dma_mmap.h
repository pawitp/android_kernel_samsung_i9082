...
typedef enum {
	...
	DMA_MMAP_TYPE_PHYS,	/* Physical address supplied */
} DMA_MMAP_TYPE_T;
...
typedef struct {
	...
	/*
	 * Where to start within the pagelist array, defaults to zero.
	 */
	int pageListStart;
 ...
 */
extern int dma_mmap_init_map(DMA_MMAP_CFG_T * memMap	/* Stores state information about the map */
    );

/*
 * Releases any memory currently being held by a memory mapping structure
 */
extern int dma_mmap_term_map(DMA_MMAP_CFG_T * memMap	/* Stores state information about the map */
    );
...
extern void dma_mmap_dump_map(const char *function,	/* Function doing the dumping       */
 ...
 */
extern DMA_MMAP_TYPE_T dma_mmap_mem_type(void *addr);
 ...
 */
extern int dma_mmap_dma_is_supported(void *addr);
 ...
 */
extern int dma_mmap_start(DMA_MMAP_CFG_T * memMap,	/* Stores state information about the map */
 ...
 */
extern int dma_mmap_add_region(DMA_MMAP_CFG_T * memMap,	/* Stores state information about the map */
			       void *mem,	/* Virtual address that we want to get a map of */
			       size_t numBytes	/* Number of bytes being mapped */
    );

/*
 * Adds a region of memory supplying a physical address.
 *
 ...
 * @return     0 on success, error code otherwise
 */
extern int dma_mmap_add_phys_region(DMA_MMAP_CFG_T * memMap,
 ...
 * @return
 */
extern int dma_mmap_map(DMA_MMAP_CFG_T * memMap,	/* Stores state information about the map */
 ...
 * @return
 */
extern int dma_mmap_unmap(DMA_MMAP_CFG_T * memMap,	/* Stores state information about the map */
...
*/
extern void dma_mmap_memcpy(DMA_MMAP_CFG_T * memMap,	/* memory map */
 ...
 * Lock the memory map
 */
static inline void dma_mmap_lock_map(DMA_MMAP_CFG_T * memMap) {
	down(&memMap->lock);
}
 ...
 */
static inline void dma_mmap_unlock_map(DMA_MMAP_CFG_T * memMap) {
	up(&memMap->lock);
}
...
extern int dma_mmap_create_pagelist
    (char __user *addr,
     ...
     struct task_struct *userTask, DMA_MMAP_PAGELIST_T ** pagelist_out);

/*
 * Free a pagelist previously allocated using dma_mmap_create_pagelist.
 * This will unlock any locked pages.
 */
extern void dma_mmap_free_pagelist(DMA_MMAP_PAGELIST_T * pagelist);

/*
 * Specifies a pagelist up front, to get it into dma_mmap_add_region.
 */
extern void dma_mmap_set_pagelist
    (DMA_MMAP_CFG_T * memMap, DMA_MMAP_PAGELIST_T * pagelist);

extern int dma_mmap_set_pagelist_start(DMA_MMAP_CFG_T * memMap, unsigned start);

extern int dma_mmap_preallocate(DMA_MMAP_CFG_T *memMap, int numRegions,
