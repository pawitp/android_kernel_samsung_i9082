...
typedef enum {
	...
	SSASW_DEVICE_SSASW_SSP6_PCM_EGR,
	SSASW_DEVICE_SSASW_SSP6_PCM_IGR,

	/* Add new entries before this line. */

	SSASW_NUM_DEVICE_ENTRIES,
	SSASW_DEVICE_NONE = 0xff,	/* Special value to indicate that no
...
typedef struct {
	...
	CAPH_SWITCH_TRIGGER_e dev_sw_trig;
	/* Main device switch trigger condition setting */
	CAPH_DATA_FORMAT_e sw_data_format;	/* Switch data format */

	CAPH_SWITCH_TRIGGER_e src_sw_trig;
	/* SRC switch trigger condition setting */
	...
	CAPH_SRCMixer_OUTPUT_e mix_dst_ch;
	/* SRC mixer destination channel */
	CAPH_SRCMixer_SRC_e mix_freq;	/* SRC up/down convert frequency mode */
	CAPH_DATA_FORMAT_e src_in_data_format;
	/* SRC in FIFO mixer data format */
	CAPH_DATA_FORMAT_e src_out_data_format;
	/* SRC out FIFO mixer data format */

} SSASW_DeviceAttribute_t;

typedef struct {
	uint32_t inUse;		/* Number of channels in use */
	SSASW_Device_t devType;
...
SSASW_Handle_t ssasw_request_channel
    (SSASW_Device_t dev, uint16_t src_addr, uint16_t dst_addr);
