...
static void sec_reboot(char str, const char *cmd)
{
	...
	__raw_writel(0x12345678,BROADCOM_INFORM2);	/* Don't enter lpm mode */

	if (!cmd) {
		__raw_writel(REBOOT_MODE_PREFIX | REBOOT_MODE_NONE, BROADCOM_INFORM3);
	} else {
		unsigned long value;
		if (!strcmp(cmd, "fota"))
			__raw_writel(REBOOT_MODE_PREFIX | REBOOT_MODE_ARM11_FOTA,
			       BROADCOM_INFORM3);
