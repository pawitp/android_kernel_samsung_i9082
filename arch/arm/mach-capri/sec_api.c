...
inline uint32_t mobicore_smc(uint32_t cmd, uint32_t arg1, uint32_t arg2, uint32_t arg3)
{
...
static void secure_api_call_shim(void *info)
{
	...
	if (data->service_id != SSAPI_BRCM_START_VC_CORE && 
		data->service_id != SSAPI_ENABLE_L2_CACHE)
	flush_cache_all();
...
static int  redo_auto = 0;

static void set_hub_autogate(bool enable)
{
	uint32_t val;
			 ...
			 unsigned arg2, unsigned arg3)
{
	...
	if (service_id == SSAPI_ROW_AES) {
		...
		rc = clk_set_rate(spum_sec_clk, FREQ_MHZ(208));
		if (rc)
