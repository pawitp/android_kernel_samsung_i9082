...
#define SRCMIXER_REGISTER_LENGTH          0xD00	/* Arbitrarily longer than
						 * required
						 */

#define MAKE_HANDLE(channel_src_idx, channel_idx)   \
			(((channel_src_idx) << 4) | (channel_idx))
...
{ \
	...
	.mix_src_ch = (CAPH_SRCM_CH5_L | CAPH_SRCM_CH5_R), \
...
#define DEVICE_SSASW_SRC_44100_AUDIOH_HANDSFREE_STEREO \
{ \
	.name = "SSASW 44100 Handsfree Stereo",            \
	.dev_sw_trig = CAPH_IHF_THR_MET,                   \
	.sw_data_format = CAPH_MONO_16BIT,                 \
	.src_sw_trig = CAPH_TAPSUP_CH5_NORM_INT,           \
	.src_in_fifo = CAPH_CH5_INFIFO,                    \
...
#define DEVICE_SSASW_SSP6_PCM_EGR \
{ \
	.name = "SSASW SSP6 PCM Egress",    \
	.dev_sw_trig = CAPH_SSP6_TRIGGER,   \
	.sw_data_format = CAPH_MONO_16BIT,  \
}

#define DEVICE_SSASW_SSP6_PCM_IGR \
{ \
	.name = "SSASW SSP6 PCM Ingress",   \
	.dev_sw_trig = CAPH_SSP6_TRIGGER,   \
	.sw_data_format = CAPH_MONO_16BIT,  \
}
...
SSASW_DeviceAttribute_t SSASW_gDeviceAttribute[SSASW_NUM_DEVICE_ENTRIES] = {
	...
	[SSASW_SRC_44100_AUDIOH_EARPIECE] =
	    DEVICE_SSASW_SRC_44100_AUDIOH_EARPIECE,
	[SSASW_SRC_44100_AUDIOH_HEADSET] =
	    DEVICE_SSASW_SRC_44100_AUDIOH_HEADSET,
	[SSASW_SRC_44100_AUDIOH_HANDSFREE] =
	    DEVICE_SSASW_SRC_44100_AUDIOH_HANDSFREE,
	[SSASW_SRC_44100_AUDIOH_HANDSFREE_STEREO] =
	    DEVICE_SSASW_SRC_44100_AUDIOH_HANDSFREE_STEREO,
	[SSASW_SRC_44100_AUDIOH_HANDSFREE_STEREO] =
	    DEVICE_SSASW_SRC_44100_AUDIOH_HANDSFREE_STEREO,

	[SSASW_DEVICE_SSASW_SSP6_PCM_EGR] = DEVICE_SSASW_SSP6_PCM_EGR,
	[SSASW_DEVICE_SSASW_SSP6_PCM_IGR] = DEVICE_SSASW_SSP6_PCM_IGR,
...
*/
static inline short dbToLinearQ14(int *db
				    /**< (io) Ptr to gain in db */
    )
{
	int dbval = *db;
...
static int ssasw_ioremap_init(SSASW_ChalHandle_t * chal_handle)
{
	...
	ssasw_ioarea =
	    request_mem_region(SSASW_PHYS_BASE_ADDR_START,
			       SSASW_REGISTER_LENGTH,
			       "SSASW Component Mem Region");
	srcmixer_ioarea =
	    request_mem_region(SRCMIXER_PHYS_BASE_ADDR_START,
			       SRCMIXER_REGISTER_LENGTH,
			       "SRC Mixer Component Mem Region");

	if (!ssasw_ioarea || !srcmixer_ioarea)
		return -EBUSY;

	ssasw_virt_addr =
	    ioremap(SSASW_PHYS_BASE_ADDR_START, SSASW_REGISTER_LENGTH);
	srcmixer_virt_addr =
	    ioremap(SRCMIXER_PHYS_BASE_ADDR_START, SRCMIXER_REGISTER_LENGTH);

	if (!ssasw_virt_addr || !srcmixer_virt_addr)
		return -ENOMEM;

	chal_handle->chalSsaswHandle =
	    chal_caph_switch_init((uint32_t)ssasw_virt_addr);
	chal_handle->chalSrcMixerHandle =
	    chal_caph_srcmixer_init((uint32_t)srcmixer_virt_addr);

	chal_caph_srcmixer_clr_all_tapbuffers(chal_handle->chalSrcMixerHandle);

	chal_caph_srcmixer_set_spkrgain_bitsel(chal_handle->chalSrcMixerHandle,
					       CAPH_M0_Left,
					       SRCMIXER_CH_DEF_BITSEL);
	chal_caph_srcmixer_set_spkrgain_bitsel(chal_handle->chalSrcMixerHandle,
					       CAPH_M0_Right,
					       SRCMIXER_CH_DEF_BITSEL);
	chal_caph_srcmixer_set_spkrgain_bitsel(chal_handle->chalSrcMixerHandle,
...
static int ssasw_ioremap_exit(SSASW_ChalHandle_t * chal_handle)
{
	chal_caph_srcmixer_set_spkrgain_bitsel(chal_handle->chalSrcMixerHandle,
					       CAPH_M0_Left, 0x0000);
	chal_caph_srcmixer_set_spkrgain_bitsel(chal_handle->chalSrcMixerHandle,
					       CAPH_M0_Right, 0x0000);
	chal_caph_srcmixer_set_spkrgain_bitsel(chal_handle->chalSrcMixerHandle,
					       CAPH_M1_Left, 0x0000);
	chal_caph_srcmixer_set_spkrgain_bitsel(chal_handle->chalSrcMixerHandle,
					       CAPH_M1_Right, 0x0000);
	...
	release_mem_region(SRCMIXER_PHYS_BASE_ADDR_START,
			   SRCMIXER_REGISTER_LENGTH);
...
static int ssasw_set_clock(SSASW_ChalHandle_t * chal_handle, int enable)
{
	...
	if (enable) {
		...
		if (err) {
			printk(KERN_ERR
			       "%s: failed to set rate on "
			       "CAPH SRC Mixer clock %d!\n", __func__, err);
			return err;
		}
		...
		chal_caph_switch_disable_clock_bypass
		    (chal_handle->chalSsaswHandle);
...
int ssasw_init(void)
{
	...
	rc = ssasw_ioremap_init(&gChalHandle);
	if (rc != 0) {
...
SSASW_Handle_t ssasw_request_channel(SSASW_Device_t dev, uint16_t src_addr,
				     uint16_t dst_addr)
{
	...
	if ((dev < 0) || (dev >= SSASW_NUM_DEVICE_ENTRIES)
	    || devAttr->name == NULL) {
	...
	channel->caph_switch_ch =
	    chal_caph_switch_alloc_given_channel(chal_handle->chalSsaswHandle,
						 CAPH_SWITCH_CH_VOID);
	...
	if (devAttr->src_in_fifo != CAPH_CH_INFIFO_NONE
	    && devAttr->src_out_fifo != CAPH_CH_INFIFO_NONE) {
		/* We require a second switch channel */
		for (channel_src_idx = 0; channel_src_idx < SSASW_NUM_CHANNELS;
		     channel_src_idx++) {
		...
		if (channel_src_idx == SSASW_NUM_CHANNELS) {
			...
			chal_caph_switch_free_channel
			    (gChalHandle.chalSsaswHandle,
			     channel->caph_switch_ch);
		...
		channel_sec->caph_switch_ch =
		    chal_caph_switch_alloc_given_channel
		    (chal_handle->chalSsaswHandle, CAPH_SWITCH_CH_VOID);
		if (channel_sec->caph_switch_ch == CAPH_SWITCH_CH_VOID) {
			/* Failed to allocate second channel. Cleanup main
			 * channel
			 */
			chal_caph_switch_free_channel
			    (gChalHandle.chalSsaswHandle,
			...
			up(&gSSASW.lock);
			printk(KERN_ERR
			       "%s Allocation of main SWITCH channel failed\n",
			       __func__);
			return SSASW_INVALID_HANDLE;
		}
	...
	if (devAttr->src_in_fifo != CAPH_CH_INFIFO_NONE) {
		chal_caph_srcmixer_set_SRC(chal_handle->chalSrcMixerHandle,
					   devAttr->src_in_fifo,
					   devAttr->mix_freq);
		chal_caph_srcmixer_set_filter_type
		    (chal_handle->chalSrcMixerHandle, devAttr->src_in_fifo,
		     CAPH_SRCM_LINEAR_PHASE);
		chal_caph_srcmixer_set_fifo_datafmt
		    (chal_handle->chalSrcMixerHandle, devAttr->src_in_fifo,
		     devAttr->src_in_data_format);
		chal_caph_srcmixer_set_fifo_thres
		    (chal_handle->chalSrcMixerHandle, devAttr->src_in_fifo, 5,
		     2);
		chal_caph_srcmixer_clr_fifo(chal_handle->chalSrcMixerHandle,
					    devAttr->src_in_fifo);
	}

	if (devAttr->src_out_fifo != CAPH_CH_INFIFO_NONE) {
		chal_caph_srcmixer_set_fifo_datafmt
		    (chal_handle->chalSrcMixerHandle, devAttr->src_out_fifo,
		     devAttr->src_out_data_format);
		chal_caph_srcmixer_set_fifo_thres
		    (chal_handle->chalSrcMixerHandle, devAttr->src_out_fifo, 3,
		     6);
		chal_caph_srcmixer_clr_fifo(chal_handle->chalSrcMixerHandle,
					    devAttr->src_out_fifo);

		if (devAttr->mix_src_ch != CAPH_SRCM_CH_NONE
		    && devAttr->mix_dst_ch != CAPH_M_NONE) {
			if (devAttr->mix_dst_ch ==
			    (CAPH_M0_Left | CAPH_M0_Right)) {
				chal_caph_srcmixer_set_mixingainstep
				    (chal_handle->chalSrcMixerHandle,
				     devAttr->mix_src_ch, CAPH_M0_Left,
				     SRCMIXER_CH_DEF_GAIN_STEP);
				chal_caph_srcmixer_set_mixingainstep
				    (chal_handle->chalSrcMixerHandle,
				     devAttr->mix_src_ch, CAPH_M0_Right,
				     SRCMIXER_CH_DEF_GAIN_STEP);
				chal_caph_srcmixer_set_mixingain(chal_handle->
								 chalSrcMixerHandle,
								 devAttr->
								 mix_src_ch,
								 CAPH_M0_Left,
				...
				chal_caph_srcmixer_set_mixingain(chal_handle->
								 ...
								 devAttr->
								 mix_dig_gain);
				chal_caph_srcmixer_set_spkrgain_bitsel
				    (chal_handle->chalSrcMixerHandle,
				     devAttr->mix_src_ch,
				     SRCMIXER_CH_DEF_BITSEL);
			} else {
				chal_caph_srcmixer_set_mixingainstep
				    (chal_handle->chalSrcMixerHandle,
				     devAttr->mix_src_ch, devAttr->mix_dst_ch,
				     SRCMIXER_CH_DEF_GAIN_STEP);
				chal_caph_srcmixer_set_mixingain(chal_handle->
								 chalSrcMixerHandle,
								 devAttr->
	...
	/* Set switch trigger */
	chal_caph_switch_select_trigger(chal_handle->chalSsaswHandle,
					channel->caph_switch_ch,
					devAttr->dev_sw_trig);

	/* Set data format */
	chal_caph_switch_set_datafmt(chal_handle->chalSsaswHandle,
				     channel->caph_switch_ch,
				     devAttr->sw_data_format);

	if (devAttr->src_out_fifo != CAPH_CH_INFIFO_NONE) {
		/* Override source address to SRC Mixer out FIFO specified by
		 * device
		 */
		srcmixer_addr =
		    (uint16_t)(SRCMIXER_BASE_ADDR +
			       chal_caph_srcmixer_get_fifo_addr_offset
			       (chal_handle->chalSrcMixerHandle,
				devAttr->src_out_fifo));

		/* Set src mixer switch source */
		chal_caph_switch_select_src(chal_handle->chalSsaswHandle,
					    channel->caph_switch_ch,
					    (uint16_t)srcmixer_addr);
	} else {
	...
	if (devAttr->src_in_fifo != CAPH_CH_INFIFO_NONE
	    && devAttr->src_out_fifo == CAPH_CH_INFIFO_NONE) {
		/* Override destination address to SRC Mixer in FIFO specified
		 * by device
		 */
		srcmixer_addr =
		    (uint16_t)(SRCMIXER_BASE_ADDR +
		...
		chal_caph_switch_add_dst(chal_handle->chalSsaswHandle,
					 channel->caph_switch_ch,
					 (uint16_t)srcmixer_addr);
	} else {
...
int ssasw_free_channel(SSASW_Handle_t handle	/* SSASW handle. */
    )
{
	...
	chal_caph_switch_free_channel(chal_handle->chalSsaswHandle,
				      channel->caph_switch_ch);
...
*/
/****************************************************************************/
int ssasw_add_dst(SSASW_Handle_t handle,	/* SSASW handle. */
		  uint16_t dst_addr)
{
	SSASW_Channel_t *channel = NULL;
	SSASW_Channel_t *channel_src = NULL;
...
*/
/****************************************************************************/
int ssasw_remove_dst(SSASW_Handle_t handle,	/* SSASW handle. */
		     uint16_t src_addr)
{
	SSASW_Channel_t *channel;
	SSASW_Channel_t *channel_src = NULL;
	...
	if (channel_src)
		chal_caph_switch_remove_dst(chal_handle->chalSsaswHandle,
					    channel_src->caph_switch_ch,
					    src_addr);
	else
		chal_caph_switch_remove_dst(chal_handle->chalSsaswHandle,
					    channel->caph_switch_ch, src_addr);
...
*/
/****************************************************************************/
int ssasw_remove_all_dst(SSASW_Handle_t handle	/* SSASW handle. */
    )
{
	SSASW_Channel_t *channel;
	SSASW_Channel_t *channel_src = NULL;
	...
	/* Only remove destinations to SRC switch channel if available */
	if (channel_src)
		chal_caph_switch_clear_all_dst(chal_handle->chalSsaswHandle,
					       channel_src->caph_switch_ch);
	else
		chal_caph_switch_clear_all_dst(chal_handle->chalSsaswHandle,
					       channel->caph_switch_ch);
...
*/
/****************************************************************************/
int ssasw_mixer_gain_set(SSASW_Device_t dev, int db)
{
	SSASW_DeviceAttribute_t *devAttr;
	SSASW_ChalHandle_t *chal_handle;
	int lin_gain;
...
*/
/****************************************************************************/
int ssasw_enable(SSASW_Handle_t handle, int enable)
{
	SSASW_Channel_t *channel;
	SSASW_Channel_t *channel_src = NULL;
	SSASW_DeviceAttribute_t *devAttr;
	...
	if (enable) {
		chal_caph_switch_enable(chal_handle->chalSsaswHandle,
					channel->caph_switch_ch);
		if (channel_src)
			chal_caph_switch_enable(chal_handle->chalSsaswHandle,
						channel_src->caph_switch_ch);
		...
		if (channel_src)
			chal_caph_switch_disable(chal_handle->chalSsaswHandle,
						 channel_src->caph_switch_ch);
		chal_caph_switch_disable(chal_handle->chalSsaswHandle,
					 channel->caph_switch_ch);
	}
...
*/
/****************************************************************************/
int ssasw_mixer_enable(SSASW_Handle_t handle, int enable)
{
	SSASW_Channel_t *channel;
	SSASW_Channel_t *channel_src = NULL;
	SSASW_DeviceAttribute_t *devAttr;
	...
	if (enable) {
		if (devAttr->src_out_fifo != CAPH_CH_INFIFO_NONE) {
			if (devAttr->mix_src_ch != CAPH_SRCM_CH_NONE
				...
				if (devAttr->mix_dst_ch ==
				    (CAPH_M0_Left | CAPH_M0_Right)) {
					chal_caph_srcmixer_enable_mixing
					    (chal_handle->chalSrcMixerHandle,
					     devAttr->mix_src_ch, CAPH_M0_Left);
					chal_caph_srcmixer_enable_mixing
					    (chal_handle->chalSrcMixerHandle,
		...
		if (devAttr->src_in_fifo != CAPH_CH_INFIFO_NONE) {
			chal_caph_srcmixer_enable_chnl
			    (chal_handle->chalSrcMixerHandle,
			     devAttr->src_in_fifo);
		}
	} else {
		if (devAttr->src_in_fifo != CAPH_CH_INFIFO_NONE) {
			chal_caph_srcmixer_disable_chnl
			    (chal_handle->chalSrcMixerHandle,
			     devAttr->src_in_fifo);
		}
		...
		if (devAttr->src_out_fifo != CAPH_CH_INFIFO_NONE) {
			if (devAttr->mix_src_ch != CAPH_SRCM_CH_NONE
				...
				if (devAttr->mix_dst_ch ==
				    (CAPH_M0_Left | CAPH_M0_Right)) {
					chal_caph_srcmixer_disable_mixing
					    (chal_handle->chalSrcMixerHandle,
					     devAttr->mix_src_ch, CAPH_M0_Left);
					chal_caph_srcmixer_disable_mixing
					    (chal_handle->chalSrcMixerHandle,
