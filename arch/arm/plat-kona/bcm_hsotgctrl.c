...
#define	P1CTL_BG_CTRL_MASK	HSOTG_CTRL_PHY_P1CTL_AFE_LDOBG_OUTADJ_MASK
#define	P1CTL_AFE_LDO_CTRL_MASK		0x000F0000
#define	P1CTL_BG_CTRL_VALUE_MASK		0x08000000
#define	P1CTL_AFE_LDO_CTRL_VALUE_MASK	0x000A0000
#endif
...
int bcm_hsotgctrl_phy_init(bool id_device)
{
	...
	if ( NULL == local_hsotgctrl_handle) return -ENODEV;
...
#ifdef CONFIG_ARCH_CAPRI
	if ((readl(bcm_hsotgctrl_handle->chipregs_base +
		    CHIPREG_CHIPID_REG_OFFSET) & 0xFF) == 0xA0) {
		val = readl(bcm_hsotgctrl_handle->hsotg_ctrl_base
			+ HSOTG_CTRL_PHY_P1CTL_OFFSET);
		val &= ~(P1CTL_BG_CTRL_MASK | P1CTL_AFE_LDO_CTRL_MASK);
		val |= (P1CTL_BG_CTRL_VALUE_MASK |
			P1CTL_AFE_LDO_CTRL_VALUE_MASK);
		writel(val, bcm_hsotgctrl_handle->hsotg_ctrl_base
			+ HSOTG_CTRL_PHY_P1CTL_OFFSET);
	}
#endif
...
int bcm_hsotgctrl_bc_reset(void)
{
	...
	val |= (BCCFG_SW_OVERWRITE_KEY | HSOTG_CTRL_BC_CFG_SW_OVWR_EN_MASK);
	val |= HSOTG_CTRL_BC_CFG_SW_RST_MASK;
	/*Reset BC1.1 state machine */
	writel(val, bcm_hsotgctrl_handle->hsotg_ctrl_base
	+ HSOTG_CTRL_BC_CFG_OFFSET);

	mdelay(BC_CONFIG_DELAY_MS);
	/* Clear reset */
...
int bcm_hsotgctrl_bc_vdp_src_off(void)
{
	...
	/* Clear overwrite key so we don't
	accidently write to these bits */
	val &= ~(HSOTG_CTRL_BC_CFG_BC_OVWR_KEY_MASK
		| HSOTG_CTRL_BC_CFG_SW_OVWR_EN_MASK);
	writel(val, bcm_hsotgctrl_handle->hsotg_ctrl_base
		+ HSOTG_CTRL_BC_CFG_OFFSET);
...
#ifdef CONFIG_ARCH_CAPRI
	if ((readl(hsotgctrl_drvdata->chipregs_base +
		    CHIPREG_CHIPID_REG_OFFSET) & 0xFF) == 0xA0) {
		val = readl(hsotgctrl_drvdata->hsotg_ctrl_base
			+ HSOTG_CTRL_PHY_P1CTL_OFFSET);
		val &= ~(P1CTL_BG_CTRL_MASK | P1CTL_AFE_LDO_CTRL_MASK);
...
int bcm_hsotgctrl_phy_set_id_stat(bool floating)
{
	...
	if (NULL != local_hsotgctrl_handle)
		...
		return -ENODEV;
  printk("USBD][%s] start floating=%d\n",__func__,floating);
	val = readl(hsotg_ctrl_base + HSOTG_CTRL_USBOTGCONTROL_OFFSET);
...
{
	if ((NULL != local_hsotgctrl_handle) && suspend_allowed) {

		/* Return the status */
		*suspend_allowed = local_hsotgctrl_handle->allow_suspend;
		
		pr_info("USBD][bcm_hsotgctrl_is_suspend_allowed]: %d\n",local_hsotgctrl_handle->allow_suspend);		
	} else {
		/* No device handle */
		return -ENODEV;
	}
