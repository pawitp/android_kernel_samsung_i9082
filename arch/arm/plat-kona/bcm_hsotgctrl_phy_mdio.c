...
/*Apply MDIO tuning value for each model*/
/*!!!!!NOTE DON'T CHANGE FOLLOW VALUE WITHOUT CONFIRM - contact USB engineer first!!!!!*/
#if defined(CONFIG_MACH_CAPRI_SS_BAFFIN) /*Baffin rev0.3 pass*/
...
#elif defined(CONFIG_MACH_CAPRI_SS_CRATER)/*Crater rev0.0 temp.*/
static int USB_MDIO[6] = {
	0x18,			
	0,				
	0,				
...
#elif defined(CONFIG_MACH_CAPRI_SS_S2VE)/*120918 S2VE rev0.5 pass - DON'T CHANGE IT*/
static int USB_MDIO[6] = {
	0x1400,			
	0,				
	0,				
...
int bcm_hsotgctrl_phy_mdio_read(int mdio_num)
{
	...
	if ((!bcm_hsotgctrl_handle->mdio_master_clk) ||
		...
		printk("%s, Err:No Mdio_Master_Clk Handler\n",__func__);
		return -EIO;
	}
	...
	* Read to MDIO Num */
	...
	val = readl(bcm_hsotgctrl_handle->chipregs_base +
		CHIPREG_MDIO_CTRL_RD_DATA_OFFSET);
...
int bcm_hsotgctrl_phy_mdio_write(int mdio_num, int value)
{
	...
	if ((!bcm_hsotgctrl_handle->mdio_master_clk) ||
		  (!bcm_hsotgctrl_handle->dev)){
		 printk("%s-Err:No Mdio_Master_Clk\n",__func__);
		return -EIO;
	}
...
bcm_hsotg_TxCurrent_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	int val;// = (bcm_hsotgctrl_phy_mdio_read(3)&TX_CURRENT_MASK)>>TX_CURRENT_OFFSET;
	int i;

	val = bcm_hsotgctrl_phy_mdio_read(3);
	
	if(val<0)
		return 0;
	printk("%s-current MDIO3:0x%x\n",__func__,val);

	val = (val&TX_CURRENT_MASK)>>TX_CURRENT_OFFSET;
				 ...
				 const char *buf, size_t count)
{
	...
	if(bcm_hsotgctrl_phy_mdio_write(3, USB_MDIO[3])<0)
		goto error_Set_Tx_Current;

	printk("%s-current MDIO3:0x%x after Setting\n",__func__,bcm_hsotgctrl_phy_mdio_read(3));
...
bcm_hsotg_Squelch_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	int val;// = (bcm_hsotgctrl_phy_mdio_read(4)&SQUELCH_MASK)>>SQUELCH_OFFSET;
	int i;
	
	val = bcm_hsotgctrl_phy_mdio_read(4);

	if(val<0)
		return 0;
	
	val = (val&SQUELCH_MASK)>>SQUELCH_OFFSET;
				 ...
				 const char *buf, size_t count)
{
	...
	if(bcm_hsotgctrl_phy_mdio_write(4, USB_MDIO[4])<0)
		goto error_Set_Squelch;

	printk("%s,new setting MDIO4:0x%x\n",__func__,bcm_hsotgctrl_phy_mdio_read(4));
...
module_init(bcm_hsotgctrl_usb_phy_mdio_init);
