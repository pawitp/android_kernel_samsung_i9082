...
static int ccu_prof_set_prof_type(struct ccu_profiler *ccu_profiler,
...
static int ccu_prof_get_prof_type(struct ccu_profiler *ccu_profiler)
{
	struct clk *clk;
	struct ccu_clk *ccu_clk;
	u32 reg;
...
static int ccu_prof_set_policy(struct ccu_profiler *ccu_profiler,
...
static int ccu_prof_get_policy(struct ccu_profiler *ccu_profiler)
{
	struct clk *clk;
	struct ccu_clk *ccu_clk;
	u32 reg;
...
static int ccu_prof_set_autogate_sel(struct ccu_profiler *ccu_profiler,
...
static int ccu_prof_get_autogate_sel(struct ccu_profiler *ccu_profiler,
					u32 select, u32 *value)
{
	struct clk *clk;
	struct ccu_clk *ccu_clk;
...
static int ccu_prof_set_clkreq_sel(struct ccu_profiler *ccu_profiler,
				      u32 select, u32 value)
{
	struct clk *clk;
	struct ccu_clk *ccu_clk;
...
static int ccu_prof_get_clkreq_sel(struct ccu_profiler *ccu_profiler,
				      u32 select, u32 *value)
{
	struct clk *clk;
	struct ccu_clk *ccu_clk;
		...
		loff_t *ppos)
{
	...
	if (ccu_profiler->profiler.ops->status(&ccu_profiler->profiler)) {
		...
		err = ccu_profiler->profiler.ops->get_counter(
				&ccu_profiler->profiler,
				&cnt, &overflow);
		duration_ms = (unsigned long)((u64)(curr_time -
			      ccu_profiler->profiler.start_time)
			      * 1000 / CLOCK_TICK_RATE);
	}
	if (overflow) {
		count = sprintf(buffer,
				"duration_ms = %lu cnt_raw = %lu* "
				"cnt_ms = %lu*\n",
...
static struct prof_ops ccu_prof_ops = {
	.init = ccu_prof_init,
...
static struct gen_ccu_prof_ops gen_ccu_prof_ops = {
