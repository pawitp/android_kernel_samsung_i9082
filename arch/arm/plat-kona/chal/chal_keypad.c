					    ...
					    keyEvents)
{
	...
	for (i = 0; i < bitCount; i++) {	/* loop through all set bits, as
		...
		/* this isolates the rightmost '1' bit from mask */
		firstbit = mask & (-mask);
...
/* max number of interrupt event that can be stored */
/* must be a power of 2 for fifo operations to work */
#define CHAL_INTERRUPT_EVENT_FIFO_LENGTH	4

/* max number of key event that can be stored since each interrupt */
/* register set can have a max of 4 simultaneous events it must be */
/* at least 4 must be a power of 2 for fifo operations to work */
#define CHAL_KEYPAD_KEY_ACTION_FIFO_LENGTH	4

/* max valid key events in one interrupt-hardware defined */
#define MAX_KEY_EVENT_PER_INTERRUPT		4
						  ...
						  cUInt32 isr)
{
	...
	/* loop through all set bits, as each indicates a key event */
	...
	if ((!FIFO_EMPTY(interruptEventQ)) && (!FIFO_FULL(keyActionQ))) {
		...
		if ((bc1 + bc2) <= MAX_KEY_EVENT_PER_INTERRUPT) {
			...
			/* appears to be an error (too many bits set in ISR)
			   clear the interrupts to reset and hopefully
			   continue */
