		       ...
		       unsigned int xfer_size, int control, int cfg)
{
	...
	case DMA_DIRECTION_MEM_TO_UNALIGNED_MEM:
		rqtype = MEMTOMEM;
		break;
...
int dma_start_transfer(unsigned int chan)
{
	 ...
	 */
	dmux_sema_protect();
	ret = pl330_submit_req(c->pl330_chan_id, &c->req);
	if (ret != 0)
		goto err2;

	/* Start DMA channel thread */
	ret = pl330_chan_ctrl(c->pl330_chan_id, PL330_OP_START);
	if (ret != 0)
		goto err2;
...
static int pl330_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(&pdev->dev, "pl330_add failed\n");
		goto probe_err2;
	}
