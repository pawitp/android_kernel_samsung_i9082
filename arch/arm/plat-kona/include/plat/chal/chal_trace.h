...
{
#if defined (_HERA_)
	void __iomem *APB2ATB_base;     /* APB2ATB register block
					 * (Apps_Subsystem.Fabric.APB8)
					 */
	void __iomem *ATB2PTI_base;     /* ATB2PTI register block (Hub.APB9) */
	void __iomem *ATB2OCP_base;     /* ATB2OCP register block (Hub.APB9) */
	void __iomem *WGM_OCP2ATB_base; /* WGM_OCP2ATB register block
					 * (Comm_Subsystem.APB13)
					 */
	void __iomem *HUBOCP2ATB_base;  /* HUBOCP2ATB register block
					 * (Hub.APB9)
					 */
#endif //_HERA_
	void __iomem *GICTR_base;/* GIC Trace (Apps_Subsystem.Processor.APB0) */
	void __iomem *CHIPREGS_base; /* CHIPREGS Block */
	void __iomem *PWRMGR_base; /* PWRMGR Block  (Hub.Power_Manager) */
	...
	void __iomem *ATB_STM_base; /* ATB_STM register block (Hub.APB9) */
	void __iomem *SW_STM_base[CHAL_TRACE_MAX_SWSTM]; /* SWSTM register block
					(Apps_Subsystem.Processor.APB0) */
}CHAL_TRACE_DEV_t;
...
*  @param   twobit_mode (in) When set, selects 2-bit output mode. When clear, 4-bit mode is active.
*  @param   intlv_mode (in) When set, selects interleaved mode of operation for STP messages from hub XTI OCP output mixed with ATB messages. When clear, only ATB messages are sent to PTI.
...
*  @param   atb_id (in) Defines ATB ID value to match for SW source (from APB2ATB). Bit 0 = don't care (source uses for timestamp enable).
*
*  @return  status 
*
*  @note    
...
* 
*  @param   handle (in) Handle returned in chal_trace_init()
*  @param   stall_mode (in) When set, stalls APB bus for full output FIFO. When clear, writes are dropped for full output FIFO.
*
*  @return  status 
*
*  @note    
...
*           the effect of excluding the port from the priority selection scheme. The reset value
...
/**
*
*  @brief  Config Funnel_Control bits CSTF Control Register Minimum_hold_time_3_0 field
* 
*  @param   handle (in) Handle returned in chal_trace_init()
*  @param   funnel_type (in) type of funnel.
...
*  @return  status 
*
*  @note    The formatting scheme can easily become inefficient if fast switching occurs, so,
*           where possible, this must be minimized. If a source has nothing to transmit, then
*           another source is selected irrespective of the minimum number of cycles. Reset
*           is 0x3. The CSTF holds for the minimum hold time and one additional cycle.
*           The maximum value that can be entered is 0xE and this equates to 15 cycles. 0xF is reserved.
*           Reset value is 0x3.
*
******************************************************************************/
...
*           location is the value that you want to assign the first slave port.
*           At reset the default configuration assigns priority 0 to port 0, to priority 1 to port 1, and
*           so on.
*           If you want to give highest priority to a particular slave port, the corresponding port
*           must be programmed with the lowest value. Typically this is likely to be a port that
*           has more important data or that has a small FIFO and is therefore likely to overflow.
*           If you want to give lowest priority to a particular slave port, the corresponding slave port
*           must be programmed with the highest value. Typically this is likely to be a device that
*           has a large FIFO that is less likely to overflow or a source that has information that is of
*           lower importance.
*           A port programmed with value 0 gets the highest priority. A port programmed with
*           value 7 gets the lowest priority. Priority must always go to the highest priority source
*           that has valid data available, if enabled. If a priority value has been entered for multiple
*           different slave ports then the arbitration logic selects the lowest port number of them.
*           This register must only be altered when the trace system is disabled, that is, trace
...
*           When no integration functionality has been put into a component, because it is not
*           required, writing a HIGH to this location must be ignored and must return a LOW
*           on read.
...
*           32'h0000000F indicates that this claim tag is four bits wide. Logic 1 indicates that this
*           bit can be set. Logic 0 indicates that this bit is unimplemented, that is, it cannot be set.
*           Writes:
...
*           All CoreSight components implement 32-bit lock access registers but they are only
*           present on accesses when PADDRDBG31 is LOW. PADDRDBG31 is used to bypass
*           the lock control mechanism and must be connected to PADDRDBG[31] which is only
*           HIGH on accesses from external tools. For more information about the Lock Access
...
/**
*
*  @brief  Return TRUE when the lock mechanism is implemented and device write access is locked.
* 
*  @param   handle (in) Handle returned in chal_trace_init()
*  @param   funnel_type (in) type of funnel.
*
...
*           Sub type, for trace sources this is broken down into cores, DSPs or buses.
*           
...
*
*  @note    Bit Field 13 Outstanding Commands Cycle Counting State. When set, counting is enabled.
*           Bit Field 12 Latency Measurement State. When set, measurements are enabled.
*           Bit Field 11 Data Beat Counting State. When set, counting is enabled.
*           Bit Field 10 Busy Cycle Counting State. When set, counting is enabled.
*           Bit Field 9 Commands Counting State. When set, counting is enabled.
*           Bit Field 8 Saturation Stopped. When set, at least one counter reached saturation with SAT_EN=1. 
*                       While set, counters are stopped. Cleared by a counter clear command.
...
*  @param   addr_low (in) Address Low. AXI addresses must be >= this value (with bits 7:0 assumed 0x00) to pass this filter.
*  @param   addr_high (in) Address High. AXI addresses must be <= this value (with bits 7:0 assumed 0xFF) to pass this filter.
*
*  @return  status 
*
*  @note    
...
* 
*  @param   handle (in) Handle returned in chal_trace_init()
*  @param   flush (in) Controls flushing of FIFO's to memory. Auto-cleared by hardware when flushing complete.
*
*  @return  status 
*
*  @note    
...
* 
*  @param   handle (in) Handle returned in chal_trace_init()
*  @param   twobit_mode (in) Two bit mode. When set, PTI output is 2 bits wide. When cleared, PTI output is 4 bits wide.
*  @param   break_limit (in) Break Limit. 
*           This field sets the STP continuous message limit before forced master ID insertion: 
*           00=always break, 01=after 7th message, 10=after 15th, and 11=after 31st. 
*           For software messages the number of continous messages includes any channel number.
*  @param   output_mode (in) Output Mode. 
*           This field sets the output mode: 00=off, 01=PTI, 10=ATB, 11=rsvd. When the output mode is off all input is dropped. 
*           When the output mode is PTI, output data is intended for the PTI pins. 
*           When the output mode is ATB, output data is sent to the ATB Funnel for TPIU consumption.
*  @param   atb_id (in) ATB output ID. This field sets the value of the ATB ID for the bus output to the Funnel.
*
*  @return  status 
*
*  @note    
...
*           Writes from disabled (value 0) masters are discarded after handshaking from the input ATB.
...
*           This field has bits for setting the type (1=SW, 0=HW) for STP master ID's 31:0. 
*           Software masters are expected to follow the SW_STM 2-cycle protocol on the input ATB. 
*           Hardware masters produce one value per ATB cycle.
*  @param   high_half (in) Master ID types (high half). 
*           This field has bits for setting the type (1=SW, 0=HW) for STP master ID's 63:32. 
*           Software masters are expected to follow the SW_STM 2-cycle protocol on the input ATB. 
*           Hardware masters produce one value per ATB cycle.
*
*  @return  status 
...
*  @param   stall_mode (in) When set, stalls APB bus for full output FIFO. When clear, writes are dropped for full output FIFO.
*  @param   atb_id (in) ATB ID value. Timestamp data sets bit 0 on ATB bus, non-timestamp data clears it. 
