...
volatile int pre_pe_count;
volatile int post_pe_count;
				      ...
				      struct cpuidle_state *state)
{
...
#ifdef CONFIG_HAS_WAKELOCK
	/*Enter simple WFI (safe state) if idle wake lock is active*/
	...
	pre_pe_count = preempt_count();
	if (pre_pe_count > 1) {
		printk("%s:pec = %d\n", __func__, pre_pe_count);
	}
	if (allow_idle) {
#ifdef CONFIG_BCM_IDLE_PROFILER
	...
	post_pe_count = preempt_count();
	if (pre_pe_count != post_pe_count) {
		printk("%s:pec = %d, %d \n", __func__, pre_pe_count, post_pe_count);
	}
	local_irq_enable();
	local_fiq_enable();
...
#endif
{
	...
	pr_info("--%s : registering cpu_idle handlers\n", __func__);
	...
	for_each_possible_cpu(cpu) {
		...
		for (i = 0; i < num_states; i++) {
			...
			state->target_residency =
			    idle_states[i].target_residency;
			state->flags = idle_states[i].flags;
			state->enter = kona_mach_enter_idle_state;
			if (i == 0)
				dev->safe_state = state;
			strncpy(state->name, idle_states[i].name,
				CPUIDLE_NAME_LEN - 1);
			state->name[CPUIDLE_NAME_LEN - 1] = 0;
			strncpy(state->desc, idle_states[i].desc,
				CPUIDLE_DESC_LEN - 1);
...
#ifdef CONFIG_SUSPEND
	pr_info("--%s : registering suspend handlers\n", __func__);
