...
#undef ENABLE_HUB_CCU_CLK_SWITCH
#if defined(ENABLE_HUB_CCU_CLK_SWITCH)
static DEFINE_SPINLOCK(ccu_caph_lock);
#endif
...
#if defined(ENABLE_HUB_CCU_CLK_SWITCH)
	hub_pi = pi_mgr_get(PI_MGR_PI_ID_HUB_SWITCHABLE);
	hub_ccu_clk = to_ccu_clk(hub_pi->pi_ccu[0]);
#endif
	for (inx = 0; inx < pi->num_ccu_id; inx++) {
		...
		res = ccu_set_freq_policy(to_ccu_clk(pi->pi_ccu[inx]),
				CCU_POLICY(policy), freq, opp_inx);
#if defined(ENABLE_HUB_CCU_CLK_SWITCH)
		if (pi->id == PI_MGR_PI_ID_ARM_CORE) {
			if (opp_inx == 0)
				fid2 = 1;
			else
				fid2 = 2;

			freq_id = ccu_get_freq_policy(hub_ccu_clk,
				CCU_POLICY(policy));
			caph_ret = AUDCTRL_CcuClk(0);
			if (caph_ret == 0 && freq_id != fid2) {
				spin_lock_irqsave(&ccu_caph_lock, flags);
				AUDCTRL_CcuClk(1);
				ccu_set_freq_policy(hub_ccu_clk,
					CCU_POLICY(policy), fid2, opp_inx);
				AUDCTRL_CcuClk(2);
				spin_unlock_irqrestore(&ccu_caph_lock, flags);
				pi_dbg("%s: A9 freq ID => %d hub fid %d\n",
					__func__, opp_inx, fid2);
			} else if (freq_id != fid2) {
				pr_err("%s CAPH not ready for fid %d"
				" --> %d\n", __func__, freq_id, fid2);
			}
		}
#endif
		if (res != 0)
				...
				u32 pi_id, u32 lat_value)
{
	...
	if (unlikely(pi_id >= PI_MGR_PI_ID_MAX \
				 ...
				 u32 weightage)
{
	...
	pi_dbg("%s:client = %s pi = %d opp = %d opp_new = %d weightage = %d\n",
		__func__, node->name, node->pi_id, opp, node->opp, weightage);
...
__weak char *get_opp_name(int opp, int pi_id)
{
	return NULL;
}
...
static int pi_debug_register_qos_client(void *data, u64 value)
{
	...
	dent_client = debugfs_create_file(debug_qos_client[val].client_name, \
				S_IWUSR|S_IRUSR, qos_dir, \
...
static int pi_debug_register_dfs_client(void *data, u64 value)
{
	...
	int ret = -1;
	u32 val = (u32)value;
	struct dentry *dfs_dir, *dent_client;
	...
	sprintf(debug_dfs_client[val].client_name, \
		"%s_%d", DEBUGFS_PM_CLIENT_NAME, val);

	ret = pi_mgr_dfs_add_request(&debug_dfs_client[val].debugfs_dfs_node, \
			debug_dfs_client[val].client_name, \
			pi->id, PI_MGR_DFS_MIN_VALUE);
	if (ret)
		pi_dbg("%s: DFS add request failed for %s\n", \
			__func__, pi->name);

	dent_client = debugfs_create_file(debug_dfs_client[val].client_name, \
		S_IWUSR|S_IRUSR, dfs_dir,
