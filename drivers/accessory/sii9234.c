...
#define DEBUG_MHL_FUNC_START() pr_info("sii9234 %s START \n", __func__)
#define PRINT_LOG pr_info("sii9234 %s %d \n", __func__, __LINE__)

/* 
 * __CONFIG_RSEN_TIMER__
 *
 * Sometimes, RSEN_CHANGE_INT does not occurr in time (about 2~3 sec)
 * after that, it is not possible to use both USB and MHL.
 * To prevent this, 5 second timer has been applyed.
 * If RSEN_CHANGE_INT does not occurr in 5 sec, the power of MHL will be off.
*/
...
#else
static int goto_d3();
static int WriteInitialRegisterValues(void);
#endif
					...
					const char *buf, size_t size)
{
	...
	//sii9234->pdata->swing_level = sii9234->pdata->swing_level | (data << 3) | clk;
	sii9234->pdata->swing_level = sii9234->pdata->swing_level | data | (clk << 3);
...
static void mhl_check_rsen_timer(unsigned long data)
{
	...
	INIT_WORK(&sii9234->rgnd_work, mhl_check_rsen_timer_callback);
...
static int sii9234_cbus_init(struct sii9234_data *sii9234)
{
	...
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x82, 0x02); /* DEV_CAT */
	if (ret < 0)
		goto i2c_error_exit;
	...
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x85, BIT0);/* VID_LINK_MODE */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x86, 0x01); /* AUD_LINK_MODE */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x87, 0); /* VIDEO_TYPE */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x88, (1<<7)); /* LOG_DEV_MAP */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x89, 0x0F); /* BANDWIDTH */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x8A, (1<<0) | (1<<1) | (1<<2)); /* FEATURE_FLAG */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x8B, 0); /* DEVICE_ID_H */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x8C, 0); /* DEVICE_ID_L */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x8D, 16); /* SCRATCHPAD_SIZE */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x8E, 0x33); /* INT_STAT_SIZE */
	if (ret < 0)
		goto i2c_error_exit;
...
static void cbus_req_abort_error(struct sii9234_data *sii9234)
{
	...
	pr_info("sii9234: MSC Request Aborted:");
	...
	if (abort_reason) {
		...
		if (abort_reason & BIT_MSC_XFR_ABORT) {
			cbus_read_reg(sii9234, MSC_REQ_ABORT_REASON_REG,
				      &abort_reason);
			pr_info("sii9234 : ABORT_REASON_REG = %d\n", abort_reason);
		...
		if (abort_reason & BIT_MSC_ABORT) {
			...
			pr_info("sii9234 :BIT_MSC_ABORT = %d\n", abort_reason);
...
static void cbus_resp_abort_error(struct sii9234_data *sii9234)
{
	...
	pr_info("sii9234: MSC Response Aborted:");
...
static bool cbus_ddc_abort_error(struct sii9234_data *sii9234)
{
	...
	if (val2 > (val1 + 50)) {
		pr_info("Applying DDC Abort Safety(SWA 18958)\n)");
	...
	pr_info("sii9234: DDC abort interrupt\n");
...
static void cbus_handle_msc_msg(struct sii9234_data *sii9234)
{
	...
	pr_info("sii9234: cmd_code:%x, key:%d\n", cmd_code, key);

	switch (cmd_code) {
	case MSG_RCP:
		pr_info("sii9234: RCP Arrived. KEY CODE:%d\n", key);
		mutex_unlock(&sii9234->cbus_lock);
		cbus_process_rcp_key(sii9234, key);
		return;
	case MSG_RAP:
		pr_info("sii9234: RAP Arrived\n");
		mutex_unlock(&sii9234->cbus_lock);
		cbus_process_rap_key(sii9234, key);
		return;
	case MSG_RCPK:
		pr_info("sii9234: RCPK Arrived\n");
		break;
	case MSG_RCPE:
		pr_info("sii9234: RCPE Arrived\n");
		break;
	case MSG_RAPK:
		pr_info("sii9234: RAPK Arrived\n");
		break;
	default:
...
static void cbus_handle_wrt_stat_recd(struct sii9234_data *sii9234)
{
	...
	pr_info("sii9234: CBUS WRT_STAT_RECD\n");
...
static void cbus_handle_set_int_recd(struct sii9234_data *sii9234)
{
	...
	pr_info("sii9234: CBUS SET_INT_RECD \n");
	...
	pr_info("sii9234: INTR_REG0 : [%d]; INTR_REG1 : [%d]\n",
		 intr_reg0, intr_reg1);
...
#ifdef CONFIG_MHL_NEW_CBUS_MSC_CMD
	if (sii9234->dcap_ready_status == 1) {
...
static int WriteInitialRegisterValues(void)
{
	...
	pr_info("sii9234: WriteInitialRegisterValues start\n");
...
static void save_cbus_pkt_to_buffer(struct sii9234_data *sii9234)
{
	...
	pr_info("sii9234: save_cbus_pkt_to_buffer index = %d\n", index);
				 ...
				 enum cbus_command command, u8 offset, u8 data)
{
	...
	switch (sii9234->cbus_pkt.command) {
	case CBUS_SET_INT:
		pr_info("sii9234: cbus_command_request"
						"CBUS_SET_INT\n");
	...
	case CBUS_MSC_MSG:
		...
		pr_info("sii9234: cbus_command_request CBUS_MSC_MSG"
		...
		break;
	case CBUS_READ_DEVCAP:
		pr_info("sii9234: cbus_command_request CBUS_READ_DEVCAP\n");
		start_bit = START_BIT_READ_DEVCAP;
		break;
	case CBUS_WRITE_BURST:
		pr_info("sii9234: cbus_command_request CBUS_WRITE_BURST\n");
	...
	case CBUS_GET_DDC_ERR_CODE:
		...
		break;
	default:
		pr_info("sii9234: error send cbus command fail\n");
		mutex_unlock(&sii9234->cbus_lock);
		return false;
	}

	pr_info("sii9234: startbit = %d\n", start_bit);
...
static u8 sii9234_tmds_control(struct sii9234_data *sii9234, bool enable)
{
	...
	if (enable) {
		...
		if (ret < 0)
			return ret;
#endif
		ret = mhl_tx_set_reg(sii9234, MHL_TX_TMDS_CCTRL, BIT4);
		if (ret < 0)
			return ret;
		pr_info("sii9234: MHL HPD High, enabled TMDS\n");
		...
		ret = mhl_tx_clear_reg(sii9234, MHL_TX_TMDS_CCTRL, BIT4);
		if (ret < 0)
			return ret;
...
intr_check:
	...
	pr_info("sii9234: irq %02x/%02x %02x/%02x %02x/%02x\n",
		 intr1, intr1_en, intr4, intr4_en, cbus_intr1, cbus_intr2);

	if (intr4 & RGND_READY_INT) {
		pr_info("sii9234: RGND_READY_INT \n");
#ifdef CONFIG_MHL_D3_SUPPORT
		...
		ret = WriteInitialRegisterValues();
		if (ret<0) {
			pr_info("sii9234: WriteInitialRegisterValues failed \n");
			goto err_exit;
		}

		/* Force Power State to ON before reading RGND*/
		ret = mhl_tx_clear_reg(sii9234, MHL_TX_DISC_CTRL1_REG, BIT1);
		if (ret < 0) {
		...
		case RGND_INTP_1K:
			...
			/* Do not power off when RGND is 1K ohm */
			mhl_poweroff = 0;
		...
		if (sii9234->rgnd != RGND_1K) {
			ret = mhl_tx_clear_reg(sii9234, MHL_TX_DISC_CTRL6_REG, BIT5);
			if (ret < 0) {
				goto i2c_error_exit;
			}			
	...
	if (intr4 & CBUS_LKOUT_INT) {	/* 1<<4 */
		pr_info("%s(): CBUS Lockout Interrupt\n", __func__);
	...
	if (intr4 & MHL_DISC_FAIL_INT) {	/* 1<<3 */
		...
		if(sii9234->rgnd == RGND_1K) {
		pr_info("sii9244 retry discovery !!!\n");
			INIT_WORK(&sii9234->redetect_work, goto_d3);
	...
	if (intr4 & MHL_EST_INT) {	/* 1<<2 */
		...
		//msleep(400);
		goto intr_check_rsen;

	}

	if (intr1 & RSEN_CHANGE_INT) {

		pr_info("%s(): RSEN_CHANGE_INT \n", __func__);

intr_check_rsen:

		if(sii9234->state != STATE_ESTABLISHED) {
			pr_err("[ERROR] sii9234: RSEN interrupt without MHL_EST_INT\n");
			goto err_exit2;
		}
...
#if 0 /*Remove lelow code for CTS 3.1.1.13*/
#ifndef CONFIG_MHL_NEW_CBUS_MSC_CMD
				cbus_command_request(sii9234, CBUS_WRITE_STAT,
						CBUS_LINK_CONTROL_2_REG,
				sii9234->mhl_status_value.linkmode);
	...
	if (sii9234->state == STATE_ESTABLISHED) {
		...
		if ((cbus_intr1 & CBUS_DDC_ABORT) ||
			...
			pr_info("sii9234: CBUS DDC abort\n");
			if (cbus_ddc_abort_error(sii9234)) {
				if (sii9234->claimed == true)
					release_otg = true;
				
				/* need to check below line !!!
				When power off needed, 
				i think mhl_poweroff should be set to 1
				sii9234_power_down(sii9234); 
				*/
				mhl_poweroff = 1;
				goto err_exit;
		...
		if (cbus_intr1 & MSC_REQ_DONE) {
			pr_info("sii9234: CBUS cmd ACK Received\n");
#ifndef CONFIG_MHL_NEW_CBUS_MSC_CMD
		...
		if (cbus_intr1 & MSC_MSG_RECD) {
			pr_info("sii9234: MSC_MSG_RECD Received\n");
 ...
 err_exit2:
	...
	if (gpio_get_value(GPIO_MHL_INT)) {
		pr_info("sii9234 interrupt is not cleared \n");

		retry_count++;		
		if(retry_count < 5) {
			goto intr_check;
		} else {
			pr_info("sii9234 too many retries \n");
			//mhl_poweroff = 1;
			sii9234_disable_irq();
			sii9234->pdata->hw_reset();
			ret = goto_d3();
...
static void sii9234_early_suspend(struct early_suspend *h) {
	...
	struct sii9234_data *sii9234 = 
		container_of(h, struct sii9234_data, suspend_desc);

	DEBUG_MHL_FUNC_START();

	if (sii9234->mhl_status_value.sink_hpd)
		sii9234_tmds_control(sii9234, false);
					      ...
					      const struct i2c_device_id *id)
{
	...
	wake_lock_init(&sii9234->wake_lock, WAKE_LOCK_SUSPEND, "mhl");
	...
	/* I9105P H/W req : swing level => 0xFF : 175mV */
	sii9234->pdata->swing_level = 0xFF;
	...
	sii9234->suspend_desc.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN+1,
	sii9234->suspend_desc.suspend = sii9234_early_suspend,
	sii9234->suspend_desc.resume = sii9234_late_resume,
	register_early_suspend(&sii9234->suspend_desc);
#endif
...
err_class:
 ...
 err_exit1:
	wake_lock_destroy(&sii9234->wake_lock); 	
#ifdef CONFIG_SII9234_RCP
...
static int __devexit sii9234_mhl_tx_remove(struct i2c_client *client)
{
	...
	unregister_early_suspend(&sii9234->suspend_desc);
