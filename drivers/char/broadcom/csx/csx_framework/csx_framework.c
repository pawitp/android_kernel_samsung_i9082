...
static char banner[] __initdata =
    KERN_INFO "CSX: 1.00 (built on " __DATE__ " " __TIME__ ")\n";
			...
			CSX_MODULE_FNCS *csx_module_fncp)
{
	...
	if (csx_module_fncp != NULL &&
		...
		memcpy(&gModuleFncp[csx_module_id], csx_module_fncp,
		       sizeof(CSX_MODULE_FNCS));
		      ...
		      CSX_UTIL_CTRL_FNCS *csx_util_ctrl_fncp)
{
	...
	if (csx_util_fncp != NULL &&
		...
		memcpy(&gUtilFncp[csx_util_id], csx_util_fncp,
		       sizeof(CSX_UTIL_FNCS));
...
static int __init csx_init(void)
{
	...
	memset(&gModuleFncp, 0,
	       sizeof(CSX_MODULE_FNCS) * CSX_IO_MODULE_NUM_MAX);
