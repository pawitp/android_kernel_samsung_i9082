...
static int csx_gist_inject(char *bufp, int bytes, void *csx_priv)
{
	...
	CSX_IO_MODULE module_id =
	    CSX_GIST_HANDLE_GET_MODULE_ID((CSX_IO_HANDLE)csx_priv);
	int sync_io_point =
	    CSX_GIST_HANDLE_GET_SYNC_FLAG((CSX_IO_HANDLE)csx_priv);
	int index;
	int inject_gist_handle;
	int allow_inject = 0;
	int rc;
	...
	if (CSX_GIST_HANDLE_GET_MAGIC_NUM((CSX_IO_HANDLE)csx_priv) !=
	    CSX_GIST_UTIL_MAGIC_NUM) {
	...
	if (allow_inject) {
		...
		if (index >= 0 && index < CSX_GIST_IO_POINT_NUM_MAX) {
			...
			memcpy(&point_info, &gPointInfoList[index],
			       sizeof(CSX_GIST_IO_POINT_INFO));
			if (point_info.io_point_active == CSX_POINT_ENABLE) {
				inject_gist_handle =
				    point_info.inject_gist_handle;

				if (inject_gist_handle != GIST_INVALID_HANDLE) {
					valid = 1;
					rc = gist_read(inject_gist_handle,
						       bytes, bufp);
					return rc;
				} else {
...
static int csx_gist_capture(char *bufp, int bytes, void *csx_priv)
{
	...
	if (allow_capture) {
		...
		if (index >= 0 && index < CSX_GIST_IO_POINT_NUM_MAX) {
			...
			if (point_info.io_point_active == CSX_POINT_ENABLE) {
				capture_gist_handle =
				    point_info.capture_gist_handle;

				if (capture_gist_handle != GIST_INVALID_HANDLE) {
					valid = 1;
					rc = gist_write(capture_gist_handle,
							bytes, bufp);
					return rc;
				} else {
					/* GIST handle is invalid */
					 ...
					 CSX_GIST_INFO *pGistInfo)
{
	...
	case CSX_IO_MODE_CAPTURE:
		{
			capture_gist_handle =
			    gist_alloc_writer(&pGistInfo->capture);

			if (capture_gist_handle != GIST_INVALID_HANDLE) {
				pInfo->capture_gist_handle =
				    capture_gist_handle;
			} else {
				rc = -EINVAL;
			}
	...
	case CSX_IO_MODE_INJECT:
		{
			inject_gist_handle =
			    gist_alloc_reader(&pGistInfo->inject);
	...
	case CSX_IO_MODE_CAPTURE_INJECT:
		{
			...
			if (capture_gist_handle != GIST_INVALID_HANDLE
			    && inject_gist_handle != GIST_INVALID_HANDLE) {
				pInfo->capture_gist_handle =
				    capture_gist_handle;
				pInfo->inject_gist_handle = inject_gist_handle;
			} else {
...
static CSX_IO_HANDLE csx_gist_generate_io_point_handle(int index)
{
	...
	handle |=
	    ((CSX_GIST_UTIL_MAGIC_NUM & CSX_GIST_HANDLE_MAGIC_NUM_MASK) <<
	     CSX_GIST_HANDLE_MAGIC_NUM_OFFSET);

	/* Add CSX GIST unique ID based on number of new added points in system */
	handle |=
	    ((gNumPointsAdded & CSX_GIST_HANDLE_UNIQUE_ID_MASK) <<
	     CSX_GIST_HANDLE_UNIQUE_ID_OFFSET);

	/* Add sync bit */
	handle |=
	    ((pInfo->csx_io_point_info.csx_io_sync & CSX_GIST_HANDLE_SYNC_FLAG_MASK) <<
	     CSX_GIST_HANDLE_SYNC_FLAG_OFFSET);

	/* Place CSX module ID */
	handle |=
	    ((pInfo->csx_io_point_info.csx_module_id & CSX_GIST_HANDLE_MODULE_ID_MASK) <<
	     CSX_GIST_HANDLE_MODULE_ID_OFFSET);

	/* Place index in list */
	handle |=
	    ((index & CSX_GIST_HANDLE_INDEX_MASK) <<
	     CSX_GIST_HANDLE_INDEX_OFFSET);
...
static int csx_gist_cleanup_point(int index)
{
	...
	if (gPointInfoList[index].io_point_active == CSX_POINT_ENABLE) {
		...
		if (csx_mod_ops->csx_module_set_point != NULL) {
			err =
			    csx_mod_ops->csx_module_set_point(csx_info->
							      csx_device_id,
		       ...
		       CSX_GIST_INFO *csx_gist_info, CSX_IO_HANDLE *csx_handle)
{
	...
	for (i = 0; i < CSX_GIST_IO_POINT_NUM_MAX; i++) {
		if (gPointInfoList[i].io_point_active == CSX_POINT_ENABLE) {
			CSX_IO_POINT_INFO *pInfo =
			    &gPointInfoList[i].csx_io_point_info;
			...
			if (pInfo->csx_module_id == csx_info->csx_module_id &&
				...
				up(&csx_gist_sem);
				printk(KERN_ERR
				       "%s: I/O point specified already exists, must remove prior to adding\n",
				       __FUNCTION__);
	...
	if (first_inactive_index < 0) {
		...
		up(&csx_gist_sem);
		printk(KERN_ERR
		       "%s: Cannot exceed maximum number of I/O points (%d)\n",
	...
	memcpy(&gPointInfoList[first_inactive_index].csx_io_point_info,
	       csx_info, sizeof(CSX_IO_POINT_INFO));

	/* Generate GIST handle and store */
	err =
	    csx_gist_allocate_gist_handle(&gPointInfoList[first_inactive_index],
					  csx_gist_info);
	...
	if (csx_mod_ops->csx_module_set_point != NULL) {
		err =
		    csx_mod_ops->csx_module_set_point(csx_info->csx_device_id,
						      csx_info->csx_point_id,
						      &csx_ops, (void *)handle);
	} else {
		up(&csx_gist_sem);
		printk(KERN_ERR
		       "%s: CSX Module selected for I/O point not registered\n",
		       __FUNCTION__);
	...
	if (!err) {
		...
		gPointInfoList[first_inactive_index].csx_io_point_handle =
		    handle;

		/* Activate I/O point on the list */
		gPointInfoList[first_inactive_index].io_point_active =
		    CSX_POINT_ENABLE;
		...
		memset(&gPointInfoList[first_inactive_index], 0,
		       sizeof(CSX_GIST_IO_POINT_INFO));
		up(&csx_gist_sem);
		*csx_handle = CSX_HANDLE_INVALID;
		return err;
...
int csx_gist_query_all(CSX_GIST_POINT *csx_gist_point, unsigned int *num_points)
{
	...
	for (i = 0; i < CSX_GIST_IO_POINT_NUM_MAX; i++) {
		if (gPointInfoList[i].io_point_active == CSX_POINT_ENABLE) {
			memcpy(&csx_gist_point[j].csx_io_point_info,
			       &gPointInfoList[i].csx_io_point_info,
			       sizeof(CSX_IO_POINT_INFO));
			memcpy(&csx_gist_point[j].csx_io_point_handle,
			       &gPointInfoList[i].csx_io_point_handle,
			       sizeof(CSX_IO_HANDLE));

			if (gPointInfoList[i].capture_gist_handle !=
			    GIST_INVALID_HANDLE) {
				gist_get_writer_params(gPointInfoList[i].
						       capture_gist_handle,
						       &csx_gist_point[j].csx_gist_info.capture);
			} else {
				memset(&csx_gist_point[j].csx_gist_info.capture,
				       0, sizeof(GIST_WRITER_PARAMS));
			}

			if (gPointInfoList[i].inject_gist_handle !=
			    GIST_INVALID_HANDLE) {
				gist_get_reader_params(gPointInfoList[i].
						       inject_gist_handle,
						       &csx_gist_point[j].csx_gist_info.inject);
			} else {
				memset(&csx_gist_point[j].csx_gist_info.inject,
				       0, sizeof(GIST_READER_PARAMS));
			}
...
static int __init csx_gist_init(void)
{
	...
	memset(&gPointInfoList, 0,
	       (sizeof(CSX_GIST_IO_POINT_INFO) * CSX_GIST_IO_POINT_NUM_MAX));
