...
static char banner[] __initdata =
    KERN_INFO "CSX HAL Audio: 1.00 (built on " __DATE__ " " __TIME__ ")\n";
				  ...
				  void *csx_priv)
{
	...
	if (csx_point_id != HALAUDIO_DIR_ADC
	    && csx_point_id != HALAUDIO_DIR_DAC) {
		return -EINVAL;
	}

	err =
	    down_timeout(&csx_halaudio_sem,
			 CSX_HALAUDIO_SEMAPHORE_TIME_WAIT_JIFFIES);
	if (!err) {
		err =
		    halAudioSetCsxIoPoints(csx_device_id, csx_point_id,
					   csx_io_point_fncs, csx_priv);
		up(&csx_halaudio_sem);
	}
...
static int csx_halaudio_set_frame_sync(CSX_FRAME_SYNC_FP frame_sync_fncp)
{
	int err;
	err =
	    halAudioSetCsxFrameSync(frame_sync_fncp,
				    (void *)CSX_IO_MODULE_HALAUDIO);
	return err;
}
...
static int __init csx_halaudio_init(void)
{
	...
	if (IS_ERR(gHalHdl)) {
		printk(KERN_ERR
		       "%s: failed to allocate HAL Audio client err=%li\n",
