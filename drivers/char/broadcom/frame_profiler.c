...
static int readProcHistory(char *buf, char **start, off_t offset, int count,
			   int *eof, void *data)
{
	...
	for (i = 0; i < HISTORY_SIZE; i++) {
		len +=
		    sprintf(buf + len, "%15u %22u %22u \n",
			    instance->profilerData.history[i],
			    microSec(instance->profilerData.
				     start[HISTORY_SIZE - 1 - i]),
			    microSec(instance->profilerData.
				     end[HISTORY_SIZE - 1 - i]));
	}
...
static int readProcStats(char *buf, char **start, off_t offset, int count,
			 int *eof, void *data)
{
	PROFILER *instance = (PROFILER *) data;
	int len = 0;
...
int readProcEnable(char *buf, char **start, off_t offset, int count, int *eof,
		   void *data)
{
	PROFILER *instance = (PROFILER *) data;
	int len = 0;
...
int readProcAlphafactor(char *buf, char **start, off_t offset, int count,
			int *eof, void *data)
{
	PROFILER *instance = (PROFILER *) data;
	int len = 0;
	...
	len =
	    sprintf(buf + len, "Alpha-factor: %d \n",
...
int writeProcAlphafactor(struct file *file, const char *buf, int count,
			 void *data)
{
	PROFILER *instance = (PROFILER *) data;
	char input[5];
...
int create_profiler_proc(PROFILER * instance)
{
	...
	if (instance->procDir == NULL) {
		printk(KERN_ERR
		       "%s: Failed to create main directory for proc entries\n",
		       __FUNCTION__);
		return 1;
	}

	create_proc_read_entry("history", 0, instance->procDir, readProcHistory,
			       (void *)instance);
	create_proc_read_entry("stats", 0, instance->procDir, readProcStats,
			       (void *)instance);

   /********** Creating Enable Proc **********/
	instance->procEnable =
	    create_proc_entry("enable", 0644, instance->procDir);
	if (instance->procEnable == NULL) {
		printk(KERN_ERR
		       "%s: Failed to create Enable directory for proc entries\n",
		       __FUNCTION__);
		return 1;
	}
   ...
   /********** Creating Reset Proc **********/
	instance->procReset =
	    create_proc_entry("reset", 0644, instance->procDir);
	if (instance->procReset == NULL) {
		printk(KERN_ERR
		       "%s: Failed to create Reset directory for proc entries\n",
		       __FUNCTION__);
		return 1;
	}
   ...
   /********** Creating Alphafactor Proc **********/
	instance->procAlphafactor =
	    create_proc_entry("alpha-factor", 0644, instance->procDir);
	if (instance->procAlphafactor == NULL) {
		printk(KERN_ERR
		       "%s: Failed to create Alphafactor directory for proc entries\n",
		       __FUNCTION__);
		return 1;
	}
	instance->procAlphafactor->data = instance;
	instance->procAlphafactor->write_proc =
	    (write_proc_t *) writeProcAlphafactor;
	instance->procAlphafactor->read_proc =
	    (read_proc_t *) readProcAlphafactor;
...
FRAME_PROFILER_HANDLE getFrameProfiler(void)
{
	...
	if (!gInitializeProfilerArray) {
		...
		if (rc) {
			printk(KERN_ERR
			       "Failed to create profiler thread semaphore\n");
			return 0;
		}
	}

	gosSemTake(FP_semWrite);
	for (profiler_ptr = gProfilerArray;
	     profiler_ptr < gProfilerArray + MAX_PROFILERS; profiler_ptr++) {
...
FRAME_PROFILER_HANDLE frame_profiler_init(char proc_name[])
{
	...
	if ((instance->profilerTask == NULL) || IS_ERR(instance->profilerTask)) {
		instance->profilerTask =
		    kthread_run(profilerThread, instance, instance->procName);

		if (IS_ERR(instance->profilerTask)) {
			printk(KERN_ERR
			       "Failed to start profiler thread: %ld\n",
...
{
	instance->profilerState.avgSum +=
	    instance->profilerData.history[1] -
	    instance->profilerData.history[AVG_RANGE];

	if ((index - AVG_RANGE + 1) < 0) {
		return ((instance->profilerState.avgSum * 10000) /
			(microSec(instance->profilerData.start[index] -
			  instance->profilerData.start[index - AVG_RANGE + 1 +
						       HISTORY_SIZE]) + 1));
	} else {
		return ((instance->profilerState.avgSum * 10000) /
			(microSec(instance->profilerData.start[index] -
...
unsigned int weightedAvg(PROFILER * instance)
{
	if (!instance->profilerStats.avgWeightedTime) {
		...
		return ((instance->profilerStats.currTime *
			...
			(instance->profilerStats.avgWeightedTime *
			 ((1 << 8) -
			  instance->profilerState.alphaFactor))) >> 8;
	}
}
