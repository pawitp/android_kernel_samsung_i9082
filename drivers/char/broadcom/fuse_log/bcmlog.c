...
static int __init setup_mtt_logbuffer(char *str)
{
	...
	if((g_module.buffer_size <= 0) || (g_module.buffer_size > BCMLOG_OUTPUT_FIFO_MAX_BYTES)){
		g_module.buffer_size = KMALLOC_MAX_SIZE;
	}
	printk("Allocating mtt log buffer size : %d Bytes\n",g_module.buffer_size);
	g_module.buffer = alloc_bootmem_pages(g_module.buffer_size);
	if(!g_module.buffer){
		printk("Failed to allocate mtt log buffer\n");
		return -1;
	}	else {
		printk("[allocated mtt log buffer] base : 0x%x, Size : %d Bytes\n",(unsigned int)g_module.buffer,g_module.buffer_size);		
	}
	return 0;
}
...
static int __init BCMLOG_ModuleInit(void)
{
	...
	printk("[%s]g_module.buffer=0x%x, g_module.buffer_size=%d\n",__func__,(unsigned int)g_module.buffer,g_module.buffer_size);
	if (!g_module.buffer)
	{
		printk("[%s] Forced to allocate 4MB mtt log buffer\n",__func__);
		g_module.buffer_size = KMALLOC_MAX_SIZE;
...
void display_crash_dump_start_ui(const char *mode)
{
	...
	dump_start_ui_sub_info = call_usermodehelper_setup(argv[0], argv, envp,
							   GFP_ATOMIC);

	if (dump_start_ui_sub_info)
		call_usermodehelper_exec(dump_start_ui_sub_info, UMH_WAIT_EXEC);
}
#endif
