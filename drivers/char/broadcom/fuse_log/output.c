		...
		do {
			...
			if (nFifo > 0) {
				...
				if ((nWrite < nFifo)
					...
					g_devWrParms.file = 0;
					/*
					 * in the case of sdcard,
					 * redirect output
					 * to 'null' as the card
					 * is full or removed at
					 * this point
					 */
					g_devWrParms.outdev =
					    BCMLOG_OUTDEV_NONE;
...
{
	/*
	 *      Add to FIFO.  If unable to add (FIFO full) then discard the
...
#endif
	/*
	 *      If output worker thread is not busy then set up
	 *    arguments and schedule it; otherwise
