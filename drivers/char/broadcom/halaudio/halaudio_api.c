...
static int halaudio_read_proc(char *buf, char **start, off_t offset, int count,
			      int *eof, void *data);
...
static ssize_t halaudio_read(struct file *file, char __user *buffer,
			     size_t count, loff_t *ppos);
static ssize_t halaudio_write(struct file *file, const char __user *buffer,
			      size_t count, loff_t *ppos);
static long halaudio_ioctl(struct file *file, unsigned int cmd,
			   unsigned long arg);
...
*/
static inline struct halaudio_client *halaudio_get_clientp(HALAUDIO_HDL client_hdl
				    /**< (i) client handle */
    )
{
...
*     -ve   - error code
*/
static int pmSuspend(struct platform_device *pdev,
				    /**< (in) pointer to platform device */
		     pm_message_t state
				    /**< (in) power management message */
    )
{
	if (gPMOps.suspend) {
...
*     -ve      Failure code
*/
int halAudioFreeClient(HALAUDIO_HDL client_hdl
				    /**< (i) Client handle */
    )
{
	if (gApi.freeClient) {
...
*     -ve      Failure code
*/
int halAudioQueryInterfaceByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
				 const char *name,
				    /**< (i) Name of interface */
				 HALAUDIO_IF * id
...
*     -ve      Failure code
*/
int halAudioQueryCodecByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
			     const char *name,
...
*/
int halAudioLockCodec(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
		      HALAUDIO_CODEC cid
				    /**< (i) Codec to lock */
    )
...
*/
int halAudioUnlockCodec(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
			HALAUDIO_CODEC cid
				    /**< (i) Codec ID to unlock */
    )
{
	if (gApi.unlockCodec) {
...
*/
int halAudioSetPower(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
		     HALAUDIO_POWER_LEVEL level
				    /**< (i) Power level request */
    )
{
	...
	if (gApi.setPower) {
		...
		if (!err) {
			struct halaudio_client *clientp =
			    halaudio_get_clientp(client_hdl);
			if (clientp) {
...
*     -ve      Error code
*/
int halAudioWrite(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
		  HALAUDIO_CODEC cid,
				    /**< (i) Codec channel to write to */
		  HALAUDIO_FMT format,
				    /**< (i) Format of samples */
		  const uint8_t *audio,
...
*     -ve      Error code
*/
int halAudioRead(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
...
*     frequency.
*/
int halAudioSetFreq(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
		    HALAUDIO_CODEC cid,
				    /**< (i) Codec channel to write to */
...
*     -EPERM   Parent interface is disabled
*/
int halAudioGetFreq(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
		    HALAUDIO_CODEC cid,
				    /**< (i) Codec channel to write to */
...
*     -EPERM   Parent interface is disabled
*/
int halAudioSetEquParms(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
			HALAUDIO_CODEC cid,
				    /**< (i) Codec to write to */
			HALAUDIO_DIR dir,
...
*     -EINVAL  Invalid parameters
*/
int halAudioGetInterfaceInfo(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
			     HALAUDIO_IF id,
				    /**< (i) Interface id */
...
*     -EPERM   Parent interface is disabled
*/
int halAudioGetCodecInfo(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
			 HALAUDIO_CODEC cid,
				    /**< (i) Codec channel to write to */
...
*     -EINVAL  Invalid parameters
*/
int halAudioEnableInterface(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
			    HALAUDIO_IF id,
				    /**< (i) Interface number */
			    int enable
...
*  @return    none
*/
void halAudioSetApiFuncs(const HALAUDIO_API_FUNCS * funcsp
    ...
    )
{
	if (gApi.addInterface) {
		return gApi.addInterface(ops, codecs, name, frame_usec, sync,
					 hdlp);
	}
	return -EPERM;
}
...
*     -ve      Other error codes
*/
int halAudioSetCsxFrameSync(CSX_FRAME_SYNC_FP fncp,
			      ...
			      int *eof, void *data)
{
	...
	if (!err) {
		...
		/* Dump interface info */
		len +=
		    sprintf(buf + len, "Interfaces: total=%i\n",
			    hwinfo.interfaces);
		for (i = 0, id = 0;
		     i < hwinfo.interfaces && id < HALAUDIO_MAX_INTERFACES;
		     i++) {
			/* Scan ID */
			do {
				err =
				    halAudioGetInterfaceInfo(NULL, id, &ifinfo);
				if (!err) {
					len +=
					    sprintf(buf + len,
						    " %s: id=%i codecs=%i frame_period=%i enabled=%i sync=%i\n",
						    ifinfo.name, id,
						    ifinfo.codecs,
						    ifinfo.frame_period_us,
						    ifinfo.enabled,
						    ifinfo.sync);
				}
				id++;
		...
		for (i = 0, id = 0;
		     i < hwinfo.codecs && id < HALAUDIO_MAX_CODECS; i++) {
			/* Scan ID */
			do {
				err =
				    halAudioGetCodecInfo(NULL, id, &codecinfo);
				if (!err) {
					/* Get parent interface info */
					halAudioGetInterfaceInfo(NULL,
								 codecinfo.
								 parent_id,
								 &ifinfo);
	...
	list_for_each_entry(clientp, &gInfo.client_list, lnode) {
		len +=
		    sprintf(buf + len,
			    " 0x%lx: superuser=%i powerlevel=%i rcid=%i rfmt=%i wcid=%i wfmt=%i\n",
			    (unsigned long)clientp->handle, clientp->superuser,
			    clientp->powerlevel, clientp->read.cid,
			    clientp->read.format, clientp->write.cid,
			    clientp->write.format);
	}
	up(&gInfo.client_lock);
...
static void halaudio_debug_init(void)
{
	create_proc_read_entry(HALAUDIO_PROC_NAME, 0, NULL, halaudio_read_proc,
			       NULL);
...
*/
static int halaudio_open(struct inode *inode,
				    /**< (io) Pointer to inode info */
			 struct file *file
...
*     -ve      Error code
*/
static int halaudio_release(struct inode *inode,
				    /**< (io) Pointer to inode info */
			    struct file *file
				    /**< (io) File structure pointer */
    )
...
*     -ve            Error code
*/
static ssize_t halaudio_read(struct file *file,
...
*/
static void uncompress(int16_t *outbuf,
				    /**< (o) Ptr to 16-bit uncompressed output */
		       const uint8_t *inbuf,
				    /**< (i) Ptr to 8-bit compressed input */
		       const int16_t *table,
				    /**< (i) Ptr to table lookup */
...
#error No endian defined.
	...
	if (format == HALAUDIO_FMT_ULAW) {
		uncompress((int16_t *)(void *)kbuf, kbuf, ulawExpandTable,
			   count);
...
*     -ve            Error code
*/
static long halaudio_ioctl(struct file *file,
				    /**< (io) File structure pointer */
			   unsigned int cmd,
				    /**< (i)  IOCTL command */
			   unsigned long arg
    ...
    )
{
	...
	case HALAUDIO_CMD_LOCK_CODEC:
		{
			rc = halAudioLockCodec(client_hdl,
					       (HALAUDIO_CODEC) arg);
		}
		break;
	...
	case HALAUDIO_CMD_UNLOCK_CODEC:
		{
			rc = halAudioUnlockCodec(client_hdl,
						 (HALAUDIO_CODEC) arg);
		}
		break;
	...
	case HALAUDIO_CMD_GET_GAIN_INFO:
		{
			HALAUDIO_GAIN_INFO info;
			rc = halAudioGetGainInfo(client_hdl, parm.getgain.block,
						 &info);
			if (rc == 0) {
				rc = copy_to_user(parm.getgain.info, &info,
						  sizeof(info));
			}
		}
		break;
	...
	case HALAUDIO_CMD_SET_GAIN:
		{
			rc = halAudioSetGain(client_hdl, parm.setgain.block,
					     parm.setgain.db);
		}
		break;
	...
	case HALAUDIO_CMD_ALTER_GAIN:
		{
			rc = halAudioAlterGain(client_hdl, parm.altergain.block,
					       parm.altergain.delta);
		}
		break;

	case HALAUDIO_CMD_SET_POWER:
		{
			rc = halAudioSetPower(client_hdl,
					      (HALAUDIO_POWER_LEVEL) arg);
		}
		break;
	...
	case HALAUDIO_CMD_GET_POWER:
		{
			...
			if (rc == 0) {
				rc = copy_to_user((void *)arg, &level,
						  sizeof(level));
			}
		}
		break;
	...
	case HALAUDIO_CMD_SET_FREQ:
		{
			rc = halAudioSetFreq(client_hdl, parm.setfreq.cid,
					     parm.setfreq.freqhz);
		}
		break;
	...
	case HALAUDIO_CMD_GET_FREQ:
		{
			int freqhz;
			rc = halAudioGetFreq(client_hdl, parm.getfreq.cid,
					     &freqhz);
			if (rc == 0) {
				rc = copy_to_user(parm.getfreq.freqhz, &freqhz,
						  sizeof(freqhz));
			}
		}
		break;
	...
	case HALAUDIO_CMD_SET_EQU_PARMS:
		{
			down(&ioctl_equ_lock);
			if ((rc =
			     copy_from_user(&ioctl_equ, parm.setequ.equ,
					    sizeof(ioctl_equ))) != 0) {
				up(&ioctl_equ_lock);
				return rc;
			}
			rc = halAudioSetEquParms(client_hdl, parm.setequ.cid,
						 parm.setequ.dir, &ioctl_equ);
			up(&ioctl_equ_lock);
		}
		break;
	...
	case HALAUDIO_CMD_GET_EQU_PARMS:
		{
			down(&ioctl_equ_lock);
			rc = halAudioGetEquParms(client_hdl, parm.getequ.cid,
						 parm.getequ.dir, &ioctl_equ);
			if (rc == 0) {
				rc = copy_to_user(parm.getequ.equ, &ioctl_equ,
						  sizeof(ioctl_equ));
			}
			up(&ioctl_equ_lock);
		}
	...
	case HALAUDIO_CMD_GET_INTERFACE_INFO:
		{
			HALAUDIO_IF_INFO info;
			rc = halAudioGetInterfaceInfo(client_hdl,
						      parm.ifinfo.interface,
						      &info);
			if (rc == 0) {
				rc = copy_to_user(parm.ifinfo.info, &info,
						  sizeof(info));
			}
		}
		break;
	...
	case HALAUDIO_CMD_GET_CODEC_INFO:
		{
			HALAUDIO_CODEC_INFO info;
			rc = halAudioGetCodecInfo(client_hdl,
						  parm.codecinfo.cid, &info);
			if (rc == 0) {
				rc = copy_to_user(parm.codecinfo.info, &info,
						  sizeof(info));
			}
		}
		break;
	...
	case HALAUDIO_CMD_ENABLE_INTERFACE:
		{
			rc = halAudioEnableInterface(client_hdl,
						     parm.enableif.interface,
						     parm.enableif.enable);
		}
		break;
	...
	case HALAUDIO_CMD_QUERY_INTERFACE_ID_BY_NAME:
		{
			HALAUDIO_IF id;
			rc = halAudioQueryInterfaceByName(client_hdl,
							  parm.interfid.name,
							  &id);
			if (rc == 0) {
				rc = copy_to_user(parm.interfid.id, &id,
						  sizeof(id));
			}
		}
		break;
	...
	case HALAUDIO_CMD_QUERY_CODEC_ID_BY_NAME:
		{
			HALAUDIO_CODEC id;
			rc = halAudioQueryCodecByName(client_hdl,
						      parm.codecid.name, &id);
			if (rc == 0) {
				rc = copy_to_user(parm.codecid.id, &id,
						  sizeof(id));
			}
		}
		break;
...
#endif
{
	...
	if (gDriverMajor < 0) {
		printk(KERN_ERR
		       "HALAUDIO: Failed to register character device major\n");
		err = -EFAULT;
		goto error_cleanup;
	}
#ifdef CONFIG_SYSFS
	...
	halaudio_dev =
	    device_create(halaudio_class, NULL, MKDEV(gDriverMajor, 0), NULL,
			  "halaudio");
