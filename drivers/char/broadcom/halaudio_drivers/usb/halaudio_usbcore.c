...
static int usbCoreInfoReadProc(char *buf, char **start, off_t offset, int count,
			       int *eof, void *data);
static int usbCoreInterfReadProc(char *buf, char **start, off_t offset,
				 int count, int *eof, void *data);
static int usbCoreFuReadProc(char *buf, char **start, off_t offset, int count,
			     int *eof, void *data);

static int usbCoreProbe(struct usb_interface *intf,
...
static int setFeature(uint8_t feature, uint8_t request, uint8_t unit,
		      uint8_t channel, int val);
static int getFeature(uint8_t feature, uint8_t request, uint8_t unit,
		      uint8_t channel);
			...
			const struct usb_device_id *id)
{
	...
	if (gUsbCore.pDev) {
		printk(KERN_INFO
		       "USB Core: Did not claim USB device since one already exists.\n");
		return -ENODEV;
	}
	...
	if (!pAlts->extralen) {
		printk(KERN_ERR
		       "USB Core: No AC interface descriptors found.\n");
		return -ENODEV;
	}
	...
	if (pCsAcIntHeaderDesc->bDescriptorType != USB_DT_CS_INTERFACE &&
		...
		printk(KERN_ERR
		       "USB Core: HEADER AC interface descriptor found.\n");
		return -ENODEV;
	}
	...
	for (i = 0; i < pCsAcIntHeaderDesc->bInCollection; i++) {
		rc = processAudioStream(gUsbCore.pDev,
					*(pCsAcIntHeaderDesc->baInterfaceNr +
					  i));
		if (rc) {
			printk(KERN_ERR
			       "USB Core [Interf=%u]: Failed to processAudioStream rc=%i.\n",
	...
	if (rc) {
		 ...
		 */
		printk(KERN_INFO
		       "USB Core: Failed to initialize IN interface\n");
	}
	...
	if (gCbs.infoIn
	    && (sampFreq != gUsbCore.inIntf.sampFreq
		|| numberChannels != gUsbCore.inIntf.numberChannels)) {
		gCbs.infoIn(gUsbCore.inIntf.sampFreq,
			    gUsbCore.inIntf.numberChannels);
	}
	...
	if (rc) {
		 ...
		 */
		printk(KERN_INFO
		       "USB Core: Failed to initialize OUT interface\n");
	}
	...
	if (gCbs.infoOut
	    && (sampFreq != gUsbCore.outIntf.sampFreq
		|| numberChannels != gUsbCore.outIntf.numberChannels)) {
		gCbs.infoOut(gUsbCore.outIntf.sampFreq,
			     gUsbCore.outIntf.numberChannels);
	}
	...
	if (atomic_read(&gUsbCore.enable)) {
		...
		if (pIf->number != -1) {
			 ...
			 */
			usb_set_interface(gUsbCore.pDev, pIf->number,
					  pIf->pAlt[pIf->activeAlt].pAlts->desc.
					  bAlternateSetting);
			...
			if (rc) {
				 ...
				 */
				printk(KERN_INFO
				       "USB Core: Failed to start URBs in IN interface\n");
			}
		}
		...
		if (pIf->number != -1) {
			...
			if (rc) {
				 ...
				 */
				printk(KERN_INFO
				       "USB Core: Failed to start URBS in OUT interface\n");
			}
		}
	}
...
probe_cleanup:
	...
	if (gUsbCore.inIntf.number != -1) {
		usb_driver_release_interface(&usbCoreDriver,
					     usb_ifnum_to_if(gUsbCore.pDev,
...
int usbCorePrepare(void)
{
	...
	if (pIf->number != -1) {
		 ...
		 */
		usb_set_interface(gUsbCore.pDev, pIf->number,
				  pIf->pAlt[pIf->activeAlt].pAlts->desc.bAlternateSetting);
	}
...
static void usbCoreEgressHandler(struct urb *urb)
{
	...
	if (urb->status) {
		USB_CORE_LOG("USB Core: Egress URB error status=%i",
			     urb->status);
	}
...
static void usbCoreIngressHandler(struct urb *urb)
{
	...
	USB_CORE_PROFILING("in  [usb]");
	if (urb->status) {
		USB_CORE_LOG("USB Core: Ingress URB error status=%i",
			     urb->status);
	}
#if defined( __BIG_ENDIAN )
	{
...
*/
int usbCoreSetIngressFreq(int freqHz/**< (i) sampling frequency in Hz */
    )
{
	int rc = 0;
	...
	if (down_interruptible(&gUsbCore.mutex)) {
		printk(KERN_ERR
		       "USB Core [Interf=%u]: Could not obtain mutex\n",
		       gUsbCore.inIntf.number);
		return -ENODEV;
	}
	 ...
	 */
	if (isSupported
	    (&gUsbCore.inIntf, freqHz, gUsbCore.inIntf.numberChannels) >= 0) {
		rc = initInterface(&gUsbCore.inIntf, freqHz,
				   gUsbCore.inIntf.numberChannels);
		if (rc) {
			printk(KERN_ERR
			       "USB Core [Interf=%u]: Could not set sampling frequency of USB_DIR_IN rc=%i\n",
...
*     -EINVAL  Invalid channel number
*/
int usbCoreSetEgressFreq(int freqHz /**< (i) sampling frequency in Hz */
    )
{
	int rc = 0;
	 ...
	 */
	if (isSupported
	    (&gUsbCore.outIntf, freqHz, gUsbCore.outIntf.numberChannels) >= 0) {
		rc = initInterface(&gUsbCore.outIntf, freqHz,
				   gUsbCore.outIntf.numberChannels);
		if (rc) {
			printk(KERN_ERR
			       "USB Core [Interf=%u]: Could not set sampling frequency of USB_DIR_OUT rc=%i\n",
...
int setVolume(struct usb_core_intf *pIf, int vol)
{
		...
		if (pF->volInfo.cur == 0x8000) {
			...
			for (i = 0; i < 8; i++) {
				if (pF->channels[USB_FEATURE_MUTE] & (1 << i)) {
					rc = setFeature(USB_FEATURE_MUTE,
							SET_CUR, pF->id, i, 0);
					if (rc < 0) {
						printk(KERN_ERR
						       "USB Core: Could not un-mute ch=%u rc=%i\n",
						       i, rc);
						return -EIO;
					}
				}
	...
	for (i = 0; i < pF->numChannels; i++) {
		if (pF->channels[feature] & (1 << i)) {
			...
			if (rc < 0) {
				printk(KERN_ERR
				       "USB Core: Could not set ingress volume ch=%u rc=%i\n",
				       i, rc);
				return -EIO;
			}
		}
...
static int getVolume(struct usb_core_intf *pIf)
{
	...
	pIf->fu.volInfo.cur =
	    getFeature(USB_FEATURE_VOLUME, GET_CUR, pF->id, minchn);
	pIf->fu.volInfo.min =
	    getFeature(USB_FEATURE_VOLUME, GET_MIN, pF->id, minchn);
	pIf->fu.volInfo.max =
	    getFeature(USB_FEATURE_VOLUME, GET_MAX, pF->id, minchn);
	pIf->fu.volInfo.res =
	    getFeature(USB_FEATURE_VOLUME, GET_RES, pF->id, minchn);
		      ...
		      uint8_t channel, int val)
{
	...
	if (!pBuf) {
		printk(KERN_ERR
		       "USB Core [feature=%u][ch=%u]: Could not allocate mem for setFeature.\n",
		       feature, channel);
		return -ENOMEM;
	}
	...
	rc = usb_control_msg(pDev,
			     (PIPE_CONTROL << 30) | (0 << 15) | (pDev->
								 devnum << 8),
			     request,
			     USB_RECIP_INTERFACE | USB_TYPE_CLASS | USB_DIR_OUT,
			     (uint16_t)feature << 8 | channel,
			     (uint16_t)unit << 8 | 0, pBuf, len, 100);
		      ...
		      uint8_t channel)
{
	...
	if (!pBuf) {
		printk(KERN_ERR
		       "USB Core [feature=%u][ch=%u]: Could not allocate mem for getFeature.\n",
		       feature, channel);
		return -ENOMEM;
	}
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "USB Core [feature=%u][ch=%u]: Could not get egress feature rc=%i\n",
		       feature, channel, rc);
	}
...
static int processAudioStream(struct usb_device *pDev, int intNum)
{
	...
	if (usb_interface_claimed(pIntf)) {
		printk(KERN_INFO
		       "USB Core: [Interf=%u] Interface already claimed.\n",
		       intNum);
		return -ENODEV;
	}
	...
	for (j = 0; j < pIntf->num_altsetting; j++) {
		...
		if (pAltsDesc->bInterfaceClass != USB_CLASS_AUDIO ||
		    pAltsDesc->bInterfaceSubClass !=
		    USB_SUBCLASS_AUDIO_STREAMING) {
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u]: InterfaceClass=%u InterfaceSubClass=%u not supported.\n",
			     intNum, j, pAltsDesc->bInterfaceClass,
			     pAltsDesc->bInterfaceSubClass);
			continue;
		}
		...
		if (pAltsDesc->bNumEndpoints != 1) {
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u]: %u endpoints. Expect 1.\n",
		...
		if (!pAlts->extralen) {
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u]: No AS interface descriptors found.\n",
			     intNum, j);
			continue;
		}
		...
		if (pCsAsIntDesc->bDescriptorType != USB_DT_CS_INTERFACE ||
			...
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u]: Could not find AS interface descriptor AS_GENERAL.\n",
			     intNum, j);
			continue;
		}
		 ...
		 */
		format =
		    (pCsAsIntDesc->wFormatTag[1] << 8) | pCsAsIntDesc->
		    wFormatTag[0];
		if (format != 0x1 /* PCM */ ) {
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u]: Unsupported format=0x%04x. Only PCM is supported.\n",
			     intNum, j, format);
			continue;
		}

		pFormatTypeIDesc =
		    (struct formatTypeIDesc *)(pAlts->extra +
					       pCsAsIntDesc->bLength);

		if (pFormatTypeIDesc->bDescriptorType != USB_DT_CS_INTERFACE ||
		    pFormatTypeIDesc->bDescriptorSubtype != FORMAT_TYPE) {
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u]: Could not find AS interface descriptor FORMAT_TYPE.\n",
			     intNum, j);
			continue;
		}
		...
		if (pFormatTypeIDesc->bFormatType != 0x1 /* FORMAT_TYPE_I */  ||
			...
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u]: Unsupported bFormatType=%u, bSubframeSize=%u, or bNrChannels=%u.\n",
			     intNum, j, pFormatTypeIDesc->bFormatType,
			     pFormatTypeIDesc->bSubframeSize,
			     pFormatTypeIDesc->bNrChannels);
			continue;
		}
		...
		if (!usb_endpoint_xfer_isoc(&pEpt->desc)) {
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u]: Not ISOC.\n",
			     intNum, j);
			continue;
		}
		...
		if (pIf->number == -1) {
			...
			rc = usb_driver_claim_interface(&usbCoreDriver, pIntf,
							(void *)-1L);
			if (rc) {
				printk(KERN_ERR
				       "USB Core [Interf=%u]: Could not claim interface\n",
				       intNum);
				return -ENODEV;
			}
		...
		if (pIf->number != intNum) {
			printk(KERN_ERR
			       "USB Core [Interf=%u][alt=%u]: Found more than one IN/OUT interface.\n",
			       intNum, j);
			return -ENODEV;
		}
		...
		if (!pNewBuf) {
			printk(KERN_ERR
			       "USB Core: [Interf=%u] Cannot allocate memory to save alternate settings.\n",
			       intNum);
			return -ENODEV;
		}

		if (pIf->pAlt) {
			memcpy(pNewBuf, pIf->pAlt,
			       pIf->numAlts * sizeof(*pIf->pAlt));
		...
		pIf->pAlt[pIf->numAlts].bEndpointAddress =
		    pEpt->desc.bEndpointAddress;
		pIf->pAlt[pIf->numAlts].wMaxPacketSize =
...
static int initInterface(struct usb_core_intf *pIf, int freqHz, int numChans)
{
	...
	if (altChoice < 0) {
		printk(KERN_ERR
		       "USB Core [Interf=%u] Failed to find usable alternate setting.\n",
		       pIf->number);
		return -EINVAL;
	}

	USB_CORE_LOG
	    ("USB Core [Interf=%u][activeAlt=%u] Selected an alternate setting.\n",
	...
	if (rc) {
		printk(KERN_ERR
		       "USB Core [Interf=%u][activeAlt=%u] Failed to set sampling frequency rc=%i.\n",
	...
	if (rc) {
		printk(KERN_ERR
		       "USB Core [Interf=%u][activeAlt=%u] Failed to initialize URB rc=%i.\n",
		       pIf->number, pIf->activeAlt, rc);
		return rc;
	}
...
static int setFreq(struct usb_core_alts *pAlts, int freqHz)
{
	...
	if (!pBuf) {
		printk(KERN_ERR
		       "USB Core: Could not allocate mem for setFreq.\n");
		return -ENOMEM;
	}
	...
	rc = usb_control_msg(pDev,
			     ...
			     pAlts->bEndpointAddress, pBuf, len, 1000);
	if (rc < 0) {
		printk(KERN_ERR
		       "USB Core [ept=0x%02x]: Could not set sampling frequency rc=%i\n",
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "USB Core [ept=0x%02x]: Could not get sampling frequency rc=%i\n",
		       pAlts->bEndpointAddress, rc);
		kfree(pBuf);
		return -EIO;
	}
	...
	if (rate == freqHz) {
		USB_CORE_LOG
		    ("USB Core [ept=0x%02x]: Successfully set sampling frequency to %u Hz\n",
		 ...
		 */
		USB_CORE_LOG
		    ("USB Core [ept=0x%02x]: Retrieved frequency %u Hz is not the same as set frequency %u Hz\n",
...
static int isSupported(struct usb_core_intf *pIf, int freqHz, int numChans)
{
	...
	for (i = 0; i < pIf->numAlts; i++) {
		...
		if (pFmtDesc->bNrChannels != numChans) {
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u][numch=%u needed=%u] Alternate not chosen due to channel descepancy.\n",
		...
		if (pFmtDesc->bSamFreqType == 0) {
			 ...
			 */
			lower =
			    pFmtDesc->tSamFreq[2] << 16 | pFmtDesc->tSamFreq[1] << 8 | pFmtDesc->tSamFreq[0];
			upper =
			    pFmtDesc->tSamFreq[5] << 16 | pFmtDesc->tSamFreq[4] << 8 | pFmtDesc->tSamFreq[3];

			if (freqHz < lower || freqHz > upper) {
				USB_CORE_LOG
				    ("USB Core [Interf=%u][alt=%u][lower=%u upper=%u needed=%u] Alternate not chosen due to sampling frequency descepancy.\n",
			...
			for (j = 0; j < pFmtDesc->bSamFreqType; j++) {
				uint32_t freq =
				    pFmtDesc->tSamFreq[3 * j + 2] << 16 | pFmtDesc->tSamFreq[3 * j + 1] << 8 | pFmtDesc->tSamFreq[3 * j];

				USB_CORE_LOG
				    ("USB Core [Interf=%u][alt=%u][supported freq=%u].\n",
				     pIf->number, i, freq);
			...
			if (j == pFmtDesc->bSamFreqType) {
				USB_CORE_LOG
				    ("USB Core [Interf=%u][alt=%u][freq needed=%u] Alternate not chosen due to sampling frequency descepancy.\n",
				     pIf->number, i, freqHz);
				continue;
			}
		}
		...
		if (pAlts->wMaxPacketSize < frameSize1ms) {
			USB_CORE_LOG
			    ("USB Core [Interf=%u][alt=%u][wMaxPacketSize=%u needed=%u] Alternate not chosen due to packet size descepancy.\n",
			     pIf->number, i, pAlts->wMaxPacketSize,
			     frameSize1ms);
			continue;
		}
...
static int initUrbs(struct usb_core_intf *pIf)
{
	...
	for (i = 0; i < USB_CORE_NUM_URBS; i++) {
		...
		pUrb->transfer_buffer_length =
		    USB_CORE_ISO_FRAMES_PER_URB * pIf->frameSize1ms;
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,34)
		pUrb->transfer_buffer =
		    usb_buffer_alloc(gUsbCore.pDev,
				     pUrb->transfer_buffer_length, GFP_KERNEL,
				     &pUrb->transfer_dma);
#else
		pUrb->transfer_buffer =
		    usb_alloc_coherent(gUsbCore.pDev,
				       pUrb->transfer_buffer_length, GFP_KERNEL,
				       &pUrb->transfer_dma);
#endif
		...
		if (usb_endpoint_dir_out(&pAlts->pAlts->endpoint[0].desc)) {
			pUrb->pipe =
			    (PIPE_ISOCHRONOUS << 30) |
			    ((pAlts->pAlts->endpoint[0].desc.
			      bEndpointAddress & 0xf) << 15) | (gUsbCore.pDev->devnum << 8);
			pUrb->complete = usbCoreEgressHandler;
		} else {
			pUrb->pipe =
			    (PIPE_ISOCHRONOUS << 30) |
			    ((pAlts->pAlts->endpoint[0].desc.bEndpointAddress & 0xf) << 15) | (gUsbCore.pDev->devnum << 8) |
			    USB_DIR_IN;
...
static int startUrbs(struct usb_core_intf *pIf)
{
	...
	if (pIf->number != -1) {
		...
		for (i = 0; i < USB_CORE_NUM_URBS; i++) {
			...
			if (rc) {
				printk(KERN_ERR
				       "USB Core: Failed to submit URB %u\n",
				       i);
				return rc;
			}
		}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,34)
			usb_buffer_free(gUsbCore.pDev,
					pUrb->transfer_buffer_length,
					pUrb->transfer_buffer,
					pUrb->transfer_dma);
#else
			usb_free_coherent(gUsbCore.pDev,
					  pUrb->transfer_buffer_length,
					  pUrb->transfer_buffer,
					  pUrb->transfer_dma);
#endif
...
static int findFeatureUnits(int8_t *pDescs, int8_t *pEndDesc)
{
	...
	while (pO != NULL) {
		...
		if (!p) {
			printk(KERN_ERR
			       "USB Core: Failed to find feature unit\n");
			return -ENODEV;
		}
		...
		while (p != NULL) {
			...
			case INPUT_TERMINAL:
				{
					printk(KERN_ERR
					       "USB Core: Failed to find feature unit. IT/OT found.\n");
					return -ENODEV;
				}
				break;
			case MIXER_UNIT:
			case SELECTOR_UNIT:
				{
					...
					if (p[4] != 1) {
						printk(KERN_ERR
						       "USB Core: Failed to find feature unit. Mixer/Selector found with more than 1 input pin.\n");
						return -ENODEV;
					}
			...
			case PROCESSING_UNIT:
				{
					...
					if (p[6] != 1) {
						printk(KERN_ERR
						       "USB Core: Failed to find feature unit. Extension/Processing found with more than 1 input pin.\n");
						return -ENODEV;
					}
			...
			default:
				{
					printk(KERN_ERR
					       "USB Core: Failed to find feature unit. Unknown descriptor %u.\n",
					       p[2]);
					return -ENODEV;
				}
			}
		...
		if (!found) {
			printk(KERN_ERR
			       "USB Core: Failed to find a feature unit connected to OT\n");
			return -ENODEV;
		}
		...
		if (((uint16_t)pO[5] << 8 | pO[4]) == 0x0101) {
			 ...
			 * microphone stream
			 */
			USB_CORE_LOG("USB Core: Found FU for IN with id=%u",
				     p[3]);
			pI = &gUsbCore.inIntf;
		} else {
			/* Likely a speaker
			 */
			USB_CORE_LOG("USB Core: Found FU for OUT with id=%u",
				     p[3]);
			pI = &gUsbCore.outIntf;
		}
		 ...
		 */
		{
			...
			if (numChans > 8) {
				printk(KERN_ERR
				       "USB Core: Cannot support so many channels in feature unit=%u\n",
				       numChans);
				return -ENODEV;
			}
...
static void createProcEntries(void)
{
	...
	if (gProcDir == NULL) {
		printk("KERN_ERR"
		       "USB Core: Failed to create directory for proc entries\n");
		return;
	}

	create_proc_read_entry("dev", 0, gProcDir, usbCoreInfoReadProc,
			       gUsbCore.pDev);
	create_proc_read_entry("fu", 0, gProcDir, usbCoreFuReadProc,
			       gUsbCore.pDev);
	...
	for (i = 0; i < gUsbCore.numInterf; i++) {
		...
		create_proc_read_entry(procName, 0, gProcDir,
				       usbCoreInterfReadProc,
				       gUsbCore.pDev->actconfig->interface[i]);
	}
			       ...
			       int *eof, void *data)
{
	...
	len +=
	    sprintf(buf + len, "      Manufacturer : %s\n", pDev->manufacturer);
	len += sprintf(buf + len, "            Serial : %s\n", pDev->serial);
	len +=
	    sprintf(buf + len, "             Speed : %s\n",
		    usbSpeed[pDev->speed]);
	len +=
	    sprintf(buf + len, " Number of Interfs : %u\n\n",
		    pCfg->desc.bNumInterfaces);

	len += sprintf(buf + len, "USB_DIR_IN (Ingress)\n");
	pIf = &gUsbCore.inIntf;
	len +=
	    sprintf(buf + len, "   Interface Number     : %i\n", pIf->number);
	len +=
	    sprintf(buf + len, "   Number of Alternates : %u\n", pIf->numAlts);
	len +=
	    sprintf(buf + len, "   Active Alternate     : %i\n",
		    pIf->activeAlt);
	len +=
	    sprintf(buf + len, "   Number of Channels   : %u\n",
		    pIf->numberChannels);
	len +=
	    sprintf(buf + len, "   Sample frequency     : %u\n", pIf->sampFreq);
	len +=
	    sprintf(buf + len, "   Frame Size 1ms       : %u\n\n",
		    pIf->frameSize1ms);
			     ...
			     int *eof, void *data)
{
					...
					case USB_FEATURE_VOLUME:
						{
							...
							len +=
							    sprintf(buf + len,
								    "      ch=%u: cur=%i(%i db) min=%i(%i db) max=%i(%i db) res=%i(%i db)\n",
								    j, cur,
								    USB_CORE_DB_VALUE
								    (cur), min,
								    USB_CORE_DB_VALUE
	...
	len += sprintf(buf + len, "   Ingress: cur=%i min=%i max=%i res=%i\n",
		       gUsbCore.inIntf.fu.volInfo.cur,
		       gUsbCore.inIntf.fu.volInfo.min,
		       gUsbCore.inIntf.fu.volInfo.max,
		       gUsbCore.inIntf.fu.volInfo.res);
	len +=
	    sprintf(buf + len, "   Egress:  cur=%i min=%i max=%i res=%i\n",
		    gUsbCore.outIntf.fu.volInfo.cur,
		    gUsbCore.outIntf.fu.volInfo.min,
		    gUsbCore.outIntf.fu.volInfo.max,
		    gUsbCore.outIntf.fu.volInfo.res);
...
*  @return  None
*/
static char *csAcIntDesc[] =
    { "UNKNOWN", "HEADER", "INPUT_TERMINAL", "OUTPUT_TERMINAL", "MIXER_UNIT",
"SELECTOR_UNIT", "FEATURE_UNIT", "PROCESSING_UNIT", "EXTENSION_UNIT" };
static char *csAsIntDesc[] =
    { "UNKNOWN", "AS_GENERAL", "FORMAT_TYPE", "FORMAT_SPECIFIC" };
static char *intClass[] = { "UNKNOWN", "USB_CLASS_AUDIO" };
static char *intSubClass[] =
    { "UNKNOWN", "USB_SUBCLASS_AUDIO_CONTROL", "USB_SUBCLASS_AUDIO_STREAMING",
				 ...
				 int count, int *eof, void *data)
{
	...
	active =
	    ((int)pIntf->cur_altsetting -
	     (int)pIntf->altsetting) / sizeof(*pIntf->altsetting);
	...
	for (j = 0; j < pIntf->num_altsetting; j++) {
		...
		len +=
		    sprintf(buf + len, ": bNumEndpoints=%u",
			    pAlts->desc.bNumEndpoints);

		if (pAlts->desc.bInterfaceClass < sizeof(intClass)) {
			len +=
			    sprintf(buf + len, " bInterfaceClass=%s",
				    intClass[pAlts->desc.bInterfaceClass]);
		} else {
		...
		if (pAlts->desc.bInterfaceSubClass < sizeof(intSubClass)) {
			len +=
			    sprintf(buf + len, " bInterfaceSubClass=%s",
				    intSubClass[pAlts->desc.
						bInterfaceSubClass]);
		} else {
			len +=
			    sprintf(buf + len, " bInterfaceSubClass=UNKNOWN");
			break;
		}
			...
			len += sprintf(buf + len, "   Ept:");
			{
				...
				len +=
				    sprintf(buf + len,
					    ...
					    pEpt->desc.bEndpointAddress);
				len +=
				    sprintf(buf + len, " bmAttributes=0x%02x",
					    pEpt->desc.bmAttributes);
				len +=
				    sprintf(buf + len, " wMaxPacketSize=%u",
					    pEpt->desc.wMaxPacketSize);
				len +=
				    sprintf(buf + len, " bInterval=%u\n",
					    pEpt->desc.bInterval);
					...
					default:
						{
							len +=
							    sprintf(buf + len,
								    " Unsupported AS Endpoint Descriptor");
						}
					}
...
static int __init halaudio_usbcore_init(void)
{
	...
	if (rc) {
		printk(KERN_ERR
		       "USB Core: Failed to register USB device driver rc=%i\n",
		       rc);
		return rc;
	}
