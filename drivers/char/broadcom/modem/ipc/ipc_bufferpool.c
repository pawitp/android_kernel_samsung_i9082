...
struct IPC_PoolList_S {
	IPC_U32 Count;
	IPC_BufferPool_T *Pool[IPC_POOLLIST_LENGTH];
};

struct IPC_PoolList_S PoolList[IPC_EndpointId_Count];
	...
	IPC_U32 FlowStopLimit, IPC_U32 LocalDescriptorSize)
{
	...
	if (NumberOfBuffers == 0) {
		IPC_TRACE(IPC_Channel_Error, "IPC_CreateBufferPool",
			  "Invalid NumberOfBuffers %d",
			  NumberOfBuffers, 0, 0, 0);
		return 0;
	}

	if (!IPC_SmEndpointInfo(SourceEndpointId)) {
		IPC_TRACE(IPC_Channel_Error, "IPC_CreateBufferPool",
			  "Invalid Source Endpoint %d",
			  SourceEndpointId, 0, 0, 0);
		return 0;
	}

	DestinationEpPtr = IPC_SmEndpointInfo(DestinationEndpointId);
	if (0 == DestinationEpPtr) {
		IPC_TRACE(IPC_Channel_Error, "IPC_CreateBufferPool",
	...
	Pool = IPC_SmPoolAlloc(sizeof(IPC_BufferPool_T),
			       DestinationEpPtr->MaxHeaderSize,
			       MaxDataSize, NumberOfBuffers);
	...
	if (LocalDescriptorSize != 0) {
		...
		LocalData = kmalloc((LocalDescriptorSize * NumberOfBuffers),
				    GFP_KERNEL);
#else
		LocalData = (char *)OSHEAP_Alloc(LocalDescriptorSize
						 * NumberOfBuffers);
#endif /* UNDER_LINUX */
	...
	for (Id = 0; Id < NumberOfBuffers; Id++) {
		...
		Buffer = IPC_BufferInitialise(Pool, Buffer, Id,
					      PoolPtr->MaxHeaderSize,
					      MaxDataSize);
	}

	/* For Debug */
	{
		struct IPC_PoolList_S *EpPools =
				&PoolList[PoolPtr->SourceEndpointId];

		if (EpPools->Count < IPC_POOLLIST_LENGTH)
			EpPools->Pool[EpPools->Count++] = PoolPtr;
	}

	IPC_TRACE(IPC_Channel_Pool, "IPC_CreateBufferPool", "Pool %08X",
		  Pool, 0, 0, 0);
