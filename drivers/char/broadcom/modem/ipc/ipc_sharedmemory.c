/**************************************************************************
*    ɲ007-2009 Broadcom Corporation
*
...
#ifdef IPC_CHECK_VT_BUFFER_RTT
static IPC_U32 CsdMaxRtt;
static IPC_U32 CsdPktsDelayed;
#define CSD_RTT_LIMIT 4
#define CSD_EP_ID IPC_EP_CsdCpCSDData

/***************************************************
// measure the round -trip delay of VT IPC buffer allocation and free */
static void IPC_BufferReturn_CheckCsdRtt(IPC_Buffer Buffer)
{
	IPC_BufferPool Pool = IPC_BufferOwningPool(Buffer);
	IPC_BufferPool_T *PoolPtr = IPC_PoolToPtr(Pool);

	if (PoolPtr->DestinationEndpointId == CSD_EP_ID) {
		IPC_U32 timestamp, rtt;
		IPC_Buffer_T *BufferPtr = IPC_BufferToPtr(Buffer);

		timestamp = TIMER_GetValue();
		rtt =
		    (IPC_U32)((int)timestamp - (int)BufferPtr->TimeStampAlloc);
		if (rtt > CsdMaxRtt)
			CsdMaxRtt = rtt;
		if (rtt >= CSD_RTT_LIMIT)
			CsdPktsDelayed++;

		/* save the buffer return time into IPC memory */
		BufferPtr->TimeStampSend = timestamp;
	}
}

static int IPC_DumpStatus_CsdPool(char *buf)
{
	IPC_SmControl SmControl = SmLocalControl.SmControl;
	IPC_BufferPool FirstPool = SmControl->FirstPool;
	IPC_BufferPool Pool = FirstPool;
	IPC_U32 PoolCount = 0;
	char *p = buf;

	p += sprintf(p, "AP CSD Pool: MaxRTT %d, PktsDelayed %d RTTLimit %d\n",
		     CsdMaxRtt, CsdPktsDelayed, CSD_RTT_LIMIT);

	while (Pool && (PoolCount < IPC_EndpointId_Count * 2)) {
		IPC_BufferPool_T *PoolPtr = IPC_PoolPtr(Pool);

		if (PoolPtr->DestinationEndpointId == CSD_EP_ID) {
			p += sprintf(p, "Dump CSD pool:\n");
			IPC_PoolDump(Pool);
			return p - buf;
		}
...
void IPC_SmFifoWrite(IPC_Fifo Fifo, IPC_Buffer Message)
{
	...
	/* use same lock for Send FIFO and FreeFIFO */
	static void *Lock;
...
IPC_SmPtr IPC_SmPoolAlloc(IPC_U32 PoolOverhead,
			  IPC_U32 HeaderSize, IPC_U32 DataSize,
			  IPC_U32 BufferCount)
{
	IPC_SmPtr Pool;
		...
		}
		{
			...
			while (0 != (Buffer = IPC_SmFifoRead(FreeFifo))) {
				IPC_BufferReturn(Buffer,
						 IPC_BufferOwningPool(Buffer));
#ifdef IPC_CHECK_VT_BUFFER_RTT
				IPC_BufferReturn_CheckCsdRtt(Buffer);
#endif
			}
		}
	}
			...
			)
{
	...
	if (SmControl->Initialised[IPC_CPU_ID_INDEX(IPC_OTHER_CPU[CPUID])] ==
	    0) {
...
{
	static IPC_U32 sleepCount;
	static IPC_U32 resumeCount;

	if (!inSleep) {
		IPC_AlignTime();
		SmLocalControl.SmControl->ApSleepTime[resumeCount % 4].
			ResumeTime = TIMER_GetValue();
		resumeCount++;
	} else {
		SmLocalControl.SmControl->ApSleepTime[sleepCount % 4].
			SleepTime = TIMER_GetValue();
		sleepCount++;
	}
	IPC_TRACE(IPC_Channel_General, "IPC_ApSleepModeSet",
		  "state %d time %u", inSleep, TIMER_GetValue(), 0, 0);
}
...
int IPC_DumpStatus(char *buf, IPC_U32 bufSize)
{
	...
	IPC_SNPRINTF(buf, bufSize, bufOffset,
		     ...
		     SmControl->IrqTime[CpuIndex][(IrqRaisedId + 1) % 4].
								IrqRaiseTime,
		     SmControl->IrqTime[CpuIndex][(IrqRaisedId + 2) % 4].
								IrqRaiseTime,
		     SmControl->IrqTime[CpuIndex][(IrqRaisedId + 3) % 4].
								IrqRaiseTime);

	/* print the interrupt raise and handle time of the last 4 CP->AP
	 * interrupts that are handled by AP We can detect interrupt loss
	 * from these info
	 */
	IrqHandledId = SmLocalControl.SmControl->IrqCounts[CpuIndex].IrqHandled;
	IPC_SNPRINTF(buf, bufSize, bufOffset,
		     "CP IrqRaiseTime %u %u %u %u\n",
		     SmControl->IrqTime[CpuIndex][IrqHandledId % 4].
							IrqRaiseTimeCopy,
		     SmControl->IrqTime[CpuIndex][(IrqHandledId + 1) % 4].
							IrqRaiseTimeCopy,
		     SmControl->IrqTime[CpuIndex][(IrqHandledId + 2) % 4].
							IrqRaiseTimeCopy,
		     SmControl->IrqTime[CpuIndex][(IrqHandledId + 3) % 4].
							IrqRaiseTimeCopy);
	...
	/* print the interrupt raise and handle time of the last 4 AP->CP
	 * interrupts that are handled by CP We can detect interrupt loss
	 * from these info
	 */
	IrqHandledId = SmLocalControl.SmControl->IrqCounts[CpuIndex].IrqHandled;
	IPC_SNPRINTF(buf, bufSize, bufOffset,
...
void IPC_UpdateIrqStats(void)
{
	...
	SmLocalControl.SmControl->IrqTime[Cpu][IrqHandled % 4].IrqRaiseTimeCopy
		= SmLocalControl.SmControl->
			IrqTime[PeerCpu][(PeerIrqRaised + 3) % 4].IrqRaiseTime;
