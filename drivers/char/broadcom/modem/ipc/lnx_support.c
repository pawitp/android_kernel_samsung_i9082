...
struct tasklet_struct *intr_tasklet_ptr_g;

/**
  @fn void bcm_lnx_init(struct tasklet_struct *tsklet_ptr);

  Note: this api is to avoid cyclic dependency between this file and the
  ipc_proxy_server code.
*/
...
unsigned int bcm_map_virt_to_phys(void *virt_addr)
{
	/* Note: since shared-mem is io-mem, these kernel call to map virt to
	 * phys does not work
...
void *bcm_map_phys_to_virt(unsigned int physical_addr)
{
	/* Note: since shared-mem is io-mem, these kernel call to map phys to
	 * virt does not work
