...
\msc
	...
	"AP Client" => RPC[label = "RPC_SyncInitialize()",
			URL = "\ref RPC_SyncInitialize()"];
	"CP Client">>RPC[label = "RPC_SyncInitialize()",
			URL = "\ref RPC_SyncInitialize()"];
	"AP Client" => RPC[label = "RPC_SyncRegisterClient()",
			URL = "\ref RPC_SyncRegisterClient()"];
	"CP Client">>RPC[label = "RPC_SyncRegisterClient()",
			URL = "\ref RPC_SyncRegisterClient()"];
	"AP Client" => RPC[label = "RPC_SyncCreateTID()",
			URL = "\ref RPC_SyncCreateTID()"];
	"AP Client" => RPC[label = "RPC_SerializeReq()",
			URL = "\ref RPC_SerializeReq()"];
	RPC rbox RPC [label="Serialize, send IPC Msg, Deser msg"];
	"AP Client" :> RPC[label = "RPC_SyncWaitForResponse()",
			URL = "\ref RPC_SyncWaitForResponse()"];
	"AP Client"=>RPC [label="CBK: Notify Request"];
	"AP Client"=>"AP Client" [label="thread blocks"];
	"CP Client" => RPC[label = "RPC_SendAckForRequest()",
			URL = "\ref RPC_SendAckForRequest()"];
	RPC rbox RPC [label="ARPC:Serialize, send IPC Msg, Deser msg"];
	RPC rbox RPC [label="SRPC:Raise ACK Sem"];
	"CP Client" => RPC[label = "RPC_SerializeRsp()",
			URL = "\ref RPC_SerializeRsp()"];
	RPC rbox RPC [label="ARPC:Serialize, send IPC Msg, Deser msg"];
...
**/
	Result_t RPC_SyncWaitForResponseTimer(UInt32 tid, UInt8 cid,
					      RPC_ACK_Result_t *ack,
					      MsgType_t *msgType,
					      UInt32 *dataSize, UInt32 timeout);
