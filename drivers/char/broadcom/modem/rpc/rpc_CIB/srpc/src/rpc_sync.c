...
typedef struct {
	...
	RPC_SyncState_t state;	/* state of transaction */
...
void RPC_SyncHandleAck(UInt32 tid, UInt8 clientID, RPC_ACK_Result_t ackResult,
		       UInt32 ackData);
static void RPC_SyncCPResetCb(RPC_CPResetEvent_t event, UInt8 clientID);
...
static void RPC_SyncCPResetCb(RPC_CPResetEvent_t event, UInt8 clientID)
{
	...
	for (i = 0; i < MAX_TASKS_NUM; i++) {
		...
		if (taskMap != NULL && taskMap->task != NULL) {
			_DBG_(RPC_TRACE
	...
	if (internalParam && internalParam->clientParams.cpResetCb) {
