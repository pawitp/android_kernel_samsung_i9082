	...
	for (i = (*offset); i < MAX_RPC_PACKETS; i++) {
		*offset = i;
		...
		if (gRpcDbgPktStatusList[i].pktHandle != 0) {
			...
			ret =
			    RpcDbgDumpStr(c,
					  "pkt:%d TS:%u:%u status:%s itype:%d c1=%d c2=0x%x\n",
					  (unsigned int)
					  gRpcDbgPktStatusList[i].pktHandle,
...
{
	gRpcDbgGenInfoList =
	    kmalloc((RPC_DBG_GEN_MAX + 1) * sizeof(RpcDbgLogInfo_t),
		    GFP_ATOMIC);
	if (gRpcDbgGenInfoList)
		memset(gRpcDbgGenInfoList, 0,
		       (RPC_DBG_GEN_MAX + 1) * sizeof(*gRpcDbgGenInfoList));

	gRpcDbgPktStatusList =
	    kmalloc((MAX_RPC_PACKETS + 1) * sizeof(RpcDbgPktStatus_t),
		    GFP_ATOMIC);
	if (gRpcDbgPktStatusList)
		memset(gRpcDbgPktStatusList, 0,
		       (MAX_RPC_PACKETS + 1) * sizeof(*gRpcDbgPktStatusList));
}
