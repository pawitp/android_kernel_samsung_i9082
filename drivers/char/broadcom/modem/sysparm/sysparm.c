...
static int SYSPARM_GetNameFromPA(char *nameaddr, char *str)
{
	 ...
	 * nameaddr:0x%08lX\n",(UInt32)nameaddr);
	 */
	name_ptr = (char *)ioremap_nocache((UInt32)nameaddr,
					   MAX_SYSPARM_NAME_SIZE);
	if (!name_ptr) {
		pr_err
	...
	if (name_len == 0 || name_len == MAX_SYSPARM_NAME_SIZE) {
		pr_err("[sysparm]: SYSPARM_GetNameFromPA: name_len"
		       " is invalid:%d\n", name_len);
		iounmap(name_ptr);
		return 0;
	}
...
*****************************************************************************/
static UInt32 SYSPARM_GePAtByIndex(char *name,
	...
	for (i = 0; i < *total_index_ptr; i++) {
		...
		if (!strcmp(sysparm_name, name)) {
			if (size != sysparm_index[i].size) {
				pr_err
				    ("[sysparm]: SYSPARM_GePAtByIndex: sysparm:"
...
******************************************************************************/
#if 0
static int SYSPARM_SetByIndex(void *ptr, char *name,
			      unsigned int size, unsigned int flag)
{
	UInt32 i, mask;
...
UInt16 SYSPARM_GetActualLowVoltReading(void)
{
	...
	default_batt_ActualLowVoltReading_addr
	    = SYSPARM_GePAtByIndex("batt_ActualLowVoltReading",
				   sizeof
				   (*default_batt_ActualLowVoltReading_ptr), 1);
...
static void __exit sysparm_module_exit(void)
{
	...
	if (ind_mmaudio_parm_table)
		iounmap(ind_mmaudio_parm_table);
	if (mmaudio_parm_table)
		iounmap(mmaudio_parm_table);
...
#include <linux/module.h>
EXPORT_SYMBOL(APSYSPARM_GetAudioParmAccessPtr);
EXPORT_SYMBOL(APSYSPARM_GetMultimediaAudioParmAccessPtr);
