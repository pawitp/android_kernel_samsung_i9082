	...
	if (sCsdPendingBytes >= 160) {
		...
		while (dwWriteLen) {
			if (CFG_RPC_CSDDATA_PKT_SIZE < dwWriteLen)
				dwSentLen =
				    CSD_CP_Loopback_Send((pBuf + dwSentBytes),
...
UInt32 CSD_Send(const void *pBuf, const UInt32 len)
{
	...
	if (RPC_RESULT_OK !=
	    RPC_PACKET_SendData(g_CsdClientId, INTERFACE_CSD, cid, bufHandle))
		VSP_DEBUG(DBG_ERROR, "vsp: FIFO is full.\n");
...
*******************************************************************************/
RPC_Result_t CSD_DataIndCB(PACKET_InterfaceType_t interfaceType, UInt8 channel,
...
void CSD_CPResetCB(RPC_CPResetEvent_t event, PACKET_InterfaceType_t interface)
{
...
*******************************************************************************/
int vsp_write(struct file *filp, const char __user *buf, size_t size,
	      loff_t *offset)
{
