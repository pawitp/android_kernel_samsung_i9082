...
#elif defined(CONFIG_MFD_BCM59056)
struct rtccal_pdata rtccal_data = {
	.device = "BCM59056",
	.nom_ratio = 8666667,
	.min_ratio = 8663200,
...
unsigned long rtc_util_convert_rtctime_t2sec(struct rtctime_t *pTimeSrc)
{
	...
	return (days + pTimeSrc->mday - 1) * 24 * 60 * 60
	    ...
	    + (unsigned long)(pTimeSrc->min) * 60 + pTimeSrc->sec;
}
...
void rtc_util_convert_sec2rtctime_t(struct rtctime_t *pDest, unsigned long time)
{
	...
	pDest->mday = (days >= rtc_util_getnumofdaysinmon(RTC_YEARBASE + y, m))
	    ? 1 : (days + 1);
...
**/
bool rtc_cal_msg(unsigned char id, unsigned long val1, unsigned long val2,
...
static u64 rtc_cal_tc_updatevals(unsigned long time)
{
	...
	if (time_left > pRTC_TC->time_hz_curr) {
		...
		msleep((time_left -
			pRTC_TC->time_hz_curr) * (RTC_ONE_SEC / HZ));
	} else
...
void rtc_cal_tc_get_time(struct rtc_time *tm)
{
	...
	pr_rtcdebug("%s:hw_time=%lu,hz_curr=%lu,hz_acc=%lu,slipp=%lu,flag=%d",
...
bool rtc_cal_init(void)
{
	...
	ret = kfifo_alloc(&rtccal_kfifo,
			  RTC_QUEUESIZE * sizeof(struct rtc_cal_msg_t),
			  GFP_KERNEL);
				     ...
				     size_t count, loff_t *ppos)
{
	...
	len += snprintf(buf + len, sizeof(buf) - len,
			"\tyear = %d, ", tm.tm_year);
	len += snprintf(buf + len, sizeof(buf) - len, "mon = %d, ", tm.tm_mon);
	len += snprintf(buf + len, sizeof(buf) - len,
			"mday = %d, ", tm.tm_mday);
	len += snprintf(buf + len, sizeof(buf) - len,
			"hour = %d, ", tm.tm_hour);
	len += snprintf(buf + len, sizeof(buf) - len, "min = %d, ", tm.tm_min);
	len += snprintf(buf + len, sizeof(buf) - len, "sec = %d\n", tm.tm_sec);
...
int __init bcm_rtc_cal_debug_init(void)
{
	...
	if (!debugfs_create_file("parameters", S_IRUSR,
				 dent_rtc_cal_root_dir, NULL,
				 &rtc_cal_parameter_fops))
		return -ENOMEM;

	if (!debugfs_create_file("read_time", S_IRUSR,
				 dent_rtc_cal_root_dir, NULL,
				 &rtc_cal_read_time_fops))
		return -ENOMEM;
	...
	if (!debugfs_create_file
	    ("ratio_event", S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH,
