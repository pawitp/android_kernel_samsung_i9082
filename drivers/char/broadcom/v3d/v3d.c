...
static v3d_job_t *v3d_job_create(struct file *filp, v3d_job_post_t * p_job_post)
{
	...
	KLOG_V
	    ("job[0x%08x] dev_id[%d] job_id[%d] job_type[%d] ct0_ca[0x%x] ct0_ea[0x%x] ct1_ca[0x%x] ct1_ea[0x%x]",
...
static int v3d_check_status(int state)
{
	...
	case 3:{
			if ((v3d_read(V3D_BFC_OFFSET) != 1) || (v3d_flags != 2)) {
				KLOG_V
				    ("state[%d] BFC[0x%08x] v3d_flags[0x%08x]",
				     state, v3d_read(V3D_BFC_OFFSET),
				     v3d_flags);
				print_status = 1;
			}
		}
...
static int v3d_job_start(int turn_on)
{
	...
	if ((p_v3d_job->job_type == V3D_JOB_REND)
		...
		//Submit binning first
		{
			...
			iowrite32(p_v3d_job->v3d_ct0ca,
				  v3d_base + V3D_CT0CA_OFFSET);
			iowrite32(p_v3d_job->v3d_ct0ea,
				  v3d_base + V3D_CT0EA_OFFSET);
		}
		...
		if (p_v3d_job->v3d_ct1ca != p_v3d_job->v3d_ct1ea) {
			...
			iowrite32(p_v3d_job->v3d_ct1ca,
				  v3d_base + V3D_CT1CA_OFFSET);
			iowrite32(p_v3d_job->v3d_ct1ea,
				  v3d_base + V3D_CT1EA_OFFSET);
		}
...
static int v3d_thread(void *data)
{
	...
	while (1) {
		if (v3d_job_curr == NULL) {
			...
			v3d_hw_release();
			KLOG_V("wait exit, v3d_in_use[%d], timeout[%d]",
			       v3d_in_use, timeout);
...
static int setup_v3d_clock(void)
{
	...
	v3d_state.dfs_node =
	    pi_mgr_dfs_add_request("v3d", PI_MGR_PI_ID_MM, PI_OPP_TURBO);
...
static bool v3d_is_not_idle(void)
{
	...
	not_idle = ((pcs & V3D_PCS_RMACTIVE_MASK) != 0 ||
		    ...
		    (srqcs & V3D_SRQCS_QPURQL_MASK) > 0 || qpurqcc != qpurqcm);
...
static void trace_add_entry(unsigned long id, uint32_t a, uint32_t b,
			    uint32_t c, uint32_t d, uint32_t e,
			    unsigned long tag, int want_cle_state_too)
{
		...
		if (v3d_oom_block_used == 0) {
			KLOG_V
			    ("v3d oom blk not used: flags[0x%02x] intctl[0x%08x] bpoa[0x%08x] bpos[0x%08x] bpca[0x%08x] bpcs[0x%08x]",
			     flags, v3d_read(V3D_INTCTL_OFFSET),
			     v3d_read(V3D_BPOA_OFFSET),
			     v3d_read(V3D_BPOS_OFFSET),
			     v3d_read(V3D_BPCA_OFFSET),
			     v3d_read(V3D_BPCS_OFFSET));
			iowrite32(v3d_bin_oom_block,
				  v3d_base + V3D_BPOA_OFFSET);
			iowrite32(v3d_bin_oom_size, v3d_base + V3D_BPOS_OFFSET);
...
static int v3d_hw_acquire(bool for_worklist)
{
	...
	v3d_state.free_time += v3d_state.j1 - v3d_state.j2;
	if ((v3d_state.show_v3d_usage)
	    && jiffies_to_msecs(v3d_state.free_time + v3d_state.acquired_time) >
	    5000) {
		KLOG_E("V3D usage = %lu \n",
		       (uint32_t)(v3d_state.acquired_time * 100) /
		       (v3d_state.free_time + v3d_state.acquired_time));
...
static long v3d_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case V3D_IOCTL_SYNCTRACE:{
			...
			BUG_ON(s);
			if (!s)
				trace_add_entry(x.t, x.arg0, x.arg1, 0, 0, 0,
						x.l, 1);
		}
		break;
	...
	case V3D_IOCTL_GET_MEMPOOL:{
			...
			if (dev->mem_slot == MEM_SLOT_UNAVAILABLE) {
				...
				if (dev->mem_slot == MEM_SLOT_UNAVAILABLE
				    || !v3d_mempool_base) {
					KLOG_E
					    ("Failed to find slot in relocatable heap\n");
					up(&v3d_state.work_lock);
					return -EPERM;
				}
				...
				dev->mempool.ptr =
				    v3d_mempool_base +
				    (v3d_relocatable_chunk_size *
				     dev->mem_slot);
				dev->mempool.addr =
				    virt_to_phys(dev->mempool.ptr);
			...
			if (copy_to_user
			    ((mem_t *) arg, &(dev->mempool), sizeof(mem_t)))
				ret = -EPERM;
		}
		break;
...
static void trace_dump_now(void)
{
	...
	tsbase =
	    v3d_state.tracebuf[first_ent & (v3d_state.num_trace_ents - 1)].
	    timestamp;
	...
	for (i = first_ent; i < last_ent; i++) {
		...
		printk(KERN_ERR
		       "djlb-trace[%d@%lu]: 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X %lu\n",
		       i, te->timestamp - tsbase, te->id, te->arg0, te->arg1,
		       te->arg2, te->arg3, te->arg4, te->tag);
	}

	if (i != v3d_state.traceptr) {
		KLOG_E(KERN_ERR
		       "djlb-trace: trace cannot be trusted.  Read the comment in the code about the race condition\n");
	}
}
...
static void trace_realloc(int newbufsz)
{
	...
	if (v3d_state.num_trace_ents > 0) {
		v3d_state.tracebuf =
		    kmalloc(v3d_state.num_trace_ents *
			    sizeof(*v3d_state.tracebuf), GFP_KERNEL);
...
int __init v3d_init(void)
{
	...
	/* reigster qos client */
	v3d_state.qos_node =
	    pi_mgr_qos_add_request("v3d", PI_MGR_PI_ID_ARM_CORE,
				   PI_MGR_QOS_DEFAULT_VALUE);
...
void __exit v3d_exit(void)
{
	...
	if (v3d_mempool_base)
		free_bootmem((long unsigned int)v3d_mempool_base,
			     v3d_mempool_size);
