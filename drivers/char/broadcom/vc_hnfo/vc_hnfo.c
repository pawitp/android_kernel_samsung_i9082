...
static dev_t vc_hnfo_devnum;
static struct class *vc_hnfo_class;
...
static long vc_hnfo_ioctl(struct file *file, unsigned int cmd,
			  unsigned long arg)
{
	int rc = 0;
...
static int vc_hnfo_proc_read(char *buf, char **start, off_t offset, int count,
			     int *eof, void *data)
{
	char *p = buf;
	...
	p += sprintf(p, "     \'whdmi_audio\' : %s\n",
		     vc_hnfo_status.whdmi_audio ? "oui" : "non");
...
static int vc_hnfo_proc_write(struct file *file, const char __user *buffer,
			      unsigned long count, void *data)
{
	...
	} else if (strcmp(name, "whdmi_audio") == 0) {
		vc_hnfo_status.whdmi_audio = value;
...
static int __init vc_hnfo_init(void)
{
	...
	rc = alloc_chrdev_region(&vc_hnfo_devnum, 0, 1, DRIVER_NAME);
	if (rc < 0) {
	...
	rc = cdev_add(&vc_hnfo_cdev, vc_hnfo_devnum, 1);
	if (rc != 0) {
	 ...
	 */
	/* No explicit power on, power on detection. */
	vc_hnfo_status.power_on = 0;
	/* Don't disable action on detection. */
	vc_hnfo_status.dis_detect = 0;
	/* 'HDMI_CEA_720p50' - Highest supported resolution preferred. */
	vc_hnfo_status.resolution = 19;
	/* Can't ask for more at this time (ie 'HDMI_CEA_1080p60'=16). */
	/* Don't disable GUI cloning. */
	vc_hnfo_status.dis_clone = 0;
	/* Don't enable wifi-hdmi audio behavior. */
	vc_hnfo_status.whdmi_audio = 0;
