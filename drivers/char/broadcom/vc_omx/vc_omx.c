...
static struct vc_omx_comp_supported
	...
	{1,
	 OMX_PLUGIN__MKNAME(OMX_PLUGIN__COMPONENT_SUPPORTED__VIDEO_DECODER,
			    OMX_PLUGIN__COMPONENT_SUPPORTED__H263)},
	{1,
	 OMX_PLUGIN__MKNAME(OMX_PLUGIN__COMPONENT_SUPPORTED__VIDEO_DECODER,
			    OMX_PLUGIN__COMPONENT_SUPPORTED__AVC)},
	{1,
	 OMX_PLUGIN__MKNAME(OMX_PLUGIN__COMPONENT_SUPPORTED__VIDEO_DECODER,
			    OMX_PLUGIN__COMPONENT_SUPPORTED__MPEG2)},
	{1,
	 OMX_PLUGIN__MKNAME(OMX_PLUGIN__COMPONENT_SUPPORTED__VIDEO_DECODER,
			    OMX_PLUGIN__COMPONENT_SUPPORTED__WMV3)},
	{1,
	 OMX_PLUGIN__MKNAME(OMX_PLUGIN__COMPONENT_SUPPORTED__VIDEO_DECODER,
			    OMX_PLUGIN__COMPONENT_SUPPORTED__WVC1)},
	{1,
	 OMX_PLUGIN__MKNAME(OMX_PLUGIN__COMPONENT_SUPPORTED__VIDEO_DECODER,
			    OMX_PLUGIN__COMPONENT_SUPPORTED__VPX)},
	{1,
	 OMX_PLUGIN__MKNAME(OMX_PLUGIN__COMPONENT_SUPPORTED__VIDEO_DECODER,
			    OMX_PLUGIN__COMPONENT_SUPPORTED__SPARK)},
	...
	case VC_OMX_IOC_COMP_STATUS:
		{
			...
			for (index = 0;
			     index < OMX_PLUGIN__COMPONENT_SUPPORTED__NUM;
			     index++) {
				if (strcmp
				    (vc_omx_status.name,
				     vc_omx_info[index].name) == 0) {
					vc_omx_status.enabled =
					    vc_omx_info[index].enabled;
					break;
				}
			}
...
static int vc_omx_proc_read(char *buf, char **start, off_t offset, int count,
			    int *eof, void *data)
{
	char *p = buf;
	int index;
	...
	for (index = 0; index < OMX_PLUGIN__COMPONENT_SUPPORTED__NUM; index++) {
		p += sprintf(p, "\t%s\t%s\n",
			     ...
			     vc_omx_info[index].enabled ? "Enabled" :
			     "DISABLED");
	}
...
static int vc_omx_proc_write(struct file *file, const char __user *buffer,
			     unsigned long count, void *data)
{
	...
	if (strcmp(name, "color") == 0) {
		...
		for (index = 0; index < OMX_PLUGIN__COMPONENT_SUPPORTED__NUM;
		     index++) {
			if (strcmp(name, vc_omx_info[index].name) == 0) {
				vc_omx_info[index].enabled =
				    (value > 0 ? 1 : 0);
				goto out;
			}
...
static int __init vc_omx_init(void)
{
	...
	rc = alloc_chrdev_region(&vc_omx_devnum, 0, 1, DRIVER_NAME);
	if (rc < 0) {
	...
	rc = cdev_add(&vc_omx_cdev, vc_omx_devnum, 1);
	if (rc != 0) {
