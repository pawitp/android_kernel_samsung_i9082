...
typedef struct opaque_vc_vchi_sm_handle_t {
	...
	SM_CMD_RSP_BLK_T free_blk[SM_MAX_NUM_CMD_RSP_BLKS];
...
/* ---- Private Functions ------------------------------------------------ */
static SM_CMD_RSP_BLK_T *vc_vchi_cmd_create(SM_INSTANCE_T * instance,
					    VC_SM_MSG_TYPE id, void *msg,
					    uint32_t size, int wait)
{
	SM_CMD_RSP_BLK_T *blk;
	VC_SM_MSG_HDR_T *hdr;
	...
	if (down_interruptible(&instance->free_sema)) {
		...
		blk =
		    list_first_entry(&instance->free_list, SM_CMD_RSP_BLK_T,
				     head);
		list_del(&blk->head);
...
static void vc_vchi_cmd_delete(SM_INSTANCE_T * instance, SM_CMD_RSP_BLK_T * blk)
{
	if (blk->alloc) {
		kfree(blk);
		return;
...
static int vc_vchi_sm_videocore_io(void *arg)
{
	...
	SM_CMD_RSP_BLK_T *cmd = NULL, *cmd_tmp;
	...
	while (1) {
		...
		if (!down_interruptible(&instance->io_sema)) {
			...
			do {
				...
				/* Send the command */
				status =
				    vchi_msg_queue(instance->vchi_handle[0],
						   cmd->msg, cmd->length,
				...
				void *msg_handle)
{
	...
	case VCHI_CALLBACK_SERVICE_CLOSED:
		LOG_INFO("%s: service CLOSED!!", __func__);
	default:
		break;
	}
}
				    ...
				    uint32_t num_connections)
{
	...
	if (instance->io_thread == NULL) {
		LOG_ERR("%s: failed to create SMIO thread", __func__);
			...
			uint32_t *cur_trans_id, uint8_t wait_reply)
{
	...
	if (msg == NULL) {
		LOG_ERR("%s: invalid msg pointer", __func__);
		return -EINVAL;
	}
...
int vc_vchi_sm_resize(VC_VCHI_SM_HANDLE_T handle,
		      VC_SM_RESIZE_T * msg, uint32_t *cur_trans_id)
{
