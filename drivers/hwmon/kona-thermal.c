					  ...
					  char *buf)
{
	...
	if (attr->nr == 1) {
		...
		if (board->sensors[attr->index].thermal_read ==
			...
			if (board->sensors[attr->index].convert_callback ==
			    NULL)
				temp = convert_adc_to_temp(raw);
			else
				temp =
				    board->sensors[attr->
						   index].convert_callback(raw);

			therm_debug
			    ("temp (deg. C) = %ld (raw = %ld (0x%lx) (0x%lx)\n",
			     temp, raw, raw, raw / 4);
			return snprintf(buf, MAXBUF, "%d\n",
					convert_adc_to_temp(raw));
		}
	}

	return snprintf(buf, MAXBUF, "%ld\n", raw);
				      ...
				      char *buf)
{
	...
	if (pdata->alarm_set) {
		...
		return snprintf(buf, MAXBUF, "1\n");
	}

	return snprintf(buf, MAXBUF, "0\n");
}
...
static ssize_t kona_thermal_get_lowest(struct device *dev,
				       struct device_attribute *devattr,
				       char *buf)
{
	struct platform_drvdata *pdata = dev_get_drvdata(dev);
	...
	board = pdata->pdev->dev.platform_data;
	return snprintf(buf, MAXBUF, "%ld\n",
			board->sensors[attr->index].thermal_lowest);
}

static ssize_t kona_thermal_get_highest(struct device *dev,
					struct device_attribute *devattr,
					char *buf)
{
	struct platform_drvdata *pdata = dev_get_drvdata(dev);
	...
	board = pdata->pdev->dev.platform_data;
	return snprintf(buf, MAXBUF, "%ld\n",
			board->sensors[attr->index].thermal_highest);
}

static ssize_t kona_thermal_reset_history(struct device *dev,
					  struct device_attribute *devattr,
					  const char *buf, size_t count)
{
	struct platform_drvdata *pdata = dev_get_drvdata(dev);
	...
	for (i = 0; i < board->num_sensors; i++) {
		board->sensors[i].thermal_highest = 0;
		board->sensors[i].thermal_lowest = 0;
	}
	return count;
}

#define SENSOR_TEMP_LABEL(ix) \
	SENSOR_ATTR_2(temp##ix##_label, S_IRUGO, \
		kona_thermal_get_name, NULL, 0, ix-1)
#define SENSOR_TEMP_INPUT(ix) \
	SENSOR_ATTR_2(temp##ix##_input, S_IRUGO, \
		kona_thermal_get_threshold, NULL, 0, ix-1)
#define SENSOR_TEMP_MAX(ix) \
	SENSOR_ATTR_2(temp##ix##_max, S_IRUGO | S_IWUSR, \
		kona_thermal_get_threshold, \
		kona_thermal_set_threshold, 1, ix-1)
#define SENSOR_TEMP_CRIT(ix) \
	SENSOR_ATTR_2(temp##ix##_crit, S_IRUGO | S_IWUSR, \
		kona_thermal_get_threshold, \
		kona_thermal_set_threshold, 2, ix-1)
#define SENSOR_TEMP_LOWEST(ix) \
	SENSOR_ATTR_2(temp##ix##_lowest, S_IRUGO, \
		kona_thermal_get_lowest, NULL, 0, ix-1)
#define SENSOR_TEMP_HIGHEST(ix) \
	SENSOR_ATTR_2(temp##ix##_highest, S_IRUGO, \
		kona_thermal_get_highest, NULL, 0, ix-1)
#define SENSOR_TEMP_ALARM(ix) \
...
static struct sensor_device_attribute_2 kona_attrs[] = {
	SENSOR_ATTR_2(update_interval, S_IWUSR | S_IRUGO,
		      ...
		      0),
	SENSOR_ATTR_2(temp_reset_history, S_IWUSR | S_IRUGO,
		      NULL, kona_thermal_reset_history, 0, 0),
	SENSOR_TEMP_LABEL(1),
	SENSOR_TEMP_INPUT(1),
	SENSOR_TEMP_MAX(1),
	SENSOR_TEMP_CRIT(1),
	SENSOR_TEMP_LOWEST(1),
	SENSOR_TEMP_HIGHEST(1),
	SENSOR_TEMP_ALARM(1),
	SENSOR_TEMP_LABEL(2),
	SENSOR_TEMP_INPUT(2),
	SENSOR_TEMP_MAX(2),
	SENSOR_TEMP_CRIT(2),
	SENSOR_TEMP_LOWEST(2),
	SENSOR_TEMP_HIGHEST(2),
	SENSOR_TEMP_ALARM(2),
	SENSOR_TEMP_LABEL(3),
	SENSOR_TEMP_INPUT(3),
	SENSOR_TEMP_MAX(3),
	SENSOR_TEMP_CRIT(3),
	SENSOR_TEMP_LOWEST(3),
	SENSOR_TEMP_HIGHEST(3),
	SENSOR_TEMP_ALARM(3),
	SENSOR_TEMP_LABEL(4),
	SENSOR_TEMP_INPUT(4),
	SENSOR_TEMP_MAX(4),
	SENSOR_TEMP_CRIT(4),
	SENSOR_TEMP_LOWEST(4),
	SENSOR_TEMP_HIGHEST(4),
	SENSOR_TEMP_ALARM(4)
};
	...
	for (i = 0; i < board->num_sensors; i++) {
		if (board->sensors[i].thermal_control == SENSOR_PERIODIC_READ) {
			if (board->sensors[i].thermal_read ==
				...
				if (board->sensors[i].convert_callback == NULL)
					temp = convert_adc_to_temp(raw);
				else
					temp =
					    board->
					    sensors[i].convert_callback(raw);

				if (temp > board->sensors[i].thermal_highest)
					board->sensors[i].thermal_highest =
					    temp;
				if ((temp < board->sensors[i].thermal_lowest)
				    || (board->sensors[i].thermal_lowest == 0))
					board->sensors[i].thermal_lowest = temp;
				if (raw == 0)
					pr_info("FAIL xero ADC read\n");
				else if (temp >
					 board->
					 sensors[i].thermal_warning_lvl_1) {
...
int kona_thermal_debug_init(void)
{
	...
	if (!debugfs_create_u32
		...
		return -ENOMEM;
	if (!debugfs_create_u32
	    ("query", S_IWUSR | S_IRUSR, dent_thermal_root_dir,
	     (int *)&adc_query_cnt))
		return -ENOMEM;
...
{
/* temporarily remove resume actions */
#if 0
