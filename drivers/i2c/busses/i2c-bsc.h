...
*
*****************************************************************************/
#ifndef __I2C_BSC_H__
#define __I2C_BSC_H__

#ifndef __KERNEL__
#include <stdio.h>
#endif

#include <mach/i2c-bsc-defs.h>
...
/**
*
*  @brief  Initialize PMU_BSC to use HS mode
*
*  @param  reg_start (in) phycical address of starting of register space of PMU_BSC
*  @param  reg_end (in) phycical address of ending of register space of PMU_BSC
*  @param  irq (in) interrupt number of PMU_BSC
*  @param  pstatus (out) status of the operation
*
*  @return pointer to the device structure of PMU_BSC
*****************************************************************************/
...
/**
*
*  @brief  De-initialize to free the requested resources
*
*  @param  handle (in) pointer to the device structure of PMU_BSC
*  @param  reg_start (in) phycical address of starting of register space of PMU_BSC
*  @param  reg_end (in) phycical address of ending of register space of PMU_BSC
*  @param  irq (in) interrupt number of PMU_BSC
*  @param  pstatus (out) status of the operation
...
static inline void isl_bsc_init(uint32_t baseAddr)
{
   ...
   /* start with non-HS, use 13MHz */
	BSC_WRITE_REG_FIELD((baseAddr+I2C_MM_HS_TIM_OFFSET), I2C_MM_HS_TIM_PRESCALE_MASK , I2C_MM_HS_TIM_PRESCALE_SHIFT, I2C_MM_HS_TIM_PRESCALE_CMD_DIV1);
...
static inline void bsc_stop(uint32_t baseAddr)
{
   BSC_WRITE_REG_FIELD((baseAddr+I2C_MM_HS_CS_OFFSET), I2C_MM_HS_CS_EN_MASK, I2C_MM_HS_CS_EN_SHIFT, I2C_MM_HS_CS_EN_CMD_RESET_BSC);
...
#define BSC_HS_HSMODE_1625KHZ             0x00000A8
...
static inline void bsc_set_bus_speed(uint32_t baseAddr, BSC_SPEED_t speed, bool src_clk_26m)
{
    uint8_t DIV = 0, M = 0, N = 0, P = 0,NO_DIV=0,PRESCALE=I2C_MM_HS_TIM_PRESCALE_CMD_DIV1;
    NO_DIV=0;
	...
	case BSC_SPD_HS_2600KHZ:
        ...
        case BSC_SPD_100K:
            if(src_clk_26m)
            {
                M = 0x05;
                N = 0x01;
                DIV = BSCTIM_DIV_1625000HZ;
                P = 0x05;
                break;
            }
        default: /* For BSC_SPD_100K and src_clk_26m==0 */
            M = 0x01;
            N = 0x01;
...
static inline void bsc_start_highspeed(uint32_t baseAddr)
{
    uint32_t val;
#ifndef FPGA 
    if( (BSC_READ_REG(baseAddr + I2C_MM_HS_HSTIM_OFFSET)&0xffff) == BSC_HS_HSMODE_TIMING_26MHZ ){
        /* set to 0xcc; prescale = 3(no division), p=1, no_div=1 */
        val = (1 << I2C_MM_HS_TIM_P_SHIFT) |
          (I2C_MM_HS_TIM_PRESCALE_CMD_DIV1 << I2C_MM_HS_TIM_PRESCALE_SHIFT) |
          (I2C_MM_HS_TIM_NO_DIV_MASK);
        BSC_WRITE_REG_FIELD((baseAddr + I2C_MM_HS_CLKEN_OFFSET), I2C_MM_HS_CLKEN_M_MASK, I2C_MM_HS_CLKEN_M_SHIFT, 1); // set m=1
    }else{
        val = (7 << I2C_MM_HS_TIM_P_SHIFT) |
              (I2C_MM_HS_TIM_NO_DIV_MASK); /* set to 0x3c, p=7, no_div=1 */
    }
#else
    val = (7 << I2C_MM_HS_TIM_P_SHIFT) |
          (I2C_MM_HS_TIM_PRESCALE_CMD_DIV1 << I2C_MM_HS_TIM_PRESCALE_SHIFT) |
...
static inline void bsc_stop_highspeed(uint32_t baseAddr)
{
    ...
    if( (BSC_READ_REG(baseAddr+I2C_MM_HS_HSTIM_OFFSET) & 0xffff) == BSC_HS_HSMODE_TIMING_26MHZ ){
        /* set to 0xeb; prescale = 3(no division), p=5, no_div=0, div=3 (divide by 2 ) for 400khz */
        val = (5 << I2C_MM_HS_TIM_P_SHIFT) |
          (I2C_MM_HS_TIM_PRESCALE_CMD_DIV1 << I2C_MM_HS_TIM_PRESCALE_SHIFT) |
          (BSCTIM_DIV_6500000HZ << I2C_MM_HS_TIM_DIV_SHIFT);
        BSC_WRITE_REG_FIELD((baseAddr + I2C_MM_HS_CLKEN_OFFSET), I2C_MM_HS_CLKEN_M_MASK, I2C_MM_HS_CLKEN_M_SHIFT, 5); // set m=5
    }else{
        val=BSC_READ_REG(baseAddr+I2C_MM_HS_HSTIM_OFFSET);
        val &= ~(I2C_MM_HS_TIM_PRESCALE_MASK | I2C_MM_HS_TIM_NO_DIV_MASK |I2C_MM_HS_TIM_DIV_MASK);
        val |= ((I2C_MM_HS_TIM_PRESCALE_CMD_DIV4 << I2C_MM_HS_TIM_PRESCALE_SHIFT) |BSCTIM_DIV_6500000HZ);
    }
    BSC_WRITE_REG((baseAddr+I2C_MM_HS_TIM_OFFSET),val);
}
    ...
    switch (cmd) {
		...
		case BSC_CMD_START:
            temp = (temp & ~I2C_MM_HS_CS_ACK_MASK & ~I2C_MM_HS_CS_CMD_MASK) |
                (I2C_MM_HS_CS_CMD_CMD_START_RESTART << I2C_MM_HS_CS_CMD_SHIFT);
			break;
        ...
        case BSC_CMD_STOP:
            temp = (temp & ~I2C_MM_HS_CS_CMD_MASK) |
                I2C_MM_HS_CS_CMD_CMD_STOP << I2C_MM_HS_CS_CMD_SHIFT;
			break;

        case BSC_CMD_READ_ACK:
            temp = (temp & ~I2C_MM_HS_CS_ACK_MASK & ~I2C_MM_HS_CS_CMD_MASK) |
                (I2C_MM_HS_CS_CMD_CMD_READ_BYTE << I2C_MM_HS_CS_CMD_SHIFT);
            break;
