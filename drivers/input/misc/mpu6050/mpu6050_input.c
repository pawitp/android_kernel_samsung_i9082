...
static ssize_t mpu6050_acc_cal_open(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(accel_cal_open, 0444, mpu6050_acc_cal_open, NULL);
						...
						char *buf)
{
	...
	int hw_result;
        ...
        printk(KERN_INFO "[MPU6K] mpu6050_input_gyro_selftest_show : start");
	...
	printk(KERN_INFO "[MPU6K] gyro_selftest : result=%d, hw_result=%d, "
		       "%d.%03d,%d.%03d,%d.%03d,"
		       "%d.%03d,%d.%03d,%d.%03d,"
			"%d.%01d,%d.%01d,%d.%01d,"
		       "%d,%d,%d\n",
...
static int mpu6050_misc_open(struct inode *inode, struct file *file)
{
	return 0;
}

static int mpu6050_misc_release(struct inode *inode, struct file *file)
{
	return 0;
}
...
/*
static int
mpu6050_misc_ioctl(struct file *file,
...
static const struct file_operations mpu6050_misc_fops = {
	.owner = THIS_MODULE,
	.open = mpu6050_misc_open,
	.release = mpu6050_misc_release,
/*	.unlocked_ioctl = mpu6050_misc_ioctl,*/
};

static struct miscdevice mpu6050_misc_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "mpu6k",
	.fops = &mpu6050_misc_fops,
};
					...
					const struct i2c_device_id *id)
{
	...
	if (client->irq > 0) {
		...
		if (error < 0) {
			dev_err(&client->dev,
				"[MPU6K] irq request failed %d, error %d\n",
				client->irq, error);
	...
	error = misc_register(&mpu6050_misc_device);
	if (error)
