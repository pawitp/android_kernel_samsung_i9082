...
#define MPU6500_SENSOR_LP_SCR_ORIENT	(0x0100)
...
#define MPU6500_CALIB_FILENAME 	"//data//mpu6050.cal"
...
static struct mpu6500_input_data *mpu6500_input_data;
#if 0
...
#ifdef CONFIG_INPUT_MPU6500_LP
static void mpu6500_input_report_screen_orientation(
...
static irqreturn_t mpu6500_input_irq_thread(int irq, void *dev)
{
	...
	if (mpuirq_logcount++ > 51) {
		pr_info("mpu6500_input_irq_thread: every 50'th\n");
		mpuirq_logcount = 0;
	}
#endif	
	if (!mot_data->is_set) {
		...
		if (data->enabled_sensors & MPU6500_SENSOR_LP_SCR_ORIENT)
			mpu6500_input_report_screen_orientation(data);
		else {
		...
		timediff = (1000*(jiffies - data->motion_recg_st_time))/HZ;

		/* ignore motion interrupt happened in 100ms to skip intial erronous interrupt */
		if(timediff < 50 && !(data->factory_mode)) 
			return IRQ_HANDLED;

		printk(KERN_INFO " MPUREG_INT_STATUS=0x%x timediff=%ld(ms) \n ", reg, timediff);
        
		if (reg & (1<<6) || data->factory_mode) {
			//handle motion recognition
			printk(KERN_INFO " motion interrupt happended! \n ");
					      ...
					      int enable, bool factory_test)
{
	...
	if (enable) {
		...
		/* 3) set interrupt */
		reg = 0x40;	//Set motion interrupt
		mpu6500_i2c_write_single_reg(data->client,
					     MPUREG_INT_ENABLE, reg);

		/* 4) Enable Accelerometer Hardware Intelligence ** */
		reg = 0x01;
		mpu6500_i2c_write_single_reg(data->client,
		...
		if (factory_test)
			...
			reg = 0x10;
		mpu6500_i2c_write_single_reg(data->client, MPUREG_WOM_THR,
					     reg);

		reg = 0x04;
		mpu6500_i2c_write_single_reg(data->client, MPUREG_LP_ACCEL_ODR,
						reg);
		...
		reg = 0x21;	/* Set the cycle bit to be 1. LP Mode */
		mpu6500_i2c_write_single_reg(data->client,
					     MPUREG_PWR_MGMT_1,
					     reg);
...
static int mpu6500_input_actiave_accel(struct mpu6500_input_data *data,
				       bool enable)
{
	int result;
...
static int mpu6500_set_delay(struct mpu6500_input_data *data)
{
	...
	if (!mot_data->is_set) {
		CHECK_RESULT(mpu6500_input_set_odr(data, delay));
	...
	if (data->enabled_sensors) {
		...
		if (atomic_read(&data->motion_recg_enable))
			mpu6500_input_set_motion_interrupt(data, true,
				mpu6500_input_data->factory_mode);
		else {
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void mpu6500_input_early_suspend(struct early_suspend *h)
{
	struct mpu6500_input_data *data = container_of(h,
						       struct
						       mpu6500_input_data,
						       early_suspend);
	mutex_lock(&data->mutex);

	data->sleep_sensors = data->enabled_sensors;
	mpu6500_input_activate_devices(data, data->enabled_sensors, false);

	mutex_unlock(&data->mutex);
}

static void mpu6500_input_early_resume(struct early_suspend *h)
{
	struct mpu6500_input_data *data = container_of(h,
						       struct
						       mpu6500_input_data,
	...
	mpu6500_input_activate_devices(data, data->sleep_sensors, true);
	data->sleep_sensors = 0;
					      ...
					      *cfg)
{
	...
	data->early_suspend.suspend = mpu6500_input_early_suspend;
	data->early_suspend.resume = mpu6500_input_early_resume;
...
static int accel_open_calibration(void)
{
	...
	cal_filp = filp_open(mpu6500_input_data->pdata->acc_cal_path,
		O_RDONLY, S_IRUGO | S_IWUSR | S_IWGRP);
	if (IS_ERR(cal_filp)) {
		pr_err("[MPU6K] %s: Can't open calibration file\n", __func__);
		set_fs(old_fs);
	...
	err = cal_filp->f_op->read(cal_filp,
		(char *)&mpu6500_input_data->acc_cal,
			3 * sizeof(s16), &cal_filp->f_pos);
	if (err != 3 * sizeof(s16)) {
		pr_err("[MPU6K] %s: Can't read the cal data from file\n", __func__);
		err = -EIO;
	}

	pr_info("[MPU6K] %s: (%d,%d,%d)\n", __func__,
		mpu6500_input_data->acc_cal[0], mpu6500_input_data->acc_cal[1],
			mpu6500_input_data->acc_cal[2]);
						...
						const char *buf, size_t count)
{
	...
	int value = simple_strtoul(buf, NULL, 10);
        ...
        printk(KERN_INFO "[MPU6K] mpu6500_input_accel_enable_store: value=%d\n", value);   

	mutex_lock(&data->mutex);

	atomic_set(&data->accel_enable, value);

	mpu6500_input_activate_devices(data, MPU6500_SENSOR_ACCEL,
				       (value == 1) ? true : false);
#ifdef CONFIG_INPUT_MPU6500_POLLING
					       ...
					       const char *buf, size_t count)
{
	...
	int value = simple_strtoul(buf, NULL, 10);
        ...
        printk(KERN_INFO "[MPU6K] mpu6500_input_accel_delay_store: value=%d\n", value);   
...
static int gyro_open_calibration(void)
{
	...
	cal_filp = filp_open(mpu6500_input_data->pdata->gyro_cal_path,
		O_RDONLY, S_IRUGO | S_IWUSR | S_IWGRP);
	if (IS_ERR(cal_filp)) {
	...
	err = cal_filp->f_op->read(cal_filp,
		(char *)&mpu6500_input_data->gyro_bias, 3 * sizeof(int),
			&cal_filp->f_pos);
	if (err != 3 * sizeof(int)) {
	...
	pr_info("%s: (%d,%d,%d)\n", __func__,
		mpu6500_input_data->gyro_bias[0],
		mpu6500_input_data->gyro_bias[1],
		mpu6500_input_data->gyro_bias[2]);
					       ...
					       const char *buf, size_t count)
{
	...
	int value = simple_strtoul(buf, NULL, 10);
        ...
        printk(KERN_INFO "[MPU6K] mpu6500_input_gyro_enable_store: value=%d\n", value);   
					      ...
					      const char *buf, size_t count)
{
	...
	int value = simple_strtoul(buf, NULL, 10);
        ...
        printk(KERN_INFO "[MPU6K] mpu6500_input_gyro_delay_store: value=%d\n", value);   
...
static ssize_t mpu6500_acc_cal_open(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(accel_cal_open, 0444, mpu6500_acc_cal_open, NULL);
...
static int read_accel_raw_xyz(s16 *x, s16 *y, s16 *z)
{
	...
	result =
		mpu6500_i2c_read_reg(mpu6500_input_data->client,
	...
	do {
		orien_raw[i] = mpu6500_input_data->pdata->orientation[i*3]
				* raw[0]
			+ mpu6500_input_data->pdata->orientation[i*3+1]
				* raw[1]
			+ mpu6500_input_data->pdata->orientation[i*3+2]
				* raw[2];
	} while (i-- != 0);
...
static int read_accel_raw_xyz_cal(s16 *x, s16 *y, s16 *z)
{
	...
	s16 raw_y;
	s16 raw_z;
	if (!(mpu6500_input_data->enabled_sensors & MPU6500_SENSOR_ACCEL)) {
		mpu6500_input_resume_accel(mpu6500_input_data);
		usleep_range(10000, 11000);
	}

	read_accel_raw_xyz(&raw_x, &raw_y, &raw_z);
	*x = raw_x - mpu6500_input_data->acc_cal[0];
	*y = raw_y - mpu6500_input_data->acc_cal[1];
	*z = raw_z - mpu6500_input_data->acc_cal[2];

	if (!(mpu6500_input_data->enabled_sensors & MPU6500_SENSOR_ACCEL)) {
		mpu6500_input_suspend_accel(mpu6500_input_data);
		usleep_range(10000, 11000);
	}
	return 0;
}
...
static int accel_do_calibrate(int enable)
{
	...
	if (enable) {
		mpu6500_input_data->acc_cal[0] =
			(sum[0] / ACC_CAL_TIME) * ACC_CAL_DIV;
		mpu6500_input_data->acc_cal[1] =
			(sum[1] / ACC_CAL_TIME) * ACC_CAL_DIV;
		mpu6500_input_data->acc_cal[2] =
			((sum[2] / ACC_CAL_TIME) - ACC_IDEAL) * ACC_CAL_DIV;
	} else {
		mpu6500_input_data->acc_cal[0] = 0;
		mpu6500_input_data->acc_cal[1] = 0;
		mpu6500_input_data->acc_cal[2] = 0;
	}
	...
	err = cal_filp->f_op->write(cal_filp,
		(char *)&mpu6500_input_data->acc_cal, 3 * sizeof(s16),
			&cal_filp->f_pos);
	if (err != 3 * sizeof(s16)) {
		pr_err("%s: Can't write the cal data to file\n", __func__);
		err = -EIO;
...
{
	pr_info("%s: state =%d\n", __func__,
		atomic_read(&mpu6500_input_data->reactive_state));
	return sprintf(buf, "%d\n",
		atomic_read(&mpu6500_input_data->reactive_state));
}
							...
							size_t count)
{
	...
	if (value == 1) {
		...
		onoff = true;
		factory_test = true;
		mpu6500_input_data->factory_mode = true;
	} else {
...
#ifdef CONFIG_INPUT_MPU6500_POLLING
	if (!value) {
		disable_irq_wake(mpu6500_input_data->client->irq);
		disable_irq(mpu6500_input_data->client->irq);
	} else {
		enable_irq(mpu6500_input_data->client->irq);
		enable_irq_wake(mpu6500_input_data->client->irq);
	}
#endif

	mutex_lock(&mpu6500_input_data->mutex);
	atomic_set(&mpu6500_input_data->reactive_state, false);
	atomic_set(&mpu6500_input_data->motion_recg_enable, onoff);

	mpu6500_input_set_motion_interrupt(mpu6500_input_data, onoff,
							factory_test);
	mutex_unlock(&mpu6500_input_data->mutex);

	pr_info("%s: onoff = %d, state =%d\n", __func__,
		atomic_read(&mpu6500_input_data->motion_recg_enable),
		atomic_read(&mpu6500_input_data->reactive_state));
	return count;
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	dev_dbg(dev, "this_client = %d\n", (int)mpu6500_input_data->client);
	count = sprintf(buf, "%d\n",
		(mpu6500_input_data->client != NULL ? 1 : 0));
				...
				struct device_attribute *attr, char *buf)
{
	...
	CHECK_RESULT(mpu6500_i2c_read_reg
		(mpu6500_input_data->client, MPUREG_TEMP_OUT_H, 2, reg));
...
static ssize_t mpu6500_input_gyro_selftest_show(struct device *dev,
						 ...
						 char *buf)
{
	struct mpu6500_input_data *data = mpu6500_input_data;
	...
	printk(KERN_INFO "[MPU6K] gyro_selftest : %d,"
		       "%d.%03d,%d.%03d,%d.%03d,"
		       "%d.%03d,%d.%03d,%d.%03d,"
		       "%d,%d,%d\n",
		       result,
					...
					char *buf)
{
	...
	x = mpu6500_input_data->acc_cal[0];
	y = mpu6500_input_data->acc_cal[1];
	z = mpu6500_input_data->acc_cal[2];
	...
	if (client->irq > 0) {
		...
		if (error) {
			...
			goto err_request_irq_failed;
                   ...
                   printk(KERN_INFO "[MPU6K] request_threaded_irq success IRQ_NO:%d", client->irq);
		}
#ifdef CONFIG_INPUT_MPU6500_POLLING
...
static int mpu6500_input_suspend(struct device *dev)
{
	...
	if (!atomic_read(&data->motion_recg_enable)) {
		...
		disable_irq(client->irq);
#endif
		ret = mpu6500_input_set_mode(data, MPU6500_MODE_SLEEP);
	}
...
static int mpu6500_input_resume(struct device *dev)
{
	...
	if (!atomic_read(&data->motion_recg_enable)) {
		ret = mpu6500_input_set_mode(data, MPU6500_MODE_NORMAL);
#ifdef CONFIG_INPUT_MPU6500_POLLING
		if (atomic_read(&data->accel_enable))
			schedule_delayed_work(&data->accel_work, 0);
		if (atomic_read(&data->gyro_enable))
