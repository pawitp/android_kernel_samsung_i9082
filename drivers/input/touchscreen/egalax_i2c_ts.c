...
#define X_ORIG 1366
#define Y_ORIG 768
static volatile int g_x_full = X_ORIG;
static volatile int g_y_full = Y_ORIG;
...
#define X_SCALED(x, x_new)  ((x_new) ? ((((x) - X_OFF(x_new)) * g_x_full) / (x_new)) : (x))
#define Y_SCALED(y, y_new)  ((y_new) ? ((((y) - Y_OFF(y_new)) * g_y_full) / (y_new)) : (y))
...
{
   volatile enum rotation_angle rotate;
   volatile int flip_x;
   volatile int flip_y;
   volatile int swap_xy;
};

/* touch controller power state */
enum pm_state {
	/* currently running */
	PM_STATE_RUNNING,

	/* sleep mode, have very small current leakage (<= 300 uA) */
	PM_STATE_SLEEP,

	/* completely turned off, no current leakage */
	PM_STATE_OFF,
};
...
#ifdef CONFIG_TOUCHSCREEN_SCALE
   if ( !((g_x_new == g_x_full) && (g_y_new == g_y_full)) && !((g_x_new == 0) && (g_y_new == 0)) )
   {
...
static int egalax_i2c_measure(struct i2c_client *client)
{
   ...
   {
      TS_INFO("received command packet: "
...
static void egalax_i2c_reset_wq_func(struct work_struct *work)
{
   ...
   {
      printk(KERN_ERR "%s() egalax_i2c == NULL\n", __func__);
      return;
   }
   ...
   if (rc != 0)
   {
      printk("%s() egalax_probe_part2() returned %d\n", __func__, rc);
   }
}
      ...
      int *eof, void *data)
{
   ...
   len += sprintf(buffer + len, "Current screen rotation configuration is %u degree clockwise\n",
      ...
      unsigned long count, void *data)
{
   ...
   {
      printk(KERN_ERR "echo <rotate> > /proc/%s/%s\n",
   ...
   {
      printk(KERN_ERR "Rotation index invalid. Use a value between 0 ~ %d\n", ROTATE_MAX - 1);
      return count;
   }
   ...
   printk(KERN_INFO "New rotation configuration is %u degree clockwise flip_x=%d flip_y=%d swap_xy=%d\n",
         rotate_string[ctrl->rotate], ctrl->flip_x, ctrl->flip_y, ctrl->swap_xy);
      ...
      unsigned long count, void *data)
{
   ...
   {
      printk(KERN_ERR "echo <x-full-screen> > /proc/%s/%s\n",
      ...
      unsigned long count, void *data)
{
   ...
   {
      printk(KERN_ERR "echo <y-full-screen> > /proc/%s/%s\n",
...
static int proc_init(struct _egalax_i2c *egalax_i2c)
{
   ...
   int rc;
   struct proc_dir_entry *proc_debug, *proc_rotate, *proc_cmd, *proc_wakeup;
#ifdef CONFIG_TOUCHSCREEN_SCALE
   ...
   proc_x_full_screen = create_proc_entry(PROC_ENTRY_X_FULL_SCREEN, 0644, proc->parent);
   ...
   proc_y_full_screen = create_proc_entry(PROC_ENTRY_Y_FULL_SCREEN, 0644, proc->parent);
...
{
	struct _egalax_i2c *dev = i2c_get_clientdata(client);

   if (g_is_hw_detected == 0)
      return 0;

	/* already resumed */
	if (dev->state == PM_STATE_RUNNING)
		return 0;

	enable_irq(client->irq);

	if (dev->state == PM_STATE_SLEEP) {
		wakeup_controller(irq_to_gpio(client->irq));
	} else {
		if (dev->hw_cfg.gpio.reset >= 0) {
			u8 x_buf[MAX_I2C_LEN];
			
			gpio_set_value(dev->hw_cfg.gpio.reset, 1);
			msleep(100);
			i2c_master_recv(client, x_buf, MAX_I2C_LEN);
		}
	}

	dev->state = PM_STATE_RUNNING;
...
{
	int rc;
	struct _egalax_i2c *dev = p_egalax_i2c_dev;
...
static int egalax_i2c_suspend(struct i2c_client *client, pm_message_t mesg)
{
	int rc;
	struct _egalax_i2c *dev = p_egalax_i2c_dev;
	enum pm_state prev_state = dev->state;

   if (g_is_hw_detected == 0)
      return 0;

	/* need to know what the current state is */
	switch (dev->state)
	{
		case PM_STATE_RUNNING:
			dev->state = PM_STATE_OFF;
	...
	/* completely turn off power if possible */
	if (dev->hw_cfg.gpio.reset >= 0) {
		gpio_set_value(dev->hw_cfg.gpio.reset, 0);
		msleep(500);
	} else if (prev_state == PM_STATE_RUNNING) {
		/*
		 * power pin not available, if the controller is in a running
		 * state, put it into sleep
		 */
		rc = i2c_master_send(client, cmd_str_sleep, MAX_I2C_LEN);
		if (rc != MAX_I2C_LEN) {
      ...
      const struct i2c_device_id *id)
{
   ...
   p_egalax_i2c_dev->state = PM_STATE_RUNNING;
   ...
   if (cfg->gpio.reset >= 0)
   {
      ...
      {  /* do not queue so probe result can be reported */
         msleep(p_egalax_i2c_dev->hw_cfg.reset_time);
         ret = egalax_probe_part2(p_egalax_i2c_dev);
      }
      else
      {  /* slow reset, have to queue so start up is not affected */
