...
#define PMU_BC_DETECT_TYPE_MSK	0x7F
...
static int is_detection_done(struct bcmpmu_accy *paccy, unsigned int status)
{
	...
	if ((paccy->bc == BCMPMU_BC_BB_BC11) ||
		...
		if (status & BB_BC_STS_BC_DONE_MSK) {
			...
			u32 type = status & PMU_BC_DETECT_TYPE_MSK;
...
void set_usb_connection_status(void *para)
{
	...
	if ((usb_type < PMU_USB_TYPE_MAX) &&
	    ...
	    pr_accy(FLOW, "%s, usb_type %d changed to %d\n", __func__, 
			bcmpmu->usb_accy_data.usb_type, usb_type);
		bcmpmu->usb_accy_data.usb_type = usb_type;
	} else {
		pr_accy(FLOW, "%s, usb_type %d cannot be changed to %d\n", __func__, 
			bcmpmu->usb_accy_data.usb_type, usb_type);
	}
}
EXPORT_SYMBOL_GPL(set_usb_connection_status);
...
{
	struct bcmpmu_accy *paccy =  bcmpmu_accy;
	send_usb_event(paccy->bcmpmu, BCMPMU_USB_EVENT_ID_CHANGE, NULL);
}
EXPORT_SYMBOL_GPL(send_otg_insert_event);
//dh0318.lee USB_DETACH
void send_USB_remove_event()
{
	struct bcmpmu_accy *paccy =  bcmpmu_accy;
	send_usb_event(paccy->bcmpmu, BCMPMU_USB_EVENT_SESSION_INVALID, NULL);
}
EXPORT_SYMBOL_GPL(send_USB_remove_event);
...
static char *get_supply_type_str(int chrgr_type)
{
	...
	default:
		pr_info("[bcmpmu-accy][%s]: chrgr_type unexpected %d\n", __func__, chrgr_type);
		break;
	}
...
void send_uart_jig_insert_event(bool attached)
{
	...
	struct bcmpmu *bcmpmu = paccy->bcmpmu;
	int vbus_status;
	enum bcmpmu_chrgr_type_t chrgr_type = PMU_CHRGR_TYPE_NONE;
	int ret;
	...
	paccy->uart_jig_attached = attached;

	if (paccy->uart_jig_attached != true) {
...
EXPORT_SYMBOL_GPL(send_uart_jig_insert_event);

static void update_uart_jig_charger_event(bool attached)
{
#if defined(CONFIG_SEC_CHARGING_FEATURE)
	struct bcmpmu_accy *paccy = bcmpmu_accy;
	enum bcmpmu_chrgr_type_t chrgr_type;
...
static ssize_t curr_lmt_show(struct device *dev,
				...
				char *buf)
{
	...
	ssize_t count;
	int i;
	count = snprintf(buf, 20, "Curr lmt tbl\n");
	for (i = 0; i < PMU_CHRGR_TYPE_MAX; i++)
		count += snprintf(buf + count, 20, "chrgr=%d, curr=%d\n",
			i, paccy->chrgr_curr_lmt[i]);
	return count;
}
...
static DEVICE_ATTR(curr_lmt, 0644, curr_lmt_show, NULL);
...
static void set_chrgr_type(struct bcmpmu_accy *paccy, enum bcmpmu_chrgr_type_t chrgr_type)
{
	struct bcmpmu *bcmpmu = paccy->bcmpmu;
	...
	if ((paccy->bc == BCMPMU_BC_BB_BC11) ||
		...
		bcmpmu->write_dev(bcmpmu,
			...
			(chrgr_type << bcmpmu->regmap
			  [PMU_REG_MBCCTRL5_CHRGR_TYPE].shift),
			bcmpmu->regmap
			  [PMU_REG_MBCCTRL5_CHRGR_TYPE].mask);
	}
}
...
//ENABLE_OTG
	...
	case BCMPMU_USB_CTRL_SET_ROTG_VBUS:
		if (paccy->otg_block_enabled == 0) {
			ret = bcmpmu->write_dev(bcmpmu,
					PMU_REG_OTG_ENABLE,
					bcmpmu->
		...
		ret =
		    bcmpmu->write_dev(bcmpmu, PMU_REG_OTG_ROTG_VBUS,
				      data ? bcmpmu->
				      regmap[PMU_REG_OTG_ROTG_VBUS].
				      mask : 0,
				      bcmpmu->
				      regmap[PMU_REG_OTG_ROTG_VBUS].
				      mask);
	...
	case BCMPMU_USB_CTRL_SET_SESS_COMP:
		ret = bcmpmu->write_dev(bcmpmu,
	...
	case BCMPMU_USB_CTRL_SET_VBUS_VALID_COMP:
		ret = bcmpmu->write_dev(bcmpmu,
	...
	case BCMPMU_USB_CTRL_SET_OTG_ENABLE:
		if (data) {
			/* OTG is enabled so make sure the required
			 * bits are set
			 */
			ret =
			    bcmpmu->write_dev(bcmpmu,
			...
			ret = bcmpmu->write_dev(bcmpmu,
					PMU_REG_OTG_VBUS_VALID_COMP,
					bcmpmu->
					regmap[PMU_REG_OTG_VBUS_VALID_COMP].
					mask,
					bcmpmu->
					regmap[PMU_REG_OTG_VBUS_VALID_COMP].
					mask);
			...
			ret =
			    bcmpmu->write_dev(bcmpmu, PMU_REG_OTG_ROTG_VBUS,
					      ...
					      regmap[PMU_REG_OTG_ROTG_VBUS].
					      mask);
			bcmpmu->otg_enabled = true;
		} else {
			/* Just set OTG var to false */
...
static int __devinit bcmpmu_accy_probe(struct platform_device *pdev)
{
	...
	if (pdata->chrgr_curr_lmt)
		...
		paccy->chrgr_curr_lmt = &chrgr_curr_lmt[0];
	...
	bcmpmu->register_irq(bcmpmu, PMU_IRQ_UBPD_CHG_F, bcmpmu_accy_isr, paccy);
	...
	bcmpmu->unmask_irq(bcmpmu, PMU_IRQ_UBPD_CHG_F);
	...
	/* Enable ROTG_VBUS for Vbus discharge timing
	 * This is used just in case the DTS file has this
	 * disabled
	 */
	ret =
	    bcmpmu->write_dev(bcmpmu, PMU_REG_OTG_ROTG_VBUS,
...
static int __devexit bcmpmu_accy_remove(struct platform_device *pdev)
{
	...
	bcmpmu->unregister_irq(bcmpmu, PMU_IRQ_UBPD_CHG_F);
