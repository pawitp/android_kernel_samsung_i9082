				...
				const char *buf, size_t count)
{
	...
	if ((map < 2) && ((addr + len) < 255) && (len < 16)) {
		...
		for (i = 0; i < len; i++) {
			...
			/* Always print this value when requested */
...
static int __devinit bcmpmu_probe(struct platform_device *pdev)
{
	...
	ret = bcmpmu->read_dev_drct(bcmpmu, 0, 0x51, &val, 0x1f);
	if (ret < 0) {
		dev_err(bcmpmu->dev,
			 "Failed to read bcmpmu revision ID: %d\n", ret);
		goto err;
	...
	bcmpmu->write_dev_drct(bcmpmu, 0, 0x51, val, 0x1F);
#endif
	...
	if (ret) {
		dev_err(bcmpmu->dev, "Add fellow devices failed: %d\n",
			ret);
			goto err1;
	}
