			    ...
			    int count, int *peof, void *dat)
{
	...
	rc = ctx->mtd->read(ctx->mtd,
			    ...
			    ctx->mtd->writesize, &len, ctx->bounce);
...
static void mtd_panic_erase(void)
{
	...
	for (i = 0; i < ctx->mtd->size; i += ctx->mtd->erasesize) {
		...
		if (get_bb(erase.addr >> ctx->mtd->erasesize_shift,
					apanic_bbt)) {
			printk(KERN_WARNING
...
static void mtd_panic_notify_add(struct mtd_info *mtd)
{
	...
	rc = mtd->read(mtd, phy_offset(mtd, 0), mtd->writesize,
		       &len, ctx->bounce);
	if (rc && rc == -EBADMSG) {
		printk(KERN_WARNING "apanic: Bad ECC on block 0 (ignored)\n");
...
static int apanic_write_console_mmc(unsigned long off)
{
	...
	while (!last_chunk) {
		...
		rc2 =
		    ctx->mmc->block_dev.block_write(ctx->dev_num, off, 1,
						    ctx->bounce);
		if (rc2 <= 0) {
			printk(KERN_EMERG
...
#ifdef APANIC_DEBUG
	printk
	    ("apanic_console_write_mmc: wrote %d pages from %d returned %d\r\n",
	     num, start, off);
#endif
	return num;
}
...
static int apanic(struct notifier_block *this, unsigned long event, void *ptr)
{
	...
	if (console_len <= 0) {
		printk(KERN_EMERG
		       "Error writing log but to panic log! (%d) \r\n",
		       console_len);
		console_len = 0;
	}
	/*
