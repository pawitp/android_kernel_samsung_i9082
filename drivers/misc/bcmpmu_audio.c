...
/*
 * Steps to increment IHF/HS gain linearly
 */
...
{
	struct bcmpmu_rw_data reg1, reg2, reg3;

	bcmpmu_audio->bcmpmu->read_dev(bcmpmu_audio->bcmpmu,
		PMU_REG_HSPGA3_GAIN, &reg3.val, PMU_BITMASK_ALL);
	bcmpmu_audio->bcmpmu->read_dev(bcmpmu_audio->bcmpmu,
		PMU_REG_HSPUP1_IDDQ_PWRDWN, &reg1.val, PMU_BITMASK_ALL);
	bcmpmu_audio->bcmpmu->read_dev(bcmpmu_audio->bcmpmu,
		PMU_REG_HSPUP2_HS_PWRUP, &reg2.val, PMU_BITMASK_ALL);

	if (on) {
		reg1.val &= ~BCMPMU_HSPUP1_IDDQ_PWRDN;  /*HSPUP1*/
		reg2.val |= BCMPMU_HSPUP2_HS_PWRUP;     /*HSPUP2*/
		reg3.val |= BCMPMU_HSPGA3_PGA_RAMPDIS;	/*HSPGA3*/
...
void bcmpmu_hs_set_gain(bcmpmu_hs_path_t path, u32 gain)
{
	...
	if (path == PMU_AUDIO_HS_LEFT || path == PMU_AUDIO_HS_BOTH)
		bcmpmu_set_gain(&ramp, PMU_REG_HSPGA1_GAIN, gain);

	if (path == PMU_AUDIO_HS_RIGHT || path == PMU_AUDIO_HS_BOTH)
		bcmpmu_set_gain(&ramp, PMU_REG_HSPGA2_GAIN, gain);
...
static int __devinit bcmpmu_audio_probe(struct platform_device *pdev)
{
	...
	/* Enable auto sequence for IHF power up and power down */
	bcmpmu_audio->bcmpmu->read_dev(bcmpmu_audio->bcmpmu,
		PMU_REG_IHFPOP_PUP, &reg.val, PMU_BITMASK_ALL);
