...
#define YAS_ERROR_UNDERFLOW (0)
#define YAS_ERROR_OVERFLOW (0)
		       ...
		       unsigned char *data)
{
	...
	short xyz_scaled[3] = {0,};
	 ...
	 geomagnetic_api_read(xyz, raw, xy1y2, NULL);
	...
	data[5] = xyz_scaled[2] & 0xFF;
	 ...
	 { 
		  ...
		  if( s32Tmp <= 0 ) 
		  { 
		   accuracy = YAS_ERROR_UNDERFLOW; 
		  } 
		  else if( YAS_YAS532_DATA_OVERFLOW <= s32Tmp )   
		  { 
		          accuracy = YAS_ERROR_OVERFLOW; 
		  } 
        } 
