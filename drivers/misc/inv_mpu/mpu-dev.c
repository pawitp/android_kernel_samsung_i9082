...
static int accel_open_calibration(void)
{
	...
	if (IS_ERR(cal_filp)) {
		pr_err("[MPUDEV]%s: Can't open calibration file\n", __func__);
		set_fs(old_fs);
	...
	if (err != 3 * sizeof(s16)) {
		pr_err("[MPUDEV]%s: Can't read the cal data from file\n", __func__);
		err = -EIO;
}

	printk(KERN_INFO"[MPUDEV]%s: (%d,%d,%d)\n", __func__,
...
static int accel_do_calibrate(int enable)
{
	...
	for (i = 0; i < CALIBRATION_DATA_AMOUNT; i++) {
		...
		if (err < 0) {
			pr_err("[MPUDEV]%s: accel_read_accel_raw_xyz() "
	...
	printk(KERN_INFO "[MPUDEV]%s: cal data (%d,%d,%d)\n", __func__,
			cal_data.x, cal_data.y, cal_data.z);
	...
	err = cal_filp->f_op->write(cal_filp,
		(char *)&cal_data, 3 * sizeof(s16), &cal_filp->f_pos);
	if (err != 3 * sizeof(s16)) {
		pr_err("[MPUDEV]%s: Can't write the cal data to file\n", __func__);
		err = -EIO;
	}
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	case MPU_ACTIVATE_SENSORS:
        	//printk(KERN_INFO "[MPUDEV]%s : MPU_ACTIVATE_SENSORS\n",__func__);
		mpu_dev_ioctl_activate_sensors(mpu);
		break;
	case MPU_IOCTL_SET_CALIBRATION:
		printk(KERN_INFO "[MPUDEV] MPU_IOCTL_SET_CALIBRATION\n");                
		if (copy_from_user(&data, (void __user *)arg, sizeof(data)))
			return -EFAULT;
                accel_open_calibration();                
		break;
	case MPU_IOCTL_GET_CALIBRATION:
		printk(KERN_INFO "[MPUDEV] MPU_IOCTL_SET_CALIBRATION\n");      
            	data.x = cal_data.x;
            	data.y = cal_data.y;
            	data.z = cal_data.z;
		if (copy_to_user((void __user *)arg, &data, sizeof(data)))
			return -EFAULT;
		break;
	case MPU_IOCTL_DO_CALIBRATION:
		printk(KERN_INFO "[MPUDEV] MPU_IOCTL_DO_CALIBRATION\n");      
                accel_do_calibrate(1);
            	data.x = cal_data.x;
            	data.y = cal_data.y;
            	data.z = cal_data.z;
		if (copy_to_user((void __user *)arg, &data, sizeof(data)))
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (mldl_cfg->inv_mpu_state->use_accel_reactive && 
		!mldl_cfg->inv_mpu_state->accel_reactive) {
        	printk(KERN_INFO"[MPUDEV] %s : %d\n", __func__, mldl_cfg->inv_mpu_state->accel_reactive);                		
...
static int __init mpu_init(void)
{
	...
	if(IS_ERR(VCC_PDA_2_8_V)){
		printk(KERN_ERR "[MPUDEV] can not get VCC_PDA_2.8V\n");
	}	

        ret = regulator_is_enabled(VCC_PDA_2_8_V);
        printk(KERN_INFO "[MPUDEV] regulator_is_enabled : %d\n", ret);

         ret = regulator_set_voltage(VCC_PDA_2_8_V,2800000,2800000);	
        printk(KERN_INFO "[MPUDEV] regulator_set_voltage : %d\n", ret);

        ret = regulator_enable(VCC_PDA_2_8_V);
        printk(KERN_INFO "[MPUDEV] regulator_enable : %d\n", ret);
