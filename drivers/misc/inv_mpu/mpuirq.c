...
int mpuirq_init(struct i2c_client *mpu_client, struct mldl_cfg *mldl_cfg)
{
	...
	printk(KERN_INFO"[MPUDEV] %s entered!\n", __func__);
		...
		if (res) {
			dev_err(&mpu_client->adapter->dev,
				...
				mpuirq_dev_data.irq);
                        ...
                        printk(KERN_INFO "[MPUDEV] request_threaded_irq success IRQ_NO:%d, GPIO:%d", 
                            mpuirq_dev_data.irq, irq_to_gpio(mpuirq_dev_data.irq));
