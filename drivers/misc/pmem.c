...
static bool has_allocation(struct file *file)
{
	...
	if (unlikely(!file->private_data)) {
		printk(KERN_ERR "has_allocation .. private_data: %x!!\n",
				(u32)file->private_data);
		return false;
	}
	data = (struct pmem_data *)file->private_data;
	if (unlikely(data->pfn == -1UL)) {
		printk(KERN_ERR "has_allocation .. data->pfn == -1UL!!\n");
		return false;
	}
	return true;
}
...
lock_mm:
	...
	if (PMEM_IS_SUBMAP(data)) {
		...
		if (!mm) {
			printk(KERN_ERR "pmem: can't remap task is gone!\n");
			up_read(&data->sem);
			return -1;
		}
	}
			 ...
			 unsigned long len)
{
	...
	printk(KERN_INFO "carveout_base (%x) f_flags:(%x)\n",
			(u32)pmem[id].carveout_base, file->f_flags);
	file->f_flags |= O_NONBLOCK;
	if (pmem[id].carveout_base &&
		(file->f_flags & O_SYNC) &&
		(file->f_flags & O_NONBLOCK)) {
		printk(KERN_INFO "carveout heap:gen_pool_alloc(%ld)\n", len);
...
static int pmem_connect(unsigned long connect, struct file *file)
{
	...
	if (has_allocation(file) && (data->pfn != src_data->pfn)) {
		printk(KERN_ERR "pmem: file is already mapped but doesn't match this"
		       " src_file!\n");
		ret = -EINVAL;
		goto err_bad_file;
	}
	       ...
	       unsigned operation)
{
	...
	if (unlikely(!PMEM_IS_PAGE_ALIGNED(region->offset) ||
		...
		printk(KERN_ERR "pmem: request for unaligned pmem suballocation "
	...
	if (operation == PMEM_MAP) {
		...
		if (!region_node) {
			ret = -ENOMEM;
			printk(KERN_ERR "No space to allocate metadata!");
			goto err;
		}
		...
		if (!found) {
			printk(KERN_ERR "pmem: Unmap region does not map "
					"any mapped region!");
			ret = -EINVAL;
			goto err;
		}
...
static void pmem_vma_open(struct vm_area_struct *vma)
{
	...
	printk(KERN_INFO "pmem_vma_open ..");
	BUG_ON(!has_allocation(file));
...
static void pmem_vma_close(struct vm_area_struct *vma)
{
	...
	printk(KERN_INFO "pmem_vma_close: "
		"current %u ppid %u file %p count %ld\n",
...
static int pmem_open(struct inode *inode, struct file *file)
{
	...
	printk(KERN_INFO "pmem_open: current %u file %p(%ld)\n",
	...
	if (!data) {
		printk(KERN_ERR "pmem: unable to allocate memory for "
				"pmem metadata.");
		return -ENOMEM;
	}
...
static int pmem_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (!vma) {
		printk(KERN_ERR "The vm_area_struct can not be NULL\n");
		return -EINVAL;
	}
	...
	if (likely(!has_allocation(file))) {
		...
		printk(KERN_INFO"pmem_mmap:FILE is not allocated: 0x%x\n",
		       (u32)file);
	}
...
static long pmem_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case PMEM_UNMAP:
		...
		break;
	case PMEM_GET_SIZE:
		printk(KERN_INFO "pmem .. get_size\n");
		if (!has_allocation(file))
		{
			printk(KERN_ERR "pmem .. has_allocation failed\n");
			return -EINVAL;
		}
		data = (struct pmem_data *)file->private_data;
		down_write(&data->sem);
	...
	case PMEM_GET_TOTAL_SIZE:
		printk(KERN_INFO "get total size\n");
		region.offset = 0;
#ifdef PMEM_CMA
	       ...
	       struct android_pmem_platform_data *pdata)
{
	...
	printk(KERN_INFO "pmem_setup carveout_base:%x  carveout_size:%d\n",
			pdata->carveout_base, pdata->carveout_size);
	...
	if (pdata->carveout_base && pdata->carveout_size) {
		...
		if (!pmem[id].carveout_vbase) {
			...
			printk(KERN_INFO "pmem_setup carveout: gen_pool: %x\n",
					(u32)pmem[id].pool);
			if (!pmem[id].pool) {
				printk(KERN_ERR
				...
				printk(KERN_INFO
					"carveout_base: %x carveout_size: %u\n",
					(u32)pmem[id].carveout_base,
					pmem[id].carveout_size);
			}
		}
	}
...
#else
	printk(KERN_INFO "Pmem initialised with %hu hwm pages (%s)\n",
			pmem[id].hwm, pdata->name);
#endif
	return 0;
