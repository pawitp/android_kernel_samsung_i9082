...
#ifdef CONFIG_ARCH_CAPRI
// bring up code only for Capri STM which will be addressed in formal form later. 
static void capri_tracepad_init(void)
{
        // final funnel
	BRCM_WRITE_REG(KONA_FIN_FUNNEL_VA, CSTF_FUNNEL_CONTROL, 0x122);
	
	// STM config
	BRCM_WRITE_REG(KONA_STM_VA, ATB_STM_CONFIG, 0x122);
	BRCM_WRITE_REG(KONA_STM_VA, ATB_STM_EN_LO, 0xFFFFFFFF);
	BRCM_WRITE_REG(KONA_STM_VA, ATB_STM_EN_HI, 0xFFFFFFFF);
	BRCM_WRITE_REG(KONA_STM_VA, ATB_STM_SW_LO, 0xFFFFFFFF);
	BRCM_WRITE_REG(KONA_STM_VA, ATB_STM_SW_HI, 0xFFFFFFFF);
}
#endif
