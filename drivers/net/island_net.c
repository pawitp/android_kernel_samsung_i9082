...
static void ethStatusWork(struct work_struct *work)
{
	...
	static uint32_t lanPortSpeedPrev = -1;
	static uint32_t pcPortSpeedPrev = -1;
	...
	int change;
	uint32_t impSpeed;
	uint32_t lanPortSpeed;
	uint32_t pcPortSpeed;
	uint32_t reg32;
...
#else
		ethHw_impSpeedSet(impSpeed, impSpeed);
#endif
	...
	port = PORT_LAN;
	lanPortSpeed = ethHw_portSpeed(port);
	if (lanPortSpeed != lanPortSpeedPrev)
	{
		if (lanPortSpeed == 1000)
		{
			...
			msg_printk(MSG_LEVEL_STATUS_VERBOSE, "LAN port at 1000Mbps, disabling RGMII Tx delay\n");
		}
		else
		{
			...
			ethHw_miiSet( port, ETHHW_MII_FLAGS_EXT, 0x1c, reg32 );
			msg_printk(MSG_LEVEL_STATUS_VERBOSE, "LAN port at 10/100Mbps, enabling RGMII Tx delay\n");
		}

		lanPortSpeedPrev = lanPortSpeed;
	}

	port = PORT_PC;
	pcPortSpeed = ethHw_portSpeed(port);
	if (pcPortSpeed != pcPortSpeedPrev)
	{
		if (pcPortSpeed == 1000)
		{
			/* Disable the RGMII transmit timing delay on the external PHY */
			...
			reg32 &= ~0x0200; /* turn off bit 9 to disable RGMII Tx delay */
			ethHw_miiSet( port, ETHHW_MII_FLAGS_EXT, 0x1c, reg32 );
			msg_printk(MSG_LEVEL_STATUS_VERBOSE, "PC port at 1000Mbps, disabling RGMII Tx delay\n");
		}
		else
		{
			/* Enable the RGMII transmit timing delay on the external PHY */
			...
			reg32 |= 0x0200; /* turn on bit 9 to enable RGMII Tx delay */
			ethHw_miiSet( port, ETHHW_MII_FLAGS_EXT, 0x1c, reg32 );
			msg_printk(MSG_LEVEL_STATUS_VERBOSE, "PC port at 10/100Mbps, enabling RGMII Tx delay\n");
		}

		pcPortSpeedPrev = pcPortSpeed;
	}
