...
static int set_icc_fc(struct bcmpmu *bcmpmu, int curr)
{
	...
	bcmpmu_chrgr_usb_en(pchrgr->bcmpmu, 0);
	...
	if (curr != 0)
		bcmpmu_chrgr_usb_en(pchrgr->bcmpmu, 1);
...
static void bcmpmu_chrgr_isr(enum bcmpmu_irq irq, void *data)
{
	...
	case PMU_IRQ_EOC:
		...
		break;
	case PMU_IRQ_USBOV:
		pr_info("PMU_IRQ_USBOV\n");
		ret = 1;
		blocking_notifier_call_chain(&pchrgr->bcmpmu->event
					     [BCMPMU_CHRGR_EVENT_USBOV].
					     notifiers,
					     BCMPMU_CHRGR_EVENT_USBOV, &ret);
		break;
	case PMU_IRQ_USBOV_DIS:
		pr_info("PMU_IRQ_USBOV_DIS\n");
		ret = 0;
		blocking_notifier_call_chain(&pchrgr->bcmpmu->event
					     [BCMPMU_CHRGR_EVENT_USBOV].
					     notifiers,
