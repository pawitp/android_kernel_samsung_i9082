...
static struct regulator *VCC_PDA_2_8_V;  //mmcldo2_uc
static int intpin_count=0;
...
{
	s16 x, y, z;
	struct iio_dev *indio_dev = i2c_get_clientdata(this_client);
	struct inv_mpu_iio_s *st = iio_priv(indio_dev);  
	int retval = 0;
	unsigned char data[6];
        u16 accel_sens_trim = 16384;  
	int cal_div = accel_sens_trim / 1024;

	retval = inv_i2c_read(st, REG_RAW_ACCEL, BYTES_PER_SENSOR, data);
...
static int accel_open_calibration(void)
{
	...
	if (IS_ERR(cal_filp)) {
		pr_err("[MPU]%s: Can't open calibration file\n", __func__);
		set_fs(old_fs);
	...
	if (err != 3 * sizeof(s16)) {
		pr_err("[MPU]%s: Can't read the cal data from file\n", __func__);
		err = -EIO;
        }

	printk(KERN_INFO"[MPU]%s: (%d,%d,%d)\n", __func__,
...
static int accel_do_calibrate(int enable)
{
	...
	mm_segment_t old_fs;
        u16 accel_sens_trim = 16384;  
	int cal_div = accel_sens_trim / 1024;    
	int ideal_z = IDEAL_Z * st->plat_data.orientation[8];

	for (i = 0; i < CALIBRATION_DATA_AMOUNT; i++) {
		err = read_accel_raw_xyz(&data);
		if (err < 0) {
			pr_err("[MPU]%s: accel_read_accel_raw_xyz() "
	...
	printk(KERN_INFO "[MPU]%s: cal data (%d,%d,%d)\n", __func__,
			cal_data.x, cal_data.y, cal_data.z);
	...
	err = cal_filp->f_op->write(cal_filp,
		(char *)&cal_data, 3 * sizeof(s16), &cal_filp->f_pos);
	if (err != 3 * sizeof(s16)) {
		pr_err("[MPU]%s: Can't write the cal data to file\n", __func__);
		err = -EIO;
	}
...
#ifdef FEATURE_IKR_INTERRUPT_TEST
        st->interrupt_count = 0;
        st->interrupt_test_mode = false;
#endif //FEATURE_IKR_INTERRUPT_TEST
	return 0;
}
	...
	case ATTR_SELF_TEST:
        {
            ...
            result = inv_sw_self_test(st, gyro_bias, gyro_rms, gyro_lsb_bias);
            return sprintf(buf, "%d "  
            "%d.%02d %d.%02d %d.%02d " 
            "%d.%02d %d.%02d %d.%02d ",	
            result, 
            (int)abs(gyro_bias[0]/100), (int)abs(gyro_bias[0])%100,
            (int)abs(gyro_bias[1]/100), (int)abs(gyro_bias[1])%100,
            (int)abs(gyro_bias[2]/100), (int)abs(gyro_bias[2])%100,
            gyro_rms[0]/100, (int)abs(gyro_rms[0])%100,
            gyro_rms[1]/100, (int)abs(gyro_rms[1])%100,
            gyro_rms[2]/100, (int)abs(gyro_rms[2])%100		
            );
        }
...
/**
 *  inv_acc_cal_open() - Read accel cal data from filesystem.
 */
static ssize_t inv_acc_cal_open(struct device *dev,
	struct device_attribute *attr, char *buf)
{
...
static IIO_DEV_ATTR_SAMP_FREQ(S_IRUGO | S_IWUSR, inv_fifo_rate_show,
	inv_fifo_rate_store);
static DEVICE_ATTR(mpu_temperature, S_IRUGO, inv_temperature_show, NULL);
...
static DEVICE_ATTR(accel_cal_open, S_IRUGO, inv_acc_cal_open, NULL);
#ifdef CONFIG_INV_TESTING
...
static ssize_t mpu_factory_power_on(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int count = 0;
...
static ssize_t mpu_factory_temperature(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    	struct iio_dev *indio_dev = i2c_get_clientdata(this_client);
	struct inv_mpu_iio_s *st = iio_priv(indio_dev);  
	...
	retval = st->set_power_state(st, true);
	if (retval) {
		pr_err("Could not power on.\n");
		return retval;        
	}

	if (st->chip_config.is_asleep)
	{
    		pr_err("chip_config.is_asleep : %d.\n", st->chip_config.is_asleep);
		return -EPERM;
	}
	retval = inv_i2c_read(st, reg->temperature, 2, data);
	if (retval) {
		pr_err("Could not read temperature register.\n");
		return retval;
	}
	...
	printk(KERN_INFO"read scale_t = %ld\n", scale_t);
        temperature = (short int)(scale_t / 65536.f);
...
static ssize_t mpu_factory_raw_data(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = i2c_get_clientdata(this_client);
	struct inv_mpu_iio_s *st = iio_priv(indio_dev);  
...
static ssize_t mpu_factory_vendor_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%s\n", MPU_VENDOR);
}

static ssize_t mpu_factory_name_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%s\n", MPU_PART_ID);
}

static ssize_t mpu_factory_selftest(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    	struct iio_dev *indio_dev = i2c_get_clientdata(this_client);
	struct inv_mpu_iio_s *st = iio_priv(indio_dev);  
	...
	printk(KERN_INFO"[MPU] %s\n", __func__);

        result = inv_sw_self_test(st, gyro_bias, gyro_rms, gyro_lsb_bias);

        printk(KERN_INFO "%s : %d.%02d, %d.%02d, %d.%02d, %d.%02d, %d.%02d, %d.%02d, %d\n",
            __func__,
            (int)abs(gyro_bias[0]/100), (int)abs(gyro_bias[0])%100,
            (int)abs(gyro_bias[1]/100), (int)abs(gyro_bias[1])%100,
            (int)abs(gyro_bias[2]/100), (int)abs(gyro_bias[2])%100,
        ...
        count = sprintf(buf, "%d.%02d,%d.%02d,%d.%02d,%d.%02d,%d.%02d,%d.%02d,%d,%d\n",	
            (int)abs(gyro_bias[0]/100), (int)abs(gyro_bias[0])%100,
            (int)abs(gyro_bias[1]/100), (int)abs(gyro_bias[1])%100,
            (int)abs(gyro_bias[2]/100), (int)abs(gyro_bias[2])%100,
            gyro_rms[0]/100, (int)abs(gyro_rms[0])%100,
					...
					char *buf)
{
	...
	printk(KERN_INFO "cal_data : %d %d %d\n", 	cal_data.x, cal_data.y, cal_data.z);
...
static ssize_t mpu_factory_reactive_alert_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	...
	if (sysfs_streq(buf, "1")) {
        ...
        printk(KERN_INFO "[MPU] mpu_factory_reactive_alert_store : onoff=%d\n", onoff);

	if (onoff ) {
		pr_info("reactive alert is on.\n");
                intpin_count = -1;
                intpin_count = inv_interrupt_pin_test(st);
	} else{
...
static ssize_t mpu_factory_reactive_alert_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{    

        printk(KERN_INFO "[MPU] mpu_factory_reactive_alert_show : intpin_count=%d\n", intpin_count);    
        
	return sprintf(buf, "%d\n", (intpin_count==0)?1:0);
}

static DEVICE_ATTR(power_on, S_IRUGO, mpu_factory_power_on, NULL);

static DEVICE_ATTR(temperature, S_IRUGO, mpu_factory_temperature, NULL);

static DEVICE_ATTR(calibration, S_IRUGO|S_IWUSR|S_IWGRP,
	accel_calibration_show, accel_calibration_store);

static DEVICE_ATTR(raw_data, S_IRUGO, mpu_factory_raw_data, NULL);

static DEVICE_ATTR(reactive_alert, S_IRUGO|S_IWUSR|S_IWGRP,
	mpu_factory_reactive_alert_show,
	mpu_factory_reactive_alert_store);

static DEVICE_ATTR(vendor, S_IRUGO, mpu_factory_vendor_show, NULL);
static DEVICE_ATTR(name, S_IRUGO, mpu_factory_name_show, NULL);
static DEVICE_ATTR(selftest, S_IRUGO, mpu_factory_selftest, NULL);
	...
	const struct i2c_device_id *id)
{
	...
	if (client->irq) {
		int gpio_pin;
		dev_info(&client->adapter->dev, "Installing irq using %d\n", client->irq);
		/* BRCM */
		/* set up the GPIO pin */
		...
		result = gpio_request(gpio_pin, "mpu6500_int");
		if (result < 0) {
			dev_err(&client->adapter->dev,
        ...
        {	
	    dev_warn(&client->adapter->dev,  "Missing MPU6500 IRQ\n");
	}
	...
	result = sensors_register(gyro_sensor_device, NULL,
	...
	result = sensors_register(accel_sensor_device, NULL,
...
static int __init inv_mpu_init(void)
{
	...
	if(IS_ERR(VCC_PDA_2_8_V)){
		printk(KERN_ERR "[MPU] can not get VCC_PDA_2.8V\n");
	}	

        result = regulator_is_enabled(VCC_PDA_2_8_V);
        printk(KERN_INFO "[MPU] regulator_is_enabled : %d\n", result);

         result = regulator_set_voltage(VCC_PDA_2_8_V,2800000,2800000);	
        printk(KERN_INFO "[MPU] regulator_set_voltage : %d\n", result);

        result = regulator_enable(VCC_PDA_2_8_V);
        printk(KERN_INFO "[MPU] regulator_enable : %d\n", result);
