...
static int inv_set_interrupt_test(struct inv_mpu_iio_s *st, bool mode);
static int inv_get_interrupt_count(struct inv_mpu_iio_s *st);
...
// IKR ADD for S/W Self Test 
int inv_sw_self_test(struct inv_mpu_iio_s *st,int* gyro_bias_regular,int* gyro_rms_regular,int* gyro_lsb_bias_regular)
{
	int result = 0;
	int test_times;
	char gyro_result;
...
int inv_interrupt_pin_test(struct inv_mpu_iio_s *st)
{
	int result = 0;
	int test_times;
	char gyro_result;
...
#ifdef FEATURE_IKR_INTERRUPT_TEST
/**
 *  inv_interrupt_test() - do the actual test of interrupt pin testing
 */
int inv_interrupt_test(struct inv_mpu_iio_s *st)
{
	struct inv_reg_map_s *reg;
	int result=0, interrupt_count=0;
    ...
    result = inv_set_interrupt_test(st, true);
    ...
    /* interrupt enable*/
	result = inv_i2c_single_write(st, reg->int_enable, st->plat_data.int_config|BIT_DATA_RDY_EN);
	if (result)
	  return result;
    ...
    interrupt_count = inv_get_interrupt_count(st);

    printk(KERN_INFO "%s : interrupt_count=%d\n",__func__, interrupt_count);

    result = inv_set_interrupt_test(st, false);
