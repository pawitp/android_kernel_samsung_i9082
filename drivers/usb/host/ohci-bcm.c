...
static const struct hc_driver ohci_bcm_hcd_driver = {
...
int bcm_ohci_probe(struct platform_device *pdev)
{
	...
	hcd = usb_create_hcd(&ohci_bcm_hcd_driver,
	...
	/* struct ohci_regs def'd in Linux ohci.h
	* which is included by Linux ohci-hcd.c
	*/
	usb->hcd = hcd;
...
{
	struct ohci_bcm_usb_cfg *usb = platform_get_drvdata(pdev);
