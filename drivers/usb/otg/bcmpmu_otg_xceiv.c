...
static int bcmpmu_otg_xceiv_vbus_invalid_notif_handler(
	struct notifier_block *nb, unsigned long value, void *data)
{
	struct bcmpmu_otg_xceiv_data *xceiv_data =
		container_of(nb, struct bcmpmu_otg_xceiv_data,
			     bcm_otg_vbus_invalidity_notifier);

	if (!xceiv_data)
		return -EINVAL;

	queue_work(xceiv_data->bcm_otg_work_queue,
		&xceiv_data->bcm_otg_vbus_invalid_work);
					  ...
					  char *buf)
{
	...
	return snprintf(buf, strlen("1") + 1,
...
static void bcmpmu_otg_xceiv_vbus_invalid_handler(
