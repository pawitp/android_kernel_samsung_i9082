...
static int setup_gpios(struct tft_panel_platform_data *plt_data)
{
	...
	if (plt_data->gpio_lcd_pwr_en >= 0) {
		...
		printk(KERN_INFO "%s() LCD Power Enable GPIO requested:"
		       " %d, status: %d\n",
		       __func__, plt_data->gpio_lcd_pwr_en, rc);
	...
	if (plt_data->gpio_lcd_reset >= 0) {
		...
		printk(KERN_INFO "%s() LCD Reset GPIO requested:"
		       " %d, status: %d\n",
		       __func__, plt_data->gpio_lcd_reset, rc);
		if (rc) {
			close_gpios(plt_data);
			return rc;
		}
	...
	if (plt_data->gpio_bl_en >= 0) {
		...
		printk(KERN_INFO
		       "%s() Backlight Enable GPIO requested: %d, status: %d\n",
		       __func__, plt_data->gpio_bl_en, rc);
		if (rc) {
			close_gpios(plt_data);
			return rc;
		}
	...
	if (plt_data->gpio_bl_pwr_en >= 0) {
		...
		printk(KERN_INFO
		       "%s() Backlight Power Enable requested: %d, status: %d\n",
		       __func__, plt_data->gpio_bl_pwr_en, rc);
		if (rc) {
			close_gpios(plt_data);
			return rc;
		}
	...
	if (plt_data->gpio_bl_pwm >= 0) {
		...
		printk(KERN_INFO
		       "%s() Backlight PWM GPIO requested: %d, status: %d\n",
		       __func__, plt_data->gpio_bl_pwm, rc);
		if (rc) {
			close_gpios(plt_data);
			return rc;
		}
