...
struct brcm_consw {
	...
	void	(*con_putcs)(struct vc_data *, const unsigned short *,
		int, int, int);
...
/* int register_con_driver(const struct brcm_consw *csw,
	int first, int last); */
/* int unregister_con_driver(const struct brcm_consw *csw); */
int take_over_brcm_console(const struct brcm_consw *sw,
 ...
 * when the first real brcm_console is registered.  This is for early-printk
 * drivers.
 */
...
extern int add_preferred_brcm_console(char *name, int idx,
	char *options);
extern int update_brcm_console_cmdline(char *name, int idx,
	char *name_new, int idx_new, char *options);
...
#if 1
#define WARN_CONSOLE_UNLOCKED()	WARN_ON(!is_brcm_console_locked() \
	&& !oops_in_progress)
#else
