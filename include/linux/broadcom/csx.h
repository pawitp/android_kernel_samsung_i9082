...
#define CSX_POINT_ENABLE          1
	/* Value associated with an enabled capture/inject point */
#define CSX_POINT_DISABLE         0
	/* Value associated with a disabled capture/inject point */
#define CSX_HANDLE_INVALID        0
	/* Value associated with invalid handle */
...
typedef struct csx_io_point_info {
	CSX_IO_MODULE csx_module_id;
	/* Identifies module location for I/O point */
	int csx_device_id;
	/* Identifies device within module.  Information specific to module */
	int csx_point_id;
	/* Identifies point within device.  Information specific to device */
	CSX_IO_MODE csx_io_mode;	/* I/O mode setting */
	int csx_io_sync;
	/* Boolean flag to indicate synchronized enable/disable operation */
