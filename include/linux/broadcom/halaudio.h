...
*/
typedef void (*HALAUDIO_CODEC_IORW_CB) (int bytes,
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_SET_FREQ_OP) (int chno,
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_GET_FREQ_OP) (int chno,
				    /**< (i) Codec channel number */
					   int *freqhz
...
*/
typedef int (*HALAUDIO_CODEC_SET_ANA_OP) (int chno,
				    /**< (i) Codec channel number */
					  int db,
				    /**< (i) Gain in db */
					  HALAUDIO_DIR dir,
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_GET_ANA_OP) (int chno,
				    /**< (i) codec channel number */
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_SET_DIG_OP) (int chno,
				    /**< (i) Codec channel number */
					  int db,
				    /**< (i) Gain in db */
					  HALAUDIO_DIR dir
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_GET_DIG_OP) (int chno,
				    /**< (i) codec channel number */
					  HALAUDIO_GAIN_INFO * info,
				    /**< (o) Ptr to gain info structure */
					  HALAUDIO_DIR dir
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_SET_SIDETONE_OP) (int chno,
				    /**< (i) Codec channel number */
					       int db
				    /**< (i) Gain in db */
);
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_GET_SIDETONE_OP) (int chno,
				    /**< (i) codec channel number */
					       HALAUDIO_GAIN_INFO * info
				    /**< (o) Ptr to gain info structure */
);
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_SET_EQU_OP) (int chno,
				    /**< (i) Codec channel number */
					  HALAUDIO_DIR dir,
				    /**< (i) Direction path */
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_GET_EQU_OP) (int chno,
				    /**< (i) Codec channel number */
					  HALAUDIO_DIR dir,
				    /**< (i) Direction path */
					  HALAUDIO_EQU * equ
...
*     -ve      Error code
*/
typedef int (*HALAUDIO_CODEC_GET_INFO_OP) (int chno,
				    /**< (i) Codec channel number */
...
typedef struct halaudio_api_funcs {
	...
	int (*write) (HALAUDIO_HDL, HALAUDIO_CODEC, HALAUDIO_FMT,
		      const uint8_t *, int);
	int (*read) (HALAUDIO_HDL, HALAUDIO_CODEC, HALAUDIO_FMT, uint8_t *,
		     int);
	int (*setSuperUser) (HALAUDIO_HDL, int);
	int (*setFreq) (HALAUDIO_HDL, HALAUDIO_CODEC, int);
	int (*getFreq) (HALAUDIO_HDL, HALAUDIO_CODEC, int *);
	int (*setEquParms) (HALAUDIO_HDL, HALAUDIO_CODEC, HALAUDIO_DIR,
			    const HALAUDIO_EQU *);
	int (*getEquParms) (HALAUDIO_HDL, HALAUDIO_CODEC, HALAUDIO_DIR,
			    HALAUDIO_EQU *);
	int (*getHardwareInfo) (HALAUDIO_HDL, HALAUDIO_HW_INFO *);
	int (*getInterfaceInfo) (HALAUDIO_HDL, HALAUDIO_IF, HALAUDIO_IF_INFO *);
	int (*getCodecInfo) (HALAUDIO_HDL, HALAUDIO_CODEC,
			     HALAUDIO_CODEC_INFO *);
	int (*enableInterface) (HALAUDIO_HDL, HALAUDIO_IF, int);

	/* Kernel specific API */
	int (*addInterface) (HALAUDIO_IF_OPS *, unsigned int, const char *, int,
			     int, HALAUDIO_IF_HDL *);
	int (*delInterface) (HALAUDIO_IF_HDL);
	int (*setSyncFlag) (HALAUDIO_IF_HDL, int);
	int (*setCsxIoPoints) (HALAUDIO_CODEC, HALAUDIO_CSX_POINT_ID,
			       const CSX_IO_POINT_FNCS *, void *);
...
*     -ENODEV  Non-existent codec or client
*/
	static inline int halAudioLockCodecByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
						  const char *name
				    /**< (i) Name of codec to lock */
	    ) {
		HALAUDIO_CODEC cid;
		int err;
...
*     -ENODEV  Non-existing codec
*/
	static inline int halAudioUnlockCodecByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
						    const char *name
				    /**< (i) Name of code to unlock */
	    ) {
		HALAUDIO_CODEC cid;
		int err;
		err = halAudioQueryCodecByName(client_hdl, name, &cid);
...
*/
	int halAudioRead(HALAUDIO_HDL client_hdl,
			 ...
			 HALAUDIO_FMT format,
				    /**< (i) Format of samples */
			 uint8_t *audio,
				    /**< (o) Ptr to data buffer to record to */
...
*     -ve      Error code
*/
	static inline int halAudioReadByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
					     const char *name,
				    /**< (i) Name of codec channel */
					     HALAUDIO_FMT format,
...
*/
	int halAudioSetFreq(HALAUDIO_HDL client_hdl,
			    ...
			    HALAUDIO_CODEC cid,
				    /**< (i) Codec channel */
			    int freqhz
				    /**< (i) Sampling frequency in Hz to set */
	    );
...
*     -EPERM   Parent interface is disabled
*/
	static inline int halAudioSetFreqByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
						const char *name,
				    /**< (i) Name of codec channel */
						int freqhz
...
*     -EPERM   Parent interface is disabled
*/
	static inline int halAudioGetFreqByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
						const char *name,
				    /**< (i) Name of codec channel */
						int *freqhz
...
*     -EPERM   Parent interface is disabled
*/
	static inline int halAudioSetEquParmsByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
						    const char *name,
				    /**< (i) Name of codec */
						    HALAUDIO_DIR dir,
...
*     -EPERM   Parent interface is disabled
*/
	static inline int halAudioGetCodecInfoByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
						     const char *name,
				    /**< (i) Name of codec channel */
						     HALAUDIO_CODEC_INFO *info
...
*     -EINVAL  Invalid parameters
*/
	static inline int halAudioEnableInterfaceByName(HALAUDIO_HDL client_hdl,
				    /**< (i) Client handle */
							const char *name,
				    /**< (i) Interface name */
							int enable
