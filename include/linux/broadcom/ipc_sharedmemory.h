...
#define IPC_FIFOCOUNT(Fifo)\
	((Fifo->WriteIndex - Fifo->ReadIndex + IPC_SM_MAX_BUFFERS) \
		& ~IPC_SM_MAX_BUFFERS)
...
#define IPC_EVENT_SET(EventPtr) (*SmLocalControl.Event.Set) \
	((void *) EventPtr)
#define IPC_EVENT_CLEAR(EventPtr) (*SmLocalControl.Event.Clear)	\
		((void *) EventPtr)
#define IPC_EVENT_WAIT(EventPtr, MilliSeconds) (*SmLocalControl.Event.Wait) \
		((void *) EventPtr, MilliSeconds)
...
#define	IPC_SmEndpointInfo(EndpointId) \
	(&SmLocalControl.SmControl->Endpoints[EndpointId])

#define IPC_SmFreeBuffer(Buffer, OwningCpu) IPC_SmFifoWrite\
