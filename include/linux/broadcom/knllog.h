...
*****************************************************************************
  ...
  NOTES:
  There are entries in /proc/sys/knllog, and the actual log buffer is in
  /proc/knllog.
  /proc/sys/knllog/enable - enable (1) or disable (1) logging. e.g.
  echo 1 >/proc/sys/knllog/enable
  /proc/sys/knllog/entries - resize log buffer.
  e.g. echo 10000 >/proc/sys/knllog/entries
  /proc/sys/knllog/clear - clear/empty the log buffer.
  e.g. echo 1 >/proc/sys/knllog/clear
  /proc/sys/knllog/deltatime - display timestamp deltas on log dump.
  e.g. echo 1 >/proc/sys/knllog/deltatime
  /proc/sys/knllog/idx - READ ONLY - show the current next log entry
  index 0 .. entries-1
  /proc/sys/knllog/wrap - READ ONLY - show the current buffer wrapped flag

  To dump the log buffer from the cmd line, use
  "echo 1 > /proc/sys/knllog/dump-log"
...
#define KNLLOG_DUMP_MEM(addr, mem, numBytes)\
	knllog_dump_mem(__FUNCTION__, addr, mem, numBytes)
...
extern void knllog_dump_mem(const char *function, uint32_t addr,
			    const void *voidMem, size_t numBytes);
...
/*
 * A global flag that code inside CONFIG_BCM_KNLLOG_IRQ checks to
 * decide to log or not.
 * This allows a powerful debug feature to be enabled at runtime i.e.
 * "echo 0xf >irq_sched_enable"
 */
