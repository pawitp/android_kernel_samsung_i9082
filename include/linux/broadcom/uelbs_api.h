...
#define	LBS_MAX_NUM_GSM_NCELL		32
	...
	typedef struct {
	/** Mobile country code that identifies uniquely the country
		of domicile of the mobile subscriber.
		Valid values: 0x0000 to 0xFFFF; Invalid/default value: 0xFFFF.
		See 3GPP TS 03.03, sub-clause 4.1.
		Also see 3GPP TS 24.008, subclause
		10.5.1.3 for MCC contained in LAI
		(Location Area Identification). */
		UInt16 mcc;

	/** Mobile network code that identifies the home GSM PLMN of the mobile
		subscriber. Valid values: 0x0000 to 0xFFFF; Invalid/default
		value: 0x00FF. See 3GPP TS 03.03, sub-clause 4.1.
		Also see 3GPP TS 24.008, sub-clause 10.5.1.3
		for mnc contained in LAI (Location Area Identification). */
		UInt16 mnc;

	/** Location area code that identifies a location area within a
		GSM PLMN. Valid values: 0x0000 to 0xFFFF;
		Invalid/default value: 0xFFFF. Value of 0xFFFE
		represents deleted LAI. See 3GPP TS 03.03, sub-clause 4.1.
		Also see 3GPP TS 24.008, sub-clause 10.5.1.3 for LAC contained
		in LAI (Location Area Identification). */
		UInt16 lac;

	} T_LBS_MM_Params;

	typedef struct {
	/** Cell identification that identifies the cell within a
		location area. Valid value: 0x0000 to 0xFFFF;
		Invalid/default: 0x0000. See 3GPP TS 03.03, sub-clause
		4.3.1. */
		UInt16 cell_id;

	/** Absolute radio frequency channel number in idle mode.
		Valid values: 0 to
		1023; Invalid/default value: 0xFFFF.
		See 3GPP TS 05.05, sub-clause 2.
		@note the most significant bit is changed from 1 to 0
		for PCS band. So it contains the
		actual arfcn number in any band. */
		UInt16 arfcn;

	/** contains the value of the RXLEV (receive power level) parameter
		for the serving cell calculated by the mobile station.
		Valid values: 0 to 63; Invalid/default
		value: 0xFF. See 3GPP TS 05.08, sub-clause 8.1.3, 8.1.4. */
		UInt8 rxlev;

	/** The base station identity code that allows a MS to distinguish
		between different neighbouring base stations.
	...
	typedef struct {
		...
		/* used frequency. Contains serving cell in non-DCH state,
		 * and active set cells in DCH state */
		T_LBS_UMTS_MeasFreq active;

		/* non-used frequencies, up to 2 frequencies, up to 32 cells
		 * each frequency, but
...
#if 0
/* just for compiling purpose, once CAPI change is done, it can be removed */
		UInt32 cell_identity;
		UInt16 psc;
		UInt16 uarfcn_DL;
		UInt16 uarfcn_UL;
...
#if defined(UMTS) || defined(CPLANE_RRC_API_INCLUDED)
	/* UMTS neighbor cells. up to 2 frequencies,
	up to 32 cells each frequency, but
	 * total is no greater than 32 cells */
	UInt8 num_umts_freq;
	T_LBS_UMTS_MeasFreq umts_freqs[LBS_MAX_NUM_UMTS_FREQ];
#endif
...
/*The serving cell information.*/
	typedef struct {
		UInt8 mRAT;
		///< Current Radio Access Technology: RAT_NOT_AVAILABLE(0),RAT_GSM(1),RAT_UMTS(2)
