...
static int block_device(struct sock *sk, u16 index, void *data, u16 len)
{
	...
	BT_DBG("[Moonys]", __FUNCTION__);
	BT_DBG("hci%u len:%d, %d", index,len,sizeof(*cp));

	if (len != sizeof(*cp))
	{
		BT_DBG("[Moonys Debug 111]");
	...
	if (!hdev)
	{
		BT_DBG("[Moonys Debug 222] index %d", index);
		return cmd_status(sk, index, MGMT_OP_BLOCK_DEVICE,
						MGMT_STATUS_INVALID_PARAMS);
	}
	...
	if (err < 0)
	{
		BT_DBG("[Moonys Debug 333]");
		err = cmd_status(sk, index, MGMT_OP_BLOCK_DEVICE,
						MGMT_STATUS_FAILED);
	}
	else
	{
		BT_DBG("[Moonys Debug 444]");
